<!--
 * @Author: sunch
 * @Date: 2020-04-20 10:06:12
 * @LastEditTime: 2020-10-09 14:30:13
 * @LastEditors: Please set LastEditors
 * @Description: 工资条打印
 * @FilePath: /ufgov-vue/src/views/prs/report/wageSlipPrint/wageSlipPrint.vue
 -->
<template>
  <div>
    <!-- 头部开始 -->
    <ufHeader :title="title">
      <template slot="btns">
        <a-button class="mr-5" :class="getBtnPer('btn-print')" @click="print" :loading="tempModalLoading">打印</a-button>
        <a-button class="mr-5" :class="getBtnPer('btn-export')" @click="exportEvent">导出</a-button>
        <a-button class="mr-5" :class="getBtnPer('btn-mail')" @click="sendMail">发邮件</a-button>
        <!-- 普通导出废弃 使用上面的自定义导出 解决[CWYXM-14527] 导出的金额单元格是字符类型的问题 -->
    ...
    <vxe-grid
      ...
      :row-style="rowStyle"
      :toolbar="{ id: 'wageOrgReportTable', resizable: { storage: true } }"
    >
...
import { defaultColumns, defaultCheckedFields, defaultExportFormat } from './wageSlipPrint' //固定显示的列
...
export default {
  ...
  methods: {
    ...
    sendMail(){
      const that = this
      this.$confirm({
        title: `即将为当前工资类别【${this.prtypeName}】查询结果中的人员发送邮件，请确认是否发送？`,
        content: "",
        okText: "确定",
        cancelText: "取消",
        onOk() {
          ...
          let argu = {
            orgCodes: that.orgs,
            prtypeCodes: that.prtypeCode === '*'?that.prtypeCodes : [that.prtypeCode],
            flag: that.dataFrom, //RPT_CALC 未结账当月数据 RPT_PAYLIST 已结账历史数据
            monthStart: that.dataFrom === 'RPT_PAYLIST' ? that.monthStart : '',
            monthEnd: that.dataFrom === 'RPT_PAYLIST' ? that.monthEnd : '',
            payNoMo: that.dataFrom === 'RPT_PAYLIST' ? that.payNoMo : '',
            totalType: that.tjType, //0  dept
            isShowZero: that.isShowZero?'1':'0',
            empCode: that.empCode,
            empNames: that.empNames.split(/\uff0c|\s+/)//中文逗号或空格分隔
          }
          that.$axios.post('/prs/rpt/PrsRptData/sendSalarySlipPrintData', argu).then(result => {
            console.log(result)
            if(result.data.flag === 'success'){
              if(result.data.data.indexOf('发送失败') > -1){
                that.$warning({
                  title: result.data.data.substr(0, result.data.data.indexOf('发送失败：')+4),
    ...
    onClickTabItem(item) {
      ...
      this.$axios
        .get('/prs/rpt/PrsRptData/getMaxPayNoMo?prtypeCode=' + (item.prtypeCode === '*' ? '' : item.prtypeCode))
        .then(result => {
          console.log(result)
          if (result.data.flag != 'success') {
            throw result.data.msg
    ...
    printModalConfirm(tempInfo){
      ...
      let that = this
      this.$axios
        .post('/prs/rpt/PrsRptData/printSalarySlipPrintData', argu)
        .then(result => {
          if (result.data.flag === 'success') {
            console.log(result)
    ...
    exportData() {
      ...
      this.$refs.xTable.exportData({
        filename: nameCode + this.pfData.svSetYear + '年' + this.mo + '月工资条导出',
        sheetName: 'Sheet1',
        type: 'xlsx'
      })
    },
    ...
    exportEvent () {
      ...
      setTimeout(()=>{
        ...
        setTimeout(()=>{
          ...
          let nameCode = this.getAgencyNameCode()
          // 保存导出
          saveAs(blob, nameCode + this.pfData.svSetYear + '年' + this.mo + '月工资条打印导出.xlsx',)
