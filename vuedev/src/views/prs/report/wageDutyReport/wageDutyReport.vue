<!--
 * @Author: sunch
 * @Date: 2020-04-13 14:41:38
 * @LastEditTime: 2020-10-09 14:29:25
 * @LastEditors: Please set LastEditors
 * @Description: 职务工资汇总表
 * @FilePath: /ufgov-vue/src/views/prs/report/wageDutyReport/wageDutyReport.vue
 -->
 <template>
  <div>
    <!-- 头部开始 -->
    ...
    <vxe-grid
      ...
      :row-style="rowStyle"
      :toolbar="{ id: 'wageDutyReportTable', resizable: { storage: true } }"
    >
...
import { defaultColumns, defaultCheckedFields, defaultExportFormat } from './wageDutyReport' //固定显示的列
...
export default {
  ...
  data() {
    return {
      title: '职务工资汇总表', //页面标题
      pageId: 'PRS_DUTYGRADE_SALARY_DATA',
      defaultColumns, //默认列
      firstColumn,
      tabList: [], //tab对象数组
      tabIndex: 0, //当前tab角标
  ...
  mounted() {
    // console.log('this.pfData: ', JSON.stringify(this.pfData))
    ...
    exportEvent () {
      ...
      setTimeout(()=>{
        ...
        setTimeout(()=>{
          ...
          // 保存导出
          saveAs(blob, nameCode + this.pfData.svSetYear + '年' + this.mo + '月职务工资汇总表导出.xlsx',)
