<!--
 * @Author: sunch
 * @Date: 2020-04-13 14:42:19
 * @LastEditTime: 2020-10-09 14:30:24
 * @LastEditors: Please set LastEditors
 * @Description: 类别工资汇总表
 * @FilePath: /ufgov-vue/src/views/prs/report/wageTypeReport/wageTypeReport.vue
 -->
 <template>
  <div>
    <!-- 头部开始 -->
    ...
    <ufQueryArea class="mt-10">
      <div class="flex-between">
        <div class="flex-start">
          ...
          <div class="ml-10">
            <a-checkbox :checked="isShowZero" @change="onNoShowEmptyChange">金额为0的列不显示</a-checkbox>
          </div>
        </div>
    ...
    <vxe-grid
      ...
      :row-style="rowStyle"
      :toolbar="{ id: 'wageTypeReportTable', resizable: { storage: true } }"
    >
...
import { defaultColumns, defaultCheckedFields, defaultExportFormat } from './wageTypeReport' //固定显示的列
...
export default {
  ...
  watch: {},
  created() {
    this.tableH = this.containerH - 170
  ...
  methods: {
    ...
    getSalaryByConditionData(prtypeCodes) {
      ...
      this.$axios
        .post('/prs/rpt/PrsRptData/getSummarySalaryByConditionData', argu)
        .then(result => {
          // console.log(result)
    ...
    printModalConfirm(tempInfo){
      ...
      let that = this
      this.$axios
        .post('/prs/rpt/PrsRptData/printSummarySalaryByConditionData', argu)
        .then(result => {
          if (result.data.flag === 'success') {
            console.log(result)
    ...
    //     filename: nameCode + this.pfData.svSetYear + '年' + this.mo + '月类别工资汇总表导出',
    //     sheetName: 'Sheet1',
    //     type: 'xlsx'
    //   })
    // },
    ...
    exportEvent () {
      ...
      setTimeout(()=>{
        ...
        setTimeout(()=>{
          ...
          let nameCode = this.getAgencyNameCode()
          // 保存导出
          saveAs(blob, nameCode + this.pfData.svSetYear + '年' + this.mo + '月类别工资汇总表导出.xlsx')
