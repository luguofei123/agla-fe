<template>
  <div>
    ...
    <!-- 表格部分开始 -->
    <div
      v-show="tableHeight > 60"
      class="table-wrap"
      :class="zoomButtonVisible ? 'tableZoom' : ''"
    >
      <vxe-grid
        head-align="center"
        :height="tableHeight"
        ...
        :toolbar="{ id: 'glRptLedgerTable', resizable: { storage: true } }"
      ...
      <ufPager :pager-config="tablePage" @page-change="handlePageChange">
        <template v-slot:footerBtns>
          ...
          <div class="calculator">
            <!-- <a-checkbox @change="calculatorChange" v-model="calculatorVal">计算总金额</a-checkbox> -->
            <!-- v-if="calculatorVal" -->
            <div class="calculatorRes">
              选中合计：
              <span
                class="total-num"
                ref="copy"
...
import SettingBar from "./components/SettingBarRptLedger"; // 设置表格单位/格式
import SettingProjectModal from "../components/SettingProjectModal"; // 设置方案
import { tableColumnsSANLAN, tableColumnsWAIBI } from "./glRptLedgerColumns";
import {
  GetReportData,
  getRptAccas,
  postBatchExport,
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      tableH: (state) => state.glRptLedger.tableH, //表格高度
      agencyCode: (state) => state.glRptLedger.agencyCode, // 单位代码
      agencyName: (state) => state.glRptLedger.agencyName, // 单位名称
      acctCode: (state) => state.glRptLedger.acctCode, // 账套代码
      acctName: (state) => state.glRptLedger.acctName, // 账套名称
      startDate: (state) => state.glRptLedger.startDate,
      endDate: (state) => state.glRptLedger.endDate,
      prjGuid: (state) => state.glRptLedger.prjGuid, //方案guid
      nowPrj: (state) => state.glRptLedger.nowPrj, //方案名称
      fromWhere: (state) => state.fromWhere, //从哪里跳转
    }),
    ...mapGetters({
      reportArgument: "getLedgerArgument", //this.tableData接口的参数拼接
    }),
  },
  methods: {
    ...mapActions([
    ...
    // 计算器
    // calculatorChange(e, item) {
    //   this.calculatorVal = e.target.checked;
    //   if (!e.target.checked) {
    //     this.clickCell = [];
    //     this.totalNum = 0;
    //   }
    ...
    addColumns(val, nowal) {
      ...
      if (nowal === true) {
        ...
        columnsArr1.splice(3, 0, ...val);
        columnsArr3.splice(3, 0, ...val);
        columnsArr1.splice(3, 0, ...moreColumns);
        columnsArr3.splice(3, 0, ...moreColumns);
      }
    ...
    setResizeColumnsWidth() {
      let tableWidthObj = localStorage.getItem("VXE_TABLE_CUSTOM_COLUMN_WIDTH"),
        obj = null;
      if (tableWidthObj) {
        obj = JSON.parse(tableWidthObj).glRptLedgerTable;
        if (!obj) {
          return;
        }
      } else {
    ...
    getTableData() {
      ...
      // this.loading = true;
      // 获得查询列表参数
      let opt = this.getQryParams();
      GetReportData("GL_RPT_LEDGER", {}, opt) // params拼接到url后面的参数
        .then((result) => {
          if (result.data.flag == "fail") {
            throw result.data.msg;
          } else {
            ...
            result.data.data.tableData.forEach((item) => {
              ...
              // item.dStadAmt = formatMoney(item.dStadAmt);
              // item.bStadAmt = formatMoney(item.bStadAmt);
              // item.cStadAmt = formatMoney(item.cStadAmt);
            });
    ...
    cellClickEvent({ row, column }, event) {
      ...
      if (column.property === "descpt" && !!row.descpt) {
        // 凭证字号列跳转到凭证录入
        // this.openJournalShow(row.descptGuid);
        this.openJournalShow(this.$(event.target));
      }
    },
    getQryParams() {
     ...
     * @description: 跳转明细账
     */
    ///pf/vue/gl/rpt/glRptJournal/glRptJournal?menuid=cef44000-2085-4fab-9565-fc6a5183019d&xFunctionId=cef44000-2085-4fab-9565-fc6a5183019d&menuname=%E6%98%8E%E7%BB%86%E8%B4%A6-vue&isNew=1
    openJournalShow($el) {
      let param = this.getQryParams();
      localStorage.setItem("from_glRptLedgerParams", JSON.stringify(param));
      this.setFromWhere("glRptLedger");
      console.log("this.pageType: ", this.pageType);
      let baseUrl = `/pf/vue/gl/rpt/glRptJournal/glRptJournal?menuid=${menuIdList.journalMenuId}&xFunctionId=cef44000-2085-4fab-9565-fc6a5183019d&action=query&dataFrom=${this.pageType}`;
      openNewPage(false, $el, "openMenu", baseUrl, false, "明细账-vue");
    },
    // table列宽拖动改变
    onTableResize(obj) {},
    showDrawer() {
