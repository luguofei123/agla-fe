  ...
  <div class="assetrc">
    ...
    <RptQueryArea @changeQueryData="getQueryData">
      <template v-slot:btns>
        ...
        <a-button type="primary" @click="save" :class="getBtnPer('btn-save')">保存</a-button>
      </template>
    </RptQueryArea>
    <!-- 查询条件结束 -->
   <!-- 设置表格单位/格式 开始 -->
    ...
    <uf-modal title="条件选择" v-model="ModalVisible" @cancel="ModalCancel" :width="720" bodySyle="modalBody">
      <div class="modal-main">
        <vxe-table
          ...
          align="center"
          :data="tableDataModal"
          :cell-style="cellStyleModal"
          :row-class-name="tableRowClassName">
          <vxe-table-column  field="name" title="总账" align="left" minWidth=150>
           <!-- <template v-slot="{ row }">
          ...
          <vxe-table-column field="assetType" title="资产">
           <template v-slot="{ row }">
          <RptTreeSelect
...
export default {
  ...
  methods: {
    ...
    // cellmouseenter ({ row, rowIndex, $rowIndex, column, columnIndex, $columnIndex, $event }) {
    //   this.tableDataModal.forEach((item,index)=>{
    //     if(item.fullName === row.fullName ) {
    //         console.log(rowIndex)
    //         console.log(index)
    //         item.setplaceholder = '请选择'
    //         // this.$set(this.tableDataModal,index,item)
    //     }
    //   })
    // },
     ...
     getglRptAssetRcModalList(param)
        .then((result) => {
          if (result.data.flag == "fail") {
            ...
            // this.tableDataModal = tableDataModalbak
...
// name: "1601 固定资产"
    ...
    change (type,obj,name) {
      ...
      this.tableDataModal.forEach((item,index)=>{
        if(item.fullName === name ) {
          ...
          item.rightId = obj.value
        }
      })
        // const xTable = this.$refs.tiaojian
        //   if (xTable) {
        //     console.log(this.tableDataModal)
        //     xTable.loadData(this.tableDataModal)
        //   }
    },
    // 条件框右侧清空时
    clearValue (name) {
    ...
    // 单元格样式
    cellStyleModal({ $rowIndex, row, column, columnIndex, $columnIndex }) {
      if (
        ...
        return {
          "text-indent": (row.levelNum-1)*8 +"px",
        };
      }
    },
    // 单元格样式
...
<style lang="scss">
.assetrc .modal-main tr>td:nth-child(2) .vxe-cell{
  overflow: visible!important;
}
.assetrc .modal-main .vxe-cell .rptTreeSelectWrap{
  border:none!important;
  background: none!important;
}
.assetrc .modal-main .vxe-cell .rptTreeSelectWrap input{
  border:none!important;
  background: none!important;
}
.assetrc .rptTreeSearchWrap:hover   .showPlaceholder::before{
 content: '请选择';
 position: absolute;
 left:2px;
 top:0px;
