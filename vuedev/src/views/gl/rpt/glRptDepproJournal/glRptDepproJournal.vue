...
import { getBtnPer, formatMoney, revertNumMoney, removeCache, setObjectCache, openNewPage } from "@/assets/js/util"; //从工具库函数中获取工具函数：{ formatMoney: '金额格式化' }
...
} from "../glRptJournal/glRptJournalColumns";
import {
  getReportDataPage,
  getRptAccas,
  postBatchExport,
...
export default {
  name: "glRptDepproJournal", // 财政项目明细账
  components: {
  ...
  data() {
    return {
      pageName: '财政项目明细账',
      pageType: 'glRptDepproJournal',
      rptType: 'GL_RPT_DEPPRO_JOURNAL',
      /* 1、公共信息模块 */
      menuId: "d426704b-3542-4f66-86b1-089e376e86da",
      /* 2、查询区域模块 */
    ...
    filterName(searchText){
      ...
      if (filterName) {
        ...
        rest = this.tableData.filter(item => {
          ...
          //判断当前列是否可用于全表搜索
          for (let key in item) {
            let flag2 = searchProps.some(it => { return it === key }),
            flag3 = XEUtils.toString(item[key]).toLowerCase().indexOf(filterName) > -1
            if ( flag2 && flag3 ) {
              item.highlight = key
  ...
  computed: {
    ...mapState({
      ...
      tableH: (state) => state.glRptDepproJournal.tableH, //表格高度
      agencyCode: (state) => state.glRptDepproJournal.agencyCode, // 单位代码
      agencyName: (state) => state.glRptDepproJournal.agencyName, // 单位名称
      acctCode: (state) => state.glRptDepproJournal.acctCode, // 账套代码
      acctName: (state) => state.glRptDepproJournal.acctName, // 账套名称
      prjGuid: (state) => state.glRptDepproJournal.prjGuid, //方案guid
      nowPrj: (state) => state.glRptDepproJournal.nowPrj, //方案名称
      rptStyle: (state) => state.glRptDepproJournal.prjContent.rptStyle, // 账表样式
      DEPPROFlag: (state) => state.glRptDepproJournal.DEPPROFlag, // 辅助项是否包含财政项目
    }),
    ...mapGetters({
  ...
  methods: {
    ...
    addColumns(val, nowal) {
      ...
      if (nowal === true) {
        ...
        columnsArr2.splice(3, 0, ...moreColumns);
        columnsArr3.splice(3, 0, ...moreColumns);
        columnsArr4.splice(3, 0, ...moreColumns);
      }
    ...
    getTableData() {
      ...
      getReportDataPage("GL_RPT_JOURNAL", {}, opt) // params拼接到url后面的参数
        .then((result) => {
          if (result.data.flag == "fail") {
            throw result.data.msg;
          } else {
    ...
    // 鼠标移入单元格事件
    cellMouseenter({ row, $rowIndex, $columnIndex, $event }) {
    ...
    cellClickEvent({ row, column }, event) {
      if (
        this.holdCtrl &&
        (column.property === "dStadAmt" ||
          column.property === "cStadAmt" ||
          column.property === "bStadAmt")
      ) {
