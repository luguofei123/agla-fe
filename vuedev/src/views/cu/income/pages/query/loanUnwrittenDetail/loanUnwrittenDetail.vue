<template>
    <div>
        <table-list ref="tableList" :uuid="'loanUnwrittenDetail'" tab-title="借票未核销明细表"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getTreeList() {
                let params = {
                    ...
                    eleCodes: 'DEPARTMENT,PROJECT,SETMODE,BILLTYPE' /*结算方式 项目 票据类型*/
                };
                ...
                this.$axios.get('/cu/incomeTaxEntryType/selectByParam', {params: params})
                    .then(res => {
                        if (res.data.flag == 'success') {
                            ...
                            this.searchConfig[3].treeOptions = this.treeOptions.taxEntryType;
                        } else {
                            this.$message.warning(res.data.msg);
                        }
                    })
                ...
                this.$axios.post('/cu/incomePrint/getPrintDataForBase', params)
                    .then(res => {
                        if (res.data.flag === 'success') {
                            ...
                            getPdf('CU_TICKET_UN_HX', '*', JSON.stringify(data));
                        } else {
                            this.$message.warning(res.data.msg);
                        }
                    })
        ...
        mounted() {
            ...
            this.tableConfig = {
                dataUrl: '/cu/incomeReport/selectTicketUnHxReport',
                dataParams: {
                    agencyCode: this.pfData.svAgencyCode
                }
            };
            this.columns = [
                {title: '部门', dataIndex: 'DEPARTMENT_CODE', width: '130', align: 'left'},
                {title: '人员', dataIndex: 'OPEN_TICKET_USER', width: '130', align: 'left'},
                ...
                {title: '对方单位', dataIndex: 'OTHER_UNIT', width: '130', align: 'left'},
                ...
                {title: '税务入账类型', dataIndex: 'TAX_ENTRY_TYPE_CODE', width: '130', align: 'left'}
            ];
            this.getTreeList();
