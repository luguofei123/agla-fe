 ...
 * @LastEditTime: 2020-09-27 18:05:01
...
import { getReportDataPage } from '@/views/gl/rpt/common/service/service'

const postfix = '_BY_GL_RPT_JOURNAL'
...
const state = {
  ...
  fromOtherRptFlag: false, //其他页面跳转过来
  frStartDate: '',
  frEndDate: '',
  selectedAccoNames: '',
...
const mutations = {
  ...
  [types.SET_FROM_OTHER_RPT_FLAG + postfix](state, payload) {
    state.fromOtherRptFlag = payload
  },
...
const actions = {
   ...
   */
  getCompleteData({ rootGetters, rootState }) {
    console.log(rootGetters)
    let opt = rootGetters.getReportArgument;
    opt.setYear = rootState.pfData.svSetYear,
    opt.userId = rootState.pfData.svUserId
    //加上分页信息
    opt.prjContent.currPage = 1;
    opt.prjContent.rowNumber = 999999999;
    // 此处“GL_RPT_JOURNAL”为rootGetters.rptType值，目前明细账和财政项目明细账页面使用，若后续其他页面请添加逻辑判断
    return getReportDataPage("GL_RPT_JOURNAL", {}, opt)
  }
}
