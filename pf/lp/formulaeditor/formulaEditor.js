    ...
    var page = function () {
        return {
            ...
            settingFun: function (val) {
                var setting = {
                    ...
                    callback: {//表示tree的一些事件处理函数
                        onDblClick: function (treeId, treeNode, leafData) {
                            ...
                            if(val == "BILL_TYPE" || val == "CUR_CODE") {
                                var codes = "'" + leafData.chrCode + "'";
                            } else {
                ...
                if(val == "BILL_TYPE" || val == "CUR_CODE") {
                    setting.data.key.name = "chrName";
                    setting.data.simpleData.idKey = "chrCode";
            ...
            dataclick: function () {
                ...
                $("body").on("click", "#ys .mytab-left .text-one", function () {
                    ...
                    if (codes == "acco") {
                        ...
                        if (onerdata.reqAcctCode.reqAcctType == 1) {
                            //如果是单位级账套
                            var url = "/lp/sys/getVouType/" + onerdata.agencyCode + "/" + ptData.svSetYear + "/" + onerdata.isCW + "/" + onerdata.reqAcctCode.reqAcctCode;
                        } else {
                            //如果是系统级科目体系
                            var url = "/lp/sys/getVouType/" + onerdata.agencyCode + "/" + ptData.svSetYear + "/" + onerdata.isCW + "/*";
                        }
                    ...
                    if (window.sessionStorage.getItem(onerdataId + codes + "FormulaEditorys") != null) {
                        ...
                        ufma.get("/lp/eleAccItem/getBillTypeList", argu, function (result) {
                            var data = result.data;
                            page.accoData = data;
                            if (data.length == 1 && data.codeName == "全部") {
                                return false;
                            ...
                            var ysrighthtmldata = '<div id="accoTree" class="ztree ufmaTree"></div>';
                            $("#ys").find(".mytab-right .right-con").html(ysrighthtmldata);
                            var setting = page.settingFun("BILL_TYPE");
                            var zTreeObj = $.fn.zTree.init($("#accoTree"), setting, data);
                            zTreeObj.expandAll(true);
                        })
                    } else if (codes == "CUR_CODE") { // 币种
                        var url = "/lp/sys/getCurrType/?setYear=" + ptData.svSetYear + "&agencyCode=" + onerdata.agencyCode + "&rgCode=" + ptData.svRgCode + "&acctCode=*";
                        ufma.get(url, {}, function (result) {
                            ...
                            var ysrighthtmldata = '<div id="accoTree" class="ztree ufmaTree"></div>';
                            $("#ys").find(".mytab-right .right-con").html(ysrighthtmldata);
                            var setting = page.settingFun("CUR_CODE");
                            var zTreeObj = $.fn.zTree.init($("#accoTree"), setting, data);
                            zTreeObj.expandAll(true);
                        })
                    } else {
            ...
            bottomdata: function () {
                ...
                ufma.get("/lp/eleAccItem/" + onerdata.agencyCode, argu, function (res) {
                    ...
                    var yslefthtmldata = '<div class="text-one hand activeleft" keys="acco">会计科目</div><div class="text-one hand activeleft" keys="vouType">凭证类型</div>';
                    for (var i = 0; i < res.data.length; i++) {
            ...
            transitEnglish: function (str) {
                if (str.indexOf("【") >= 0 && str.indexOf("】") < 0) {
                    ...
                    var url = "/lp/formulaEditor/verifyFormula";
                    var arg = {};
                    arg.formula = str;
                    ufma.post(encodeURI(url).replace(/\+/g, '%2B'), arg, function (res) {
            ...
            CorE: function (str) {
                str = str.toString().replace('ZDY:', '');
                // var rel = /'ZDY:\w+'/g;
                // var keysArr = page.matchAll(str, rel);
                // for (var i = 0; i < keysArr.length; i++) {
                //     str = str.replace(keysArr[i], "{" + keysArr[i][0].substring(1, keysArr[i][0].length - 1) + "}");
                // }
                console.log(str);
                if (str.indexOf("【") >= 0 && str.indexOf("】") < 0) {
                    return str;
