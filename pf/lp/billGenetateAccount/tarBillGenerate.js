		...
		return {
			...
			getBillData: function (result) {
				...
				if (result.data != null) {
					...
					if (result.data && result.data.content && result.data.content.list && result.data.content.list == undefined) {
						page.showTableData([], [{
							data: null
						}], []);
					} else {
				...
				vgDataTable = $('#' + id).DataTable({
					...
					"initComplete": function (settings, json) {
						...
						$("#dtToolbar .buttons-excel").off().on('click', function (evt) {
							...
							evt.preventDefault();
							ufma.expXLSForDatatable($('#' + id), '业务单据记账');
						});
						//导出end
						...
						$("body").on("click", 'input#th-check', function () { // 表头全选
							...
							$('#tarBillGenerate-dataleft .checkboxes').each(function () {
								isCorrect ? $(this).prop("checked", !0) : $(this).prop("checked", !1);
								isCorrect ? $(this).closest("tr").addClass("selected") : $(this).closest("tr").removeClass("selected");
							})
						...
						$('.datatable-group-checkable').on("change", function () {
							...
							$("#tarBillGenerate-data_wrapper").find("tbody tr").each(function(){
								...
								$("#tarBillGenerate-dataleft tbody").find("tr").eq(indexs).find(".checkboxes").attr("checked", isCorrect).prop("checked", isCorrect);
								if (isCorrect) {
									$(this).addClass("selected");
									$("#tarBillGenerate-dataleft tbody").find("tr").eq(indexs).addClass("selected")
								} else {
									$(this).removeClass("selected");
									$("#tarBillGenerate-dataleft tbody").find("tr").eq(indexs).removeClass("selected")
								}
							})
							page.calculateAmts();
						});
						...
						$("#tarBillGenerate-dataleft,#tarBillGenerate-dataright,#tarBillGenerate-datahead").hide()
						if(dataObj.content.list.length>0){
							page.leftfixedcolumns()
							page.rightfixedcolumns()	
							page.headfixedcolumns()
							$('#tarBillGenerate-data').tblcolResizable({},function(){
								page.getwidthorleft()
							});
							page.getwidthorleft()
			...
			headfixedcolumns:function(){
				$("#tarBillGenerate-datahead").html($("#tarBillGenerate-data thead").prop("outerHTML"))
				.css({"width":$("#tarBillGenerate-data").width(),'min-width':$("#tarBillGenerate-data").css('min-width')}).show()
				for(var i=0;i<$("#tarBillGenerate-datahead thead").find('th').length;i++){
					$("#tarBillGenerate-datahead thead").find('th').eq(i).css('width',$("#tarBillGenerate-data thead").find('th').eq(i).css('width'))
				}
				$("#tarBillGenerate-datahead th").on('click',function(){
					var trsd = $(this).index()
					$("#tarBillGenerate-data thead").find('th').eq(trsd).click()
				})
			},
			leftfixedcolumns:function(){
				var ths = ''
				ths+='<tr role="row">'
				ths+=$("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(0).prop('outerHTML')
				ths+='</tr>'
				$("#tarBillGenerate-dataleft").find('thead').html(ths)
				$("#datalefthead").find('thead').html(ths)
				var trs = ''
				var trbody = $("#tarBillGenerate-data").find('tbody').find('tr')
				...
				$("#tarBillGenerate-dataleft").find('tbody').html(trs).show()
				$("#tarBillGenerate-dataleft").show()
				$("#tarBillGenerate-dataright .checkboxes").on('change',function(){
					var isCorrect = $(this).is(':checked');
					$("#tarBillGenerate-data tbody").find("tr").eq(indexs).find(".checkboxes").attr("checked", isCorrect).prop("checked", isCorrect);
				})
			},
			rightfixedcolumns:function(){
				var lengs = $("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').length-1
				var ths = ''
				ths+='<tr role="row">'
				ths+=$("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(lengs).prop('outerHTML')
				ths+='</tr>'
				$("#tarBillGenerate-dataright").find('thead').html(ths)
				...
				$("#tarBillGenerate-dataright").find('tbody').html(trs)
				$("#tarBillGenerate-dataright").show()
				// $("#tarBillGenerate-dataright .btn").off('click').on('click',function(){
				// 	var sd=$(this).index()
				// 	var trsd = $(this).parents('tr').index()
				// 	$("#tarBillGenerate-data tbody").find('tr').eq(trsd).find('.nowrap').find(".btn").eq(sd).click()
				// })
			},
			getwidthorleft:function(){
				$("#tarBillGenerate .table-tab").css("height",page.getScrollY())
				$("#tarBillGenerate-dataleft,#datalefthead").find('thead').find('tr').eq(0).find('th').eq(0).css('width',$("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(0).css('width'))
				$("#tarBillGenerate-dataleft,#datalefthead").css('width',$("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(0).css('width'))
				var lengs = $("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').length-1
				$("#tarBillGenerate-dataright,#datarigthhead").find('thead').find('tr').eq(0).find('th').eq(0).css('width',$("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(lengs).css('width'))
				$("#tarBillGenerate-dataright,#datarigthhead").css('width',$("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(lengs).css('width'))
				$("#tarBillGenerate-datahead").css({"width":$("#tarBillGenerate-data").width()+"px",'min-width':$("#tarBillGenerate-data").width()+"px"}).show()
				var rightwz = $("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(lengs).offset().left
				var kuandu = $("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(lengs).width()
				var scroll = $(".table-tab").scrollLeft()
				var tabwidth = $(".table-tab").width()
				if(rightwz+kuandu>tabwidth){
					$("#tarBillGenerate-dataright,#datarigthhead").css({'left':tabwidth-20-kuandu+scroll+"px"})
				}else{
					$("#tarBillGenerate-dataright,#datarigthhead").css({'left':rightwz-30+scroll+'px'})
				} 
				$("#tarBillGenerate-dataleft,#datalefthead").css({'left':scroll+"px"})
				$('#tarBillGenerate-datahead,#datarigthhead,#datalefthead').css({'top':$('#tarBillGenerate .table-tab').scrollTop()+'px'})
				$('.JCLRgrips').css('width',$('.JCLRgrips').width() - $('#datarigthhead').width() + 'px');
				for(var i=0;i<$("#tarBillGenerate-datahead thead").find('th').length;i++){
					$("#tarBillGenerate-datahead thead").find('th').eq(i).css('width',$("#tarBillGenerate-data thead").find('th').eq(i).css('width'))
				}
			...
			definedVouType: function (tarArgu) {
				tarArgu.vouBudDate = "";
				tarArgu.vouBudTypeCode = "";
				tarArgu.vouDate = "";
				tarArgu.vouFinTypeCode = "";
				// liuyyn #3956需求
				if (lp.IS_DOUBLE_VOU) {
					if ($("#vouBudDate").find("input").val() !== '【自定义日期】') {
						tarArgu.vouBudDate = page.CorE($("#vouBudDate").find("input").val() ? $("#vouBudDate").find("input").val() : "");
					} else {
						tarArgu.vouBudDate = page.CorE($("#vouBudDate").find("input").attr('data-formula'));
					}
					tarArgu.vouBudTypeCode = $("#vouBudTypeCode").getObj().getValue() ? "'" + $("#vouBudTypeCode").getObj().getValue() + "'" : "";
					if ($("#vouFinDate").find("input").val() !== '【自定义日期】') {
						tarArgu.vouDate = page.CorE($("#vouFinDate").find("input").val() ? $("#vouFinDate").find("input").val() : "");
					} else {
						tarArgu.vouDate = page.CorE($("#vouFinDate").find("input").attr('data-formula'));
					}
					tarArgu.vouFinTypeCode = $("#vouFinTypeCode").getObj().getValue() ? "'" + $("#vouFinTypeCode").getObj().getValue() + "'" : "";
				} else if (lp.SINGLE) {
					if ($("#VouDate").find("input").val() !== '【自定义日期】') {
						tarArgu.vouDate = page.CorE($("#VouDate").find("input").val() ? $("#VouDate").find("input").val() : "");
					} else {
						tarArgu.vouDate = page.CorE($("#VouDate").find("input").attr('data-formula'));
					}
					tarArgu.vouFinTypeCode = $("#VouType").getObj().getValue() ? "'" + $("#VouType").getObj().getValue() + "'" : "";
				}
			},
			//表操作事件
			tableClick: function () {
				...
				$(document).on('click', 'tbody tr .vg-vougen', function (e) {
					...
					if (vouGen) {
						...
						for (var i = 0; i < tableRowDatas.length; i++) {
							if (tableRowDatas[i].MAIN_BILL_NO == mainBillNo) {
								billGuids[0].push(tableRowDatas[i].BILL_GUID);
							}
						}
					} else {
						billGuids[0].push(oneData.BILL_GUID);
					}
					...
					var tarArgu = {
						...
						seed: seed,
						rgCode: ptData.svRgCode, // 以下参数为后端接口优化参数结构修改
						setYear: ptData.svSetYear,
						currAgencyCode: pgArgu.currAgencyCode,
						currAcctCode: pgArgu.currAcctCode,
						agencyTypeCode: pgArgu.agencyTypeCode,
						isMainBillNo: vouGen ? 1 : 0, // 是否带主单据编号 1是0否
					};
					tarArgu.SummaryGenerate = "0";
					page.definedVouType(tarArgu);
					if ($("input[name='isPreview']").prop("checked") && vouGen) {
						isPreviewFlag = true;
						//勾选了生成前预览并且是主单据编号
						ufma.showloading('正在加载数据，请耐心等待...');
						tarArgu.isAllBills = 0; // 单行生成传0 预览接口传isAllBills
						var newUrl = interfaceURL.preview;
						ufma.post(newUrl, tarArgu, function (result) {
							ufma.hideloading();
							//凭证弹窗
							var oneData = undefined;
							page.openPreviewWindow(oneData, tarArgu, result.data, tarArgu.SummaryGenerate);
						});
					} else {
						ufma.showloading('正在加载数据，请耐心等待...');
						tarArgu.isAllBills = 0; // 单行生成传0
						var newUrl = interfaceURL.createTargerBill;
						ufma.post(newUrl, tarArgu, function (result) {
					...
					if(newData.length > 0){
						...
						for (var i = 0; i < newData.length; i++) {
							if (newData[i].bizBillCount > 0) {
								ts += '<li><a href="javascript:;" code="' + newData[i].schemeGuid + '">' + newData[i].schemeName + ' <span>(' + newData[i].bizBillCount + ')</span></a></li>'
							} else {
								ts += '<li><a href="javascript:;" code="' + newData[i].schemeGuid + '">' + newData[i].schemeName + '</a></li>'
							}
						}
						...
						if(lp.getUrlParam('schemeGuid')!=null){
								...
								var selfInterval = setInterval(function(){
										//如果表格内有了数据
										if($('#tarBillGenerate-data tbody tr').eq(0).find('td.dataTables_empty').length===0&&$('#tarBillGenerate-data tbody tr').length>0){
			...
			//打开凭证弹窗前,查看凭证带上guid，deleVouGuidWatch此方法未对data做处理，故去掉此方法
			...
			openPreviewWindow: function (oneData, pgArgu, previewData, SummaryGenerate) {
				var urlPath = "";
				var turnUrl = urlPath + '/pf/gl/vou/index.html?menuid=f24c3333-9799-439a-94c9-f0cdf120305d&action=preview&preview=1&dataFrom=vouBox&isJump=1'
				turnUrl = page.addRueicode(turnUrl);
				ufma.open({
					url: turnUrl,
					title: '凭证生成',
					...
					ondestory: function (data) {
						...
						if (data.action && data.action.action == "save") {
							...
							argu.schemeGuid = pgArgu.schemeGuid;
							argu.tmpGuid = pgArgu.tmpGuid;
							if (SummaryGenerate == "1") {
								//汇总生成
								argu.billGuid = pgArgu.guids;
							} else {
								//单条生成
				...
				for (var i = 0; i < mainSelectedRows.length; i++) {
					...
					for (var y = 0; y < tableData.length; y++) {
						if (tableData[y].MAIN_BILL_NO == mainSelectedRows[i]) {
							tarObj.guids[i].push(tableData[y].BILL_GUID);
						}
					}
					page.definedVouType(tarObj);
				}
				tarArgu = JSON.parse(JSON.stringify(tarObj));
							...
							if (result.flag == 'success') {
								...
								for (var i = 0; i < result.data.length; i++) {
									for (var j = 0; j < result.data[i].length; j++) {
										newTarArgu.guids.push(result.data[i][j]);
									}
								}
								newTarArgu.isAllBills = 1; 
								var newUrl = interfaceURL.preview;
								ufma.post(newUrl, newTarArgu, function (result) {
									ufma.hideloading();
									//凭证弹窗
									var oneData = undefined;
									page.openPreviewWindow(oneData, newTarArgu, result.data, newTarArgu.SummaryGenerate);
								});
							} else {
								ufma.showTip(result.msg, function () { }, "error");
								ufma.hideloading();
						...
						page.getBillGuids(function(result) {
							if (result.flag == 'success') {
								...
								for (var i = 0; i < result.data.length; i++) {
									...
									newTarArgu[i].guids = result.data[i];
									newTarArgu[i].isAllBills = isAll ? 1 : 0;
									totalNum += result.data[i].length;
								}
								if (totalNum > 1000) {
									ufma.confirm('当前所选共有' + totalNum +'条明细单据，将批量生成，是否继续', function (action) { // 带主单据编号的批量生成
										if (action) {
											var newUrl = interfaceURL.createTargerBill; // 20200928后端接口优化修改 生成接口统一调用createTargerBill
											ufma.post(newUrl, newTarArgu, function (result) {
												if (result.flag == "success") {
													mainSelectedRows = [];
							...
							ufma.confirm('当前所选共有' + vgDataTable.rows("tr.selected").data().length +'条明细单据，将批量生成，是否继续', function (action) { // 带主单据编号的批量生成
								if (action) {
									for (var i = 0; i < tarArgu.length; i++) {
										tarArgu[i].isAllBills = isAll ? 1 : 0;
									}
									var newUrl = interfaceURL.createTargerBill; // 20200928后端接口优化修改 生成接口统一调用createTargerBill
									ufma.post(newUrl, tarArgu, function (result) {
										if (result.flag == "success") {
			...
			billSummaryGenerate: function (oneData) {
				...
				if (page.hasMainBillNo) {
					...
					for (var y = 0; y < tableDataSelected.length; y++) {
						if (newmainSelectedRows.indexOf(tableDataSelected[y].MAIN_BILL_NO) > -1) {
							billGuids[0].push(tableDataSelected[y].BILL_GUID);
						}
					}
				} else {
					var tableData = vgDataTable.rows("tr.selected").data();
					for (var i = 0; i < tableData.length; i++) {
						billGuids[0].push(tableData[i].BILL_GUID);
					}
				}
				var pgArgu = page.combinePGArgu();
				...
				if ($("input[name='isPreview']").prop("checked")) {
					...
					if (isAll) { // 勾选了全选所有页 请求查询所有单据接口
						...
						if (tarArgu.guids.length > 1000) {
							ufma.confirm('当前所选单据共' + tarArgu.guids.length +'条，将汇总生成1张凭证，是否继续', function (action) { // 汇总生成
								if (action) {
									var newUrl = interfaceURL.createTargerBill;
									ufma.post(newUrl, tarArgu, function (result) {
										//汇总生成,生成成功后，弹出凭证录入弹窗；生成失败，提示失败，不弹窗
			...
			billGenerate: function () {
				...
				page.definedVouType(tarArgu);
				tarArgu.SummaryGenerate = "0";
				if ($("input[name='isPreview']").prop("checked") && tarArgu.guids.length == 1) {
					isPreviewFlag = true;
					//如果只勾选了一条单据并且勾选了生成前预览 不带主单据编号
					ufma.showloading('正在加载数据，请耐心等待...');
					var oneData = {
						BILL_GUID: tarArgu.guids[0]
					};
					tarArgu.isAllBills = 0; // 此时只有一条选中的单据，确定没有勾选全选所有页 预览接口传isAllBills
					var newUrl = interfaceURL.preview;
					ufma.post(newUrl, tarArgu, function (result) {
						ufma.hideloading();
						//凭证弹窗
					...
					if (isAll) { // 勾选了全选所有页 请求查询所有单据接口
						...
						if (tarArgu.guids.length > 1000) {
							ufma.confirm('当前所选单据共' + tarArgu.guids.length +'条将批量生成，是否继续；', function (action) { // 批量生成
								if (action) {
									//点击确定的回调函
									var newUrl = interfaceURL.createTargerBill;
			...
			cancelGenerateBill: function (selectedRows, type) {
				...
				if (type == "0") {
					...
					if (selectedRows.length == 0) {
						ufma.showTip("请勾选要取消生成的单据", function () {}, "warning");
						return false;
					}
			...
			deleteBillByGuid: function (selectedRows, type, t, tab) {
				...
				if (type == "0") {
					...
					if (selectedRows.length == 0) {
						ufma.showTip("请勾选要删除的单据", function () {}, "warning");
						return false;
					}
					for (var i = 0; i < selectedRows.length; i++) {
						targu.billGuids.push(selectedRows[i].BILL_GUID);
			...
			cancelGen: function (selectedRows, type) {
				...
				if (type == "0") {
					...
					if (selectedRows.length == 0) {
						ufma.showTip("请勾选不生成的单据", function () {}, "warning");
						return false;
					}
					for (var i = 0; i < selectedRows.length; i++) {
						targu.billGuids.push(selectedRows[i].BILL_GUID);
			...
			clearVgMoreQuery: function () {
				...
				// #8689 业务单据记账，设置金额查询条件，选择起始金额不会带出截止金额，需要再次保存查询条件才可以
			...
			setSencondRemberData: function () {
				if ($("#VouType").getObj() && !$.isNull(lp.rememberData) && !$.isNull(lp.rememberData.VouType)) {
			...
			setRememberData: function (data) {
				/** 这是库中预置的
				 * modeName: "false,false,false,false"
				 * isPreview: "false"
				 * date: "benQi"
				 * vgBillType: ""
				 * temName: ""
				...
				// 	if(dateArr[0] == "bennian") {
				// 		page.dateBenNian("dateStart", "dateEnd");
				// 		$("#vgTimeYear").removeClass("btn-default").addClass("btn-primary");
				// 		$("#vgTimeYear").siblings(".btn").removeClass("btn-primary").addClass("btn-default");
				// 	} else if(dateArr[0] == "today") {
				// 		page.dateToday("dateStart", "dateEnd");
				// 		$("#vgTimeDay").removeClass("btn-default").addClass("btn-primary");
				// 		$("#vgTimeDay").siblings(".btn").removeClass("btn-primary").addClass("btn-default");
				// 	} else if(dateArr[0] == "benQi") {
				// 		page.dateBenQi("dateStart", "dateEnd");
				// 		$("#vgTimeMonth").removeClass("btn-default").addClass("btn-primary");
				// 		$("#vgTimeMonth").siblings(".btn").removeClass("btn-primary").addClass("btn-default");
				// 	}
				// 	if(dateArr[1]) {
				// 		$("#dateStart").getObj().setValue(dateArr[1])
				// 	}
				// 	if(dateArr[2]) {
				// 		$("#dateEnd").getObj().setValue(dateArr[2])
				// 	}
				// 	if(dateArr[3] && dateArr[3] != "") {
				// 		$("#vgBillNo").val(dateArr[3])
				// 	}
				// }
			...
			onEventListener: function () {
				...
				$(document).on("click", "tbody tr", function (e) {
					...
					var indexs = $(this).index()
					$("#tarBillGenerate-dataleft tbody").find("tr").eq(indexs).find(".checkboxes").attr("checked", !inputCheck).prop("checked", !inputCheck);
				...
				$("#b-import").on("click", function () {
					...
					if (item.dataSrcType == "01" || item.dataSrcType == "06") {
						...
						ufma.open({
							...
							width: 500,
							height: 350,
							data: argu, // 业务单据记账-公务之家方案点取数时，日期没有带过去问题
							ondestory: function (action) {
								if (action.action && action.action.action == 'save') {
				...
				$("#tarBillGenerate .table-tab").scroll(function() {
					var lengs = $("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').length-1
					var rightwz = $("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(lengs).offset().left
					var kuandu = $("#tarBillGenerate-data").find('thead').find('tr').eq(0).find('th').eq(lengs).width()
					var scroll = $(".table-tab").scrollLeft()
