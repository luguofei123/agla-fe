$(function() {
  ...
  //接口URL集合
  var interfaceURL = {
    getPrsItemCoList: '/prs/prsitemco/getPrsItemCoList', // 查询工资项目列表
    deletePrsItemCo: '/prs/prsitemco/deletePrsItemCo', // 批量删除工资项目
    checkUsed: '/prs/prsitemco/checkUsed' // 删除前检查
  }
  var pageLength = 25
  var pritemTypeObj = {
    '01': '基本工资项',
  ...
  var page = (function() {
    return {
          ...
          initComplete: function(settings, json) {
            ...
            $('#dtToolbar .buttons-excel')
              ...
              .on('click', function(evt) {
                ...
                evt.preventDefault()
                ufma.expXLSForDatatable($('#' + id), '工资项目')
              })
            //导出end

            $('#salary-project-table_wrapper').ufScrollBar({
              hScrollbar: true,
              mousewheel: false
            })
      ...
      setTableData: function(queryData) {
        ...
        ufma.post(interfaceURL.getPrsItemCoList, queryData, function(result) {
          ufma.hideloading()
      ...
      openWin: function(ele) {
        var title, openData
        if (ele[0].id == 'btn-add-salary-project') {
          title = '新增工资项目'
          openData = {}
        } else {
          title = '编辑工资项目'
            ...
            ufma.confirm(
              ...
              function(action) {
                if (action) {
                  ...
                  var argu = page.delArgu(ele)
                  ufma.showloading('正在加载数据请耐心等待...')
                  ufma.post(interfaceURL.deletePrsItemCo, argu, function(
                    result
                  ) {
                    ufma.hideloading()
                    page.queryTableData()
      ...
      onEventListener: function() {
        ...
        // 新增
        $(document).on('click', '#btn-add-salary-project', function() {
          page.openWin($(this))
        })
        // 编辑
