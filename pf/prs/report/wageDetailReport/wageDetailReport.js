$(function () {
    ...
    //接口URL集合
    var interfaceURL = {
        findCalcDataBase: "/prs/rpt/PrsRptData/getSalaryDetailData",//薪资编制基础数据查询
        findCalcDatas: "/prs/rpt/PrsRptData/getSalaryByConditionData",//薪资编制数据条件查询
        getPrsOrgTreeIsUsed:"/prs/emp/prsOrg/getPrsOrgTreeIsUsed",//部门树
        printSalaryDetailData:"/prs/rpt/PrsRptData/printSalaryDetailData"//获取打印数据
    };
    var pageLength = 25;
    var newCellsData = [];
    var isGet = false;
    ...
    var notatm = ["empName","sex","orgCodeName","prtypeCodeName","levelGradeName","dutyGradeName","isFugle","smo"];
    var page = function () {
        return {
                    ...
                    "initComplete": function (settings, json) {
                      ...
                      //导出begin
                        $("#dtToolbar .buttons-print").off().on('click', function (evt) {
                            ...
                            ufma.post(interfaceURL.printSalaryDetailData, argu, function (result) {
                        ...
                        $("#dtToolbar .buttons-excel").off().on('click', function (evt) {
                            ...
                            evt.preventDefault();
                            ufma.expXLSForDatatable($('#' + id), '工资明细表');
                        });
                        // 导出end
            ...
            */
            ...
            getSearchData: function () {
            	...
            	var dataSource = $(".source .btn-primary").attr("value");
            	var totalType = $(".total-type .btn-primary").attr("value");
                var payNoMo = "";
            	var argu = {
                    orgCodes: $("#departmentCode").ufmaTextboxlist().getValue() ? $("#departmentCode").ufmaTextboxlist().getValue().split(",") : [],
                    prtypeCodes: page.selPrtypeCodes(),
                    ...
                    totalType : totalType == '1' ? "dept" : totalType
                };
                ufma.showloading("正在加载数据请耐心等待...");
...
//                page.initMonth();
            ...
            onEventListener: function () {
                ...
                //按钮切换
                $(document).on("click", ".total-type .select-item", function () {
                    $(this).removeClass("btn-default").addClass("btn-primary");
                    $(this).siblings(".btn").addClass("btn-default").removeClass("btn-primary");
                });
