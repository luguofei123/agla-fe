$(function() {
  ...
  //接口URL集合
  var interfaceURL = {
    getPrsOrgTree: '/prs/emp/prsOrg/getPrsOrgTree', // 获取部门树
    delete: '/prs/emp/prsOrg/delete', //  删除部门
    able: '/prs/emp/prsOrg/able', // 启用/停用
    getPropertyInList: '/prs/emp/maEmpProperty/getPropertyInList', // 获取在列表中显示的属性字段
    getMaEmpByOrgCodes: '/prs/emp/maEmp/getMaEmpByOrgCodes', // 查询部门下属的人员信息
    delMaEmp: '/prs/emp/maEmp/delMaEmp', // 批量删除人员信息
  ...
  var createHtml = (function() {
    return {
      ...
      createInput: function(id, name) {
        var html =
          '<div class="form-group margin-top-10 col-xs-10 col-sm-6 col-md-6">' +
          '<label class="control-label text-align-left data-label"><%=name%>:</label>' +
          '<div class="control-element" style="width:180px;">' +
          '<input id="<%=id%>" type="text" name="<%=id%>" class="form-control" maxlength="200" style="width: 180px" autocomplete="off" />' +
          '</div></div>'
      ...
      createDate: function(id, name) {
        ...
        var html =
          ...
          '<div id="<%=startId%>" name="<%=startId%>" class="uf-datepicker" style="width:120px;"></div></div>' +
          '<span class="split">-</span>' +
          '<div class="control-element" style="width:120px;">' +
          '<div id="<%=endId%>" name="<%=endId%>" class="uf-datepicker" style="width:120px;"></div></div></div>'
        return ufma.htmFormat(html, {
          id: id,
          name: name,
          startName: startName,
    ...
    return {
      ...
      initQuery: function(propertyInList, propertyListData, prsLevelList) {
        ...
        for (var i = 0; i < page.combox.length; i++) {
          ...
          var prsLevelListData = []
          for (var j = 0; j < prsLevelList.length; j++) {
            prsLevelListData.push({
              valId: prsLevelList[j].prlevelCode,
              val: prsLevelList[j].prlevelName
            })
          }
          if (id === 'posiCode') {
              ...
              if (result.flag == 'fail') {
                ...
                ufma.get(interfaceURL.selectMaEmpAndPrsCalcData, '', function(
                  ...
                  if (result.flag == 'fail') {
                    ...
                    ufma.post(interfaceURL.getPrsLevelList, {}, function(
                      result
                    ) {
                      var prsLevelList = result.data
                      page.initQuery(
                        propertyInList,
                        propertyListData,
                        prsLevelList
      ...
      initTable: function(data, columns) {
        ...
        page.DataTable = $('#' + id).DataTable({
          ...
          drawCallback: function(settings) {
            ...
            ufma.isShow(page.reslist)
            ufma.setBarPos($(window))
            $('#salary-project-table_wrapper').ufScrollBar('update')
          }
        })
      },
      // 转换表格数据
          ...
          for (var key in obj) {
            if (obj.hasOwnProperty(key)) {
              ...
              for (var j = 0; j < element.length; j++) {
                ...
                // posiLevel 单独处理
                if (code === element[j].dutyCode) {
                  item[key] = element[j].dutyName
                }
                // posiCode 单独处理
                if (code === element[j].prlevelCode) {
                  item[key] = element[j].prlevelName
                }
              }
            }
          }
