...
//账表所用接口
rpt.portList = {
	getVouBoxCount:rpt.urlPath+"/gl/vouBox/getVouBoxCount",
	...
	//getVoutype:rpt.urlPath+"/gl/eleVouType/getVoutypeByAgencyCode/",//请求凭证字号
...
rpt.selectTree=function(treeId,nodes){
	...
	function updateNodes(highlight) {
		for( var i=0, l=nodeList.length; i<l; i++) {
			...
			myTree.expandNode(nodeList[i], true, true, true);
		}
	}
		...
		prjContObj.rptCondItem = [
			{
				"condCode":"vouType",
				"condName":"凭证字号",
				"condText":$(rpt.namespace+" #rpt-pzzh-select").text(),
			...
			},
			{
				"condCode":"vouSource",
				"condName":"凭证来源",
				"condText":$(rpt.namespace+" #rpt-pzly-select").text(),
				"condValue":$(rpt.namespace+" #rpt-pzly-select option:checked").val()
			},
			{
				"condCode":"accoSumLevel",
				"condName":"汇总方式",
				"condText":$(rpt.namespace+" #rpt-hzfs-select").text(),
...
rpt.reqCurrType = function(){
	var reqUrl = rpt.portList.getCurrType+rpt.nowAgencyCode;
...
rpt.reqVouBoxCount = function(){
	...
	ufma.get(rpt.portList.getVouBoxCount,accArgu,function(result){
		var data = result.data;
		var accArr = [];
		var numAll = 0;
		var numO = 0;
		...
		accArr.push(accLast);
		rpt.newAccTable(accArr);
	})
};
//----------------------请求账务凭证数列表 end------------------------//

//请求科目辅助项下拉列表(明细账、多栏账)
rpt.reqSelectItems=function(){
...
//		});
		...
		$(rpt.namespace+" #selectTitle").html(seleHtml);
		$(rpt.namespace+" #selectTitle").find("option").eq(0).prop("checked",true);
		$(rpt.namespace+" .rpt-p-search-key").eq(0).find("input").attr("id",listItem[0].accItemCode+"-data-key");
		$(rpt.namespace+" .rpt-tree-data").eq(0).find("ul").attr("id",listItem[0].accItemCode+"-data")
...
//			tipsHtml += tHtml;			
//		}
//		$(rpt.namespace+" .rpt-query-li-tip-c").eq(0).attr({
...
//		if(rpt.rptType == "GL_RPT_JOURNAL"){
			//清空新增的辅助项
//			$(rpt.namespace+" .rpt-query-box-bottom .rpt-query-box-li0").remove();
		
//		}else if(rpt.rptType == "GL_RPT_COLUMNAR"){
			//清空新增的辅助项
			$(rpt.namespace+" .rpt-query-box-bottom .rpt-query-box-li0:gt(0)").remove();
//		}
...
rpt.reqOptList=function(){
	ufma.ajax(rpt.portList.optList,"get",{"rptType":rpt.rptType,"userId":rpt.nowUserId,"setYear":rpt.nowSetYear},rpt.showOptList);
};
//回调函数——请求查询条件其他选项列表
...
//		"accoCodeRange":accoCodeRange,
	...
	ufma.ajax(rpt.portList.accItemTree,"get",treeArgu,rpt.resCreateTree2);
};
//回调函数——创建返回树
...
//		window.location.href = '../../vou/index.html?menuid=BF09D5DB0CA54A268CD6EA8584121401&dataFrom='+page+'&action=query&vouGuid='+vouGuid+"&agencyCode="+rpt.nowAgencyCode+"&acctCode="+rpt.nowAcctCode;
		
		$(this).attr('data-href','../../../gl/vou/index.html?menuid=BF09D5DB0CA54A268CD6EA8584121401&dataFrom='+page+'&action=query&vouGuid='+vouGuid+"&agencyCode="+rpt.nowAgencyCode+"&acctCode="+rpt.nowAcctCode);
...
rpt.atree=function(zNodes){
	...
	$(document).ready(function(){
		...
		{
			myPageFrom = rpt.GetQueryString("pageFrom");
		}
		if(myPageFrom == "accQuery"){
			var arguStr = sessionStorage.getItem("accQueryArgu");
			var arguObj = JSON.parse(arguStr);
			rpt.nowAgencyCode = arguObj.agencyCode;
			rpt.nowAgencyName = arguObj.agencyName;
			rpt.nowAcctCode = arguObj.acctCode;
			rpt.nowAcctName = arguObj.acctName;
			var pnodeList = treeObj.getNodesByParam("agencyCode",rpt.nowAgencyCode);
			var nodeList = treeObj.getNodesByParam("acctCode",rpt.nowAcctCode,pnodeList[0]);
			treeObj.selectNode(nodeList[0]);
			
		}else{
			if(rpt.nowAgencyCode != "" && rpt.nowAgencyName != "" && rpt.nowAcctCode != "" && rpt.nowAcctName != ""){
				var pnodeList = treeObj.getNodesByParam("agencyCode",rpt.nowAgencyCode);
				var nodeList = treeObj.getNodesByParam("acctCode",rpt.nowAcctCode,pnodeList[0]);
				if (nodeList.length != 0) {
					treeObj.selectNode(nodeList[0]);
					...
					rpt.nowAcctCode = nodeList[0].acctCode;
					rpt.nowAcctName = nodeList[0].acctName;
				}
			}else{
		...
		if(rpt.rptType == "accQuery"){
			$(".rpt-acc-sub").find("span").eq(0).text(rpt.nowAgencyName);
			$(".rpt-acc-sub").find("span").eq(1).text(rpt.nowAcctName);
		}else{
			//表格单位信息
...
rpt.clickAtree = function(agencyCode,agencyName,acctCode,acctName){
	...
	//请求会计体系列表
	rpt.reqAccaList();
	console.info(rpt.rptType);
	
	if(rpt.rptType == "accQuery"){
		//修改bug75017--zsj--显示编码+名称
		//$(".rpt-acc-sub").find("span").eq(0).text(rpt.nowAgencyName);
		$(".rpt-acc-sub").find("span").eq(0).text(rpt.nowAgencyCode+' '+rpt.nowAgencyName);
		$(".rpt-acc-sub").find("span").eq(1).text(rpt.nowAcctName);
		if(rpt.accTableObj != null){
			rpt.accTableObj.clear().destroy();
		}
		rpt.reqVouBoxCount();
	}else if(rpt.rptType == "GL_RPT_BAL"){
		//表格单位信息
		$(".rpt-table-sub-tip1").find("span").eq(0).text(rpt.nowAgencyName);
		$(".rpt-table-sub-tip1").find("span").eq(1).text(rpt.nowAcctName);
		//还原查询条件
...
rpt.openTabMenu = function(dom){
	...
	// if(window.location.href[index+1] != "t"){
...
//账务查询表格初始化
rpt.newAccTable = function(tableData){
	var id = "accTable";//表格id
	$("#"+id).dataTable().fnDestroy();
	rpt.accTableObj = $("#"+id).DataTable({
    	"language":{
    	...
    	"processing":true,//显示正在加载中
      	...
      	"columns":[//O:未审核凭证;A:已审核凭证;F:已复审凭证;P:已记账凭证;C:已作废凭证
            ...
            // {title:"复审数",data:"vouStatusF"},暂时不显示revise
