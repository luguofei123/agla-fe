...
//账表所用接口
// rpt.urlPath = "http://127.0.0.1:8083";
 rpt.urlPath = "";
rpt.portList = {
...
rpt.selectTree=function(treeId,nodes,checkFlag,radioType){
    ...
    function getFontCss(treeId, treeNode) {
        // color:"#F04134"
        return (!!treeNode.highlight) ? {color:"#333", "font-weight":"bold"} : {color:"#333", "font-weight":"normal"};
    }
    function filter(node) {
...
rpt.changeItems=function(){
    ...
    if($(".glRptVousummary").length > 0){
        // accItemArgu.accsCode = rpt.accsCode;
...
rpt.reqAccList=function(){
    ...
    if($(".glRptVousummary").length > 0){
        // accItemArgu.accsCode = rpt.accsCode;
        var str1 = '&accsCode='+rpt.accsCode;
        url+=str1;
    }
    //汇总余额表 E
    if(rpt.accsSel){
        // accItemArgu.acctCode = "*";
        // accItemArgu.agencyCode = "*";
...
//		];
        var oneArr = [{"accItemCode":"","accItemName":"请选择"}];
...
rpt.reqAccoTree=function(radioType){
    ...
    if(rpt.accsSel || $(".glRptVousummary").length > 0){
    ...
    if(isHaveAcco) {
        ...
        var getPdata = function(result){
            var s = {labels:{
                    ...
                    dyr: rpt.nowUserName,
                    sh: "",
                    dyrq: rpt.nowDate},rows:result.data.tableData};
            pDataArr.push(s);
        }
        console.log(tabArgu);
        var t = 0;
