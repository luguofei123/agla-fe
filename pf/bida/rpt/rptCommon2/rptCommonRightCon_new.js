...
rpt.summaryJournalMenuId = "b0dd89c2-fcb2-49eb-9ed9-71a7aa8aacdc"; //汇总明细账
...
rpt.urlPath = "";//服务器（提交svn前一定要将注释放开）
rpt.portList = {
    //agencyList:"/gl/eleAgency/getRptAgencys",//单位列表接口
    agencyList: rpt.urlPath + "/gl/eleAgency/getAgencyTree",//单位列表接口
    acctList: rpt.urlPath + "/gl/eleCoacc/getRptAccts",//账套列表接口
    accaList: rpt.urlPath + "/gl/eleAcca/getRptAccas",//会计体系列表接口
    ...
    accoTree: rpt.urlPath + "/gl/sys/coaAcc/getRptAccoTree",//会计科目树（账套级）接口
    accItemTree: rpt.urlPath + "/gl/common/glAccItemData/getAccItemTree",//辅助项资料树（单位级）接口
    ...
    rptAccItemTypeList: rpt.urlPath + "/gl/EleAccItem/getRptAccItemTypePost",//辅助项类别列表接口包括科目
    //getVoutype:"/gl/eleVouType/getVoutypeByAgencyCode/",//请求凭证字号
    getVoutype: rpt.urlPath + "/gl/eleVouType/getVouType/",//请求凭证字号
    getCurrType: rpt.urlPath + "/gl/eleCurrRate/getCurrType",//请求币种列表
    getEleLevelNum: rpt.urlPath + "/gl/rpt/cond/getEleLevelNum",//获取汇总方式（凭证汇总表）
    getVouTypeFull: rpt.urlPath + "/gl/eleVouType/getVouType/",//请求凭证类型、字号
    getVOU_SOURCE: rpt.urlPath + "/gl/enumerate/VOU_SOURCE"//获取凭证来源
};
...
rpt.selectTree = function (treeId, nodes, checkFlag, radioType) {
    ...
    function getFontCss(treeId, treeNode) {
        // color:"#F04134"
        return (!!treeNode.highlight) ? {color: "#333", "font-weight": "bold"} : {
            color: "#333",
            "font-weight": "normal"
        };
    }
...
//		var radioType = $("#"+treeId).parents(".rpt-query-li-cont").find(".rpt-query-li-action input[type='hidden']").val();
        ...
        $(".rpt-query-li-selete .rpt-tags-list").on("click", "b.icon-close", function (event) {
            ...
            if (rpt.rptType == "GL_RPT_LEDGER") {
                ...
                var id = $(this).parents(".rpt-tree-view").next(".rpt-tree-data").find("ul").attr("id");
                if (myTree.setting.treeId === id) {
                    rpt.deleteSelete(this, myTree);
...
rpt.openSaveMethodModal = function () {
    $(rpt.namespace).find('#saveMethod').on('click', function () {
    	//修改bug76953--zsj--修改未选择账套时点击“保存”方案按钮报错问题
		var tabArgu = rpt2.backTabArgu();
		if($.isNull(tabArgu.acctCode)) {
			ufma.showTip("请选择至少一个账套", function() {}, "warning");
			return false;
		}
        ...
        if ($(meLi).length > 0) {
            ...
            if(scope == "3" && $(".glRptVousummary").length > 0 || $(".bida-rpt-summaryJournal").length > 0 || $(".bida-rpt-summaryLedger").length > 0){
                nn = parseInt(scope - 2);
            }
...
rpt.qryItemsArr = function () {
    ...
    if ($(rpt.namespace).selector !== "#glRptWS") { // 非工作量统计表
    ...
    var colSetVal = JSON.parse(localStorage.getItem("colSetVal"));
	if (rpt.rptType == "GL_RPT_BAL" && colSetVal) {
		for (let i = 0; i < colSetVal.length; i++) {
			for (let j = 0; j < qryItemsArr.length; j++) {
...
rpt.prjContObj = function () {
    ...
    //prjScope方案作用域为3代表
    if(rpt.prjScope == "3" && ($(".glRptVousummary").length > 0 || $(".bida-rpt-summaryJournal").length > 0 || $(".bida-rpt-summaryLedger").length > 0||$(".bida-rpt-workload").length > 0)){
        acctInfoObj.acctCode = pfData.svAcctCode;//账套代码
        acctInfoObj.agencyCode = pfData.svAgencyCode;//单位代码
    }
    prjContObj.agencyAcctInfo.push(acctInfoObj);
    var startDate = $("#dateStart").getObj().getValue();
    var startEnd = $("#dateEnd").getObj().getValue();

    if (rpt.rptType == "GL_RPT_BAL" || rpt.rptType == "GL_RPT_LEDGER"||rpt.rptType == "BIDA_RPT_WORKLOAD") {
...
//		console.info("不是余额表、明细账、总账、日记账、凭证汇总表、多栏账,查看是否需要做细化处理！");
    ...
    if (rpt.rptType != "GL_RPT_VOUSUMMARY") {
        ...
        if ($(rpt.namespace).selector !== "#glRptWS") {
        ...
        if($(".glRptVousummary").length > 0 || $(".bida-rpt-summaryJournal").length > 0){
            //汇总余额表
...
rpt.changeItems = function () {
    ...
    accoCodeRange = accoCodeRange || '0'; //选中会计科目范围
    ...
    //汇总余额表 、汇总总账、汇总明细账，辅助项根据科目体系过滤 S
    if ($(".glRptVousummary").length > 0 || $(".bida-rpt-summaryLedger").length > 0 || $(".bida-rpt-summaryJournal").length > 0) {
        var str = '&accsCode='+rpt.accsCode;
        url+=str;
    }
    //汇总余额表  或者汇总总账，辅助项根据科目体系过滤 E
...
rpt.reqCurrType = function () {
    ...
    ufma.ajax(reqUrl, "get", {"agencyCode": agencyCode}, function (result) {
...
//	};
    ufma.ajax(rpt.portList.agencyList, "get", "", function (result) {
        ...
        if (rpt.nowAgencyCode != "" && rpt.nowAgencyName != "") {
            ...
            if (agency != undefined) {
                rpt.cbAgency.setValue(rpt.nowAgencyCode, rpt.nowAgencyCode + " " + rpt.nowAgencyName);
            } else {
...
rpt.reqAcctList = function () {
    ...
    ufma.ajax(rpt.portList.acctList, "get", acctArgu, function (result) {
        ...
        if (data.length > 0) {
            ...
            ufma.showTip("该单位下面没有账套，请重新选择单位！", function () {
            }, "warning");
        }
    });
};
...
rpt.reqAccList = function () {
	...
	accoCodeRange = accoCodeRange.split(',');
    // if($("#domId").length > 0){
    //     accoCodeRange = $("#domId").getObj().getValue();
    // }
    var url = rpt.portList.rptAccItemTypeList + '?setYear='+rpt.nowSetYear+'&userId='+rpt.nowUserId;
    //汇总余额表 或者汇总总账，辅助项根据科目体系过滤 S
    if ($(".glRptVousummary").length > 0 || $(".bida-rpt-summaryLedger").length > 0 || $(".bida-rpt-summaryJournal").length > 0) {
        var str1 = '&acctCode=*&agencyCode=*&accsCode=' + rpt.accsCode;
        url += str1;
    } else {
        var str2 = '?acctCode='+rpt.nowAcctCode+'&agencyCode='+rpt.nowAgencyCode+'&accsCode='+rpt.nowAccsCode;
        url += str2;
    }
// //汇总余额表  或者汇总总账，辅助项根据科目体系过滤 E
//     if (rpt.accsSel || $(".glRptVousummary").length > 0 || $(".bida-rpt-summaryLedger").length > 0 || $(".bida-rpt-summaryJournal").length > 0) {
//         var str2 = '&acctCode=*&agencyCode=*'
//         url+=str2;
//     }
...
rpt.reqSavePrj = function (isNew) {
    var savePrjArgu = {};
    if($(".glRptVousummary").length > 0 || $(".bida-rpt-summaryJournal").length > 0 || $(".bida-rpt-summaryLedger").length > 0 || $(".bida-rpt-workload").length > 0){
        //汇总余额表、汇总明细账、汇总总账、工作量统计表，保存多单位多账套
        ...
        //     }
        // }
        // savePrjArgu.acctCode = acctArr.join();//账套代码
        // savePrjArgu.agencyCode = agencyArr.join();//单位代码
        // 后端刘飞确认修改20200421：汇总余额表、汇总明细账、汇总总账、工作量统计表不管左侧怎么选，这一层的单位账套都传*，所以注释掉以上代码
    ...
    rpt.prjScope = savePrjArgu.prjScope;//保存，方便使用
    savePrjArgu.rptType = $(".rptType").val();//账表类型
    savePrjArgu.setYear = rpt.nowSetYear;//业务年度
    savePrjArgu.userId = rpt.nowUserId;//用户Id

    if ($(".bida-rpt-bal").length > 0) {
        //余额表
        savePrjArgu.rptType = "BIDA_RPT_BAL";
    } else if ($(".bida-rpt-journal").length > 0) {
        //明细账
        savePrjArgu.rptType = "BIDA_RPT_JOURNAL";
    } else if ($(".bida-rpt-ledger").length > 0) {
        //总账
        savePrjArgu.rptType = "BIDA_RPT_LEDGER";
    }else if ($(".bida-rpt-summaryLedger").length > 0) {
        //汇总总账
        savePrjArgu.rptType = "GL_RPT_LEDGERSUMMARY";
        // savePrjArgu.acctCode = "*";
        // savePrjArgu.agencyCode = "*";
    }else if ($(".glRptVousummary").length > 0) {
        //汇总余额表
        savePrjArgu.rptType = "GL_RPT_BALSUMMARY";
        // savePrjArgu.acctCode = "*";
        // savePrjArgu.agencyCode = "*";
    }else if ($(".bida-rpt-summaryJournal").length > 0) {
        //汇总明细账
        savePrjArgu.rptType = "GL_RPT_JOURNALSUMMARY";
        // savePrjArgu.acctCode = "*";
        // savePrjArgu.agencyCode = "*";
    }else if ($(".bida-rpt-workload").length > 0) {
        //工作量统计表
        savePrjArgu.rptType = "BIDA_RPT_WORKLOAD";
        // savePrjArgu.acctCode = "*";
        // savePrjArgu.agencyCode = "*";
    }
...
rpt.addQryCount = function (prjGuid) {
	ufma.ajaxDef(rpt.portList.addQryCount + "?prjId=" + (prjGuid ? prjGuid : ''), "get", "", function(result) {});
};
//组织集中查询方案查询参数
rpt.prjArgu = function(prjArgu){
    if ($(".glRptVousummary").length > 0) {
        //汇总余额表
        prjArgu.rptType = "GL_RPT_BALSUMMARY";
        prjArgu.agencyCode = "*";
        prjArgu.acctCode = "*";
    } else if ($(".bida-rpt-summaryLedger").length > 0) {
        //汇总总账
        prjArgu.rptType = "GL_RPT_LEDGERSUMMARY";
        prjArgu.agencyCode = "*";
        prjArgu.acctCode = "*";
    } else if ($(".bida-rpt-summaryJournal").length > 0) {
        //汇总明细账
        prjArgu.rptType = "GL_RPT_JOURNALSUMMARY";
        prjArgu.agencyCode = "*";
        prjArgu.acctCode = "*";
    } else if ($(".bida-rpt-workload").length > 0) {
        //工作量统计表
        prjArgu.rptType = "BIDA_RPT_WORKLOAD";
        prjArgu.agencyCode = "*";
        prjArgu.acctCode = "*";
    } else if ($(".bida-rpt-bal").length > 0) {
        //余额表
        prjArgu.rptType = "BIDA_RPT_BAL";
    } else if ($(".bida-rpt-journal").length > 0) {
        //明细账
        prjArgu.rptType = "BIDA_RPT_JOURNAL";
    } else if ($(".bida-rpt-ledger").length > 0) {
        //总账
        prjArgu.rptType = "BIDA_RPT_LEDGER";
    }
};
//渲染查询方案
rpt.renderPriList = function(lsit){
    //清空查询方案
    ...
    if(data){
        for (var i = 0; i < data.length; i++) {
            ...
            if (rpt.curPlan) {
                $(liHtml)[data[i].prjCode == rpt.curPlan.prjCode ? 'addClass' : 'removeClass']('selected').appendTo("#rptPlanList ul");
            } else {
                $(liHtml).appendTo("#rptPlanList ul");
            }
...
rpt.showPrjList = function (result) {
    rpt.reqSharePrjList(result.data);
};

//方案初始化begin
rpt.showPlan = function () {
    $("#rptPlanList").html('<ul class="uf-tip-menu"></ul>');
    rpt.reqPrjList();
    $("#rptPlanList").off().on('click', 'li', function (e) {
        if ($(e.target).is('.btn-close')) {
            ...
            rpt.delPlan(planCode, planName, function (action) {
                if (action) {
                    _li.remove();
            ...
            rpt.selectPlan(planCode);
        }
    });
},
//----------------------查询方案列表 end-------------------------//
    //初始化查询方案
    rpt.initPageNew = function () {
...
//选择查询方案
    rpt.selectPlan = function (planCode) {
        rpt.reqPrjCont(planCode);
    },
//删除查询方案
    rpt.delPlan = function (planCode, planName, _callback) {
        ufma.confirm('您确定要删除查询方案' + planName + '吗?', function (action) {
            if (action) {
                ...
                ufma.ajax(rpt.portList.deletePrj, "delete", argu, function (result) {
                    var flag = result.flag;
                    if (flag == "success") {
                        _callback(true);
...
//----------------------共享方案列表 start------------------------//
//请求函数——共享方案列表
    rpt.reqSharePrjList = function (list) {
        var prjArgu = {
            "agencyCode": rpt.nowAgencyCode,
            "acctCode": rpt.nowAcctCode,
            "rptType": rpt.rptType,
        ...
        rpt.prjArgu(prjArgu);
        ufma.ajax(rpt.portList.sharePrjList, "get", prjArgu, function (result) {
            var data = result.data;
            if(list && data){
                var newList = list.concat(data);
                rpt.renderPriList(newList);
            }
        });
    };
//----------------------共享方案列表 end-------------------------//
...
rpt.showOptList = function (result) {
    ...
    if (rpt.rptType == "GL_RPT_VOUSUMMARY") {
        ...
        for (var i = 0; i < optArr.length; i++) {
            ...
            if ($(".glRptVousummary").length > 0 || rpt.rptType == "GL_RPT_BAL") {
                //如果是汇总余额表，不显示逐级汇总
                if (optArr[i].optCode !== "IS_GRADSUM") {
...
rpt.reqAccoTree = function (radioType) {
    ...
    if (rpt.accsSel || $(".glRptVousummary").length > 0 || $(".bida-rpt-summaryLedger").length > 0 || $(".bida-rpt-summaryJournal").length > 0) {
        treeArgu.accsCode = rpt.accsCode;
        treeArgu.acctCode = "*";
        treeArgu.agencyCode = "*";
    }
...
rpt.reqPrjCont = function (prjCode) {
    ...
    if ($(".glRptVousummary").length > 0) {
        prjContArgu.rptType = "GL_RPT_BALSUMMARY";
    }
    if ($(".bida-rpt-summaryLedger").length > 0) {
        prjContArgu.rptType = "GL_RPT_LEDGERSUMMARY";
    }
    if ($(".bida-rpt-summaryJournal").length > 0) {
        prjContArgu.rptType = "GL_RPT_JOURNALSUMMARY";
    }
    if ($(".bida-rpt-workload").length > 0) {
        prjContArgu.rptType = "BIDA_RPT_WORKLOAD";
    }
    if ($(".bida-rpt-bal").length > 0) {
        prjContArgu.rptType = "BIDA_RPT_BAL";
    }
    if ($(".bida-rpt-journal").length > 0) {
        prjContArgu.rptType = "BIDA_RPT_JOURNAL";
    }
    if ($(".bida-rpt-ledger").length > 0) {
        prjContArgu.rptType = "BIDA_RPT_LEDGER";
    }
...
rpt.ACCOtext = "";
//回调函数——展示查询方案内容
rpt.showPrjCont = function (result, flag) {
    ...
    if($(".glRptVousummary").length > 0 || $(".bida-rpt-summaryJournal").length > 0 || $(".bida-rpt-summaryLedger").length > 0 || $(".bida-rpt-workload").length > 0){
        //根据返回来的单位账套自动勾选左侧单位账套树
        var prjContent = JSON.parse(result.data.prjContent);
        var agencyCodeList = prjContent.agencyAcctInfo[0].agencyCode;
        var acctCodeList = prjContent.agencyAcctInfo[0].acctCode;
        var  agencyCodeArr = agencyCodeList ? agencyCodeList.split(",") : [],
        acctCodeArr = acctCodeList ? acctCodeList.split(",") : [];
        if (flag) { // 汇总余额表跳转汇总明细账时用，setTimeout解决左侧科目树无数据值带不过来问题
            setTimeout(function () {
                var treeObj = $.fn.zTree.getZTreeObj("atree");
                treeObj.checkAllNodes(false);
                for(var i = 0;i<acctCodeArr.length;i++){
                    if(agencyCodeArr[i]){
                        var agencyCode = agencyCodeArr[i],
                        acctCode = acctCodeArr[i];
                        if(agencyCode===acctCode){
                            treeObj.checkNode(treeObj.getNodeByParam("acctCode",acctCodeArr[i]),true);
                        }else{
                            var parentNode = treeObj.getNodeByParam("agencyCode",agencyCodeArr[i]), // 与2650行对应修改key解决单位账套不对应问题
                            node = treeObj.getNodeByParam("acctCode",acctCodeArr[i],parentNode);
                ...
                for(var i = 0;i<acctCodeArr.length;i++){
                    if(agencyCodeArr[i]){
                        ...
                        if(agencyCode===acctCode){
                            ...
                            var parentNode = treeObj.getNodeByParam("acctCode",agencyCodeArr[i]),
                            node = treeObj.getNodeByParam("acctCode",acctCodeArr[i],parentNode);
                            console.log(parentNode);
                            console.log(node);
                            treeObj.checkNode(node,true);
        ...
        //根据返回来的单位账套自动点击左侧单位账套树
        var prjContent = JSON.parse(JSON.stringify(result.data.prjContent));
        var acctCodeList = prjContent.agencyAcctInfo.acctCode;
        var acctCodeArr = acctCodeList ? acctCodeList.split(",") : [];
        var treeObj = $.fn.zTree.getZTreeObj("atree");
        if(treeObj){
            var node = treeObj.getNodeByParam('acctCode', acctCodeArr);//获取id为1的点
    ...
    //总账
    else if (rpt.rptType == "GL_RPT_LEDGER" || rpt.rptType == "GL_RPT_BAL"|| rpt.rptType == "BIDA_RPT_WORKLOAD") {
        ...
        // console.log("不是余额表、明细账、总账、日记账、凭证汇总表、多栏账,查看是否需要做细化处理！");
    }
    //zsj--修改点击方案时期间和日期与方案中的内容不一致问题
	if(cont.period == "dateBn") {
		$('#dateBn').addClass('selected').siblings().removeClass('selected');
	} else if(cont.period == "dateBq") {
		$('#dateBq').addClass('selected').siblings().removeClass('selected');
	} else if(cont.period == "dateJr") {
...
rpt.accQueryShow = function (cont) {
    //bug76947--zsj---解决点击某保存方案时辅助核算项没有显示问题
...
rpt.clearTagsTree = function () {
    //销毁所有树
    $(rpt.namespace + " .ztree:not(#atree)").each(function () {
        var zId = $(this).attr("id");
        $.fn.zTree.destroy(zId);
    });
...
rpt.backTabArgu = function () {
    ...
    //tabArgu.acctCode = rpt.nowAcctCode;//账套代码
    if(rpt.nowAgencyCode == '' || rpt.nowAgencyCode == undefined|| rpt.nowAgencyCode == null){
        tabArgu.agencyCode = $(".rpt-table-sub-tip1").find("span").eq(0).attr('code')//单位代码
    }else{
...
rpt.showHideTree = function (dom, code, text, codeType, eleCode) {
    //汇总余额表 汇总总账 汇总明细账S
    // if (($(".glRptVousummary").length > 0 || $(".bida-rpt-summaryLedger").length > 0 || $(".bida-rpt-summaryJournal").length > 0) && code == "ACCO") {
    //     var treeObj = $.fn.zTree.getZTreeObj("atree");
    //     var nodes = treeObj.getCheckedNodes(true);
    //     if(nodes.length == 0){
    //         ufma.showTip("请选择账套",function () {
    ...
    var checkbox = true;
    //明细账、总账、汇总明细账、汇总总账都是单选，checkbox为false
    if (rpt.rptType == "GL_RPT_LEDGER" || $(".bida-rpt-summaryJournal").length > 0 || $(".bida-rpt-journal").length > 0){
        checkbox = false;
    }
    var openData = {
        'flag': code,
        ...
        'leafRequire': false,
        'data': params,
        'isParallelsum':rpt.isParallel
    };
    //汇总余额表 汇总总账 汇总明细账科目和辅助项都是获取系统级的，agencyCode和acctCode都是*
    //汇总余额表 汇总总账 汇总明细账科目接口是用的另一个，用ACCS作为区分
    if ($(".glRptVousummary").length > 0 || $(".bida-rpt-summaryLedger").length > 0 || $(".bida-rpt-summaryJournal").length > 0) {
        params.agencyCode = "*";
        params.acctCode = "*";
        if (code == "ACCO"){
            ...
            openData.setYear = rpt.nowSetYear;
            openData.flag = "ACCS";
    ...
    ufma.open({
        ...
        ondestory: function (result) {
            ...
            if (result.action) {
                ...
                $(input).attr('code', codeList.join(',')).attr('itemLevel',result.itemLevel);
				// 优化内容：组织查询接口的参数
				// 账务处理-明细账/余额表/总账/日记账，集中查询-汇总余额表
				if (result.isRelation && $(".glRptVousummary").length > 0) { // 级联
...
//打开联查凭证页面
rpt.openVouShow = function (dom, page, isCrossDomain) {
...
//		window.location.href = '../../vou/index.html?dataFrom='+page+'&action=query&vouGuid='+vouGuid+"&agencyCode="+rpt.nowAgencyCode+"&acctCode="+rpt.nowAcctCode;
        ...
        var baseUrl = '/gl/vou/index.html?menuid=' + rpt.vouMenuId + '&dataFrom=' + page + '&action=query&vouGuid=' + vouGuid + '&agencyCode=' + rpt.nowAgencyCode + '&acctCode=' + rpt.nowAcctCode;
        // baseUrl = isCrossDomain ? '/pf' + baseUrl : '../../..' + baseUrl;
        baseUrl = '/pf' + baseUrl;
        uf.openNewPage(isCrossDomain, $(this), 'openMenu', baseUrl, false, "凭证录入");		
...
rpt.rptPrint = function(pageName, tableId, tableType) {
    ...
    if(isHaveAcco) {
        ...
        for(var i = 0; i < items1.length; i++) {
            ...
            ufma.ajaxDef(rpt.urlPath + "/gl/rpt/getReportData/" + rpt.rptType, "post", tabArgu, function(result) {
                getPdata(result);
            });
        }
        ufma.ajaxDef(rpt.urlPath + "/gl/GlRpt/getPrtFormatList", "post", getData, getPrtFormatList);
        var printCache = {};
...
rpt.accHtml = function (sender, data) {
    ...
    $('#' + id).on('click', 'span.icon-close', function() {//辅助核算项点击“×”时对应 的辅助核算项消失--修改bug76460、76458--zsj
    ...
    $('.rpt-query-li-cont').on('click', 'span.icon-close', function() { //辅助核算项点击“×”时对应 的辅助核算项消失
...
rpt.initQueryAcco = function (flag) {
    var params = {
        "acctCode": rpt.nowAcctCode,
        "agencyCode": rpt.nowAgencyCode,
        "setYear": rpt.nowSetYear,
    ...
    var myPageFrom = rpt.GetQueryString("pageFrom");
    if (!$.isNull(myPageFrom) && myPageFrom == "accQuery") {
        var arguStr = sessionStorage.getItem("accQueryArgu");
        if (arguStr) {
            var arguObj = JSON.parse(arguStr);
            params.acctCode = arguObj.acctCode;
            params.agencyCode = arguObj.agencyCode
        }
    }
    ...
    ufma.get('/gl/common/glAccItemData/getAccItemTree', params, function (result) {
        ...
        if (result.data.length > 0) {
            if (rpt.rptType != "GL_RPT_CHRBOOK" && rpt.rptType != "GL_RPT_BAL" && !flag) {
