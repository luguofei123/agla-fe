...
rpt.portList = {
	...
	rptAccItemTypeList: "/gl/EleAccItem/getRptAccItemType", //辅助项类别列表接口包括科目
...
rpt.prjContObj = function() {
	...
	} else if(rpt.rptType == "GL_RPT_JOURNAL" || rpt.rptType == "GL_RPT_CHRBOOK" || rpt.rptType == "GL_RPT_DAILYJOURNAL" || rpt.rptType == "GL_RPT_VOUSUMMARY" || rpt.rptType == "GL_RPT_COLUMNAR") {
...
rpt.reqAcctList = function() {
	...
	ufma.ajax(rpt.portList.acctList, "get", acctArgu, function(result) {
		...
		if(data.length > 0) {
			...
			rpt.isParallelsum = data[0].IS_PARALLEL;
			rpt.isDoubleVousum = data[0].IS_DOUBLE_VOU;
...
rpt.initAcctList = function() {
	rpt.cbAcct = $("#cbAcct").ufmaCombox2({
		...
		onchange: function(data) {
			...
			rpt.isParallelsum = data.IS_PARALLEL;
			rpt.isDoubleVousum = data.IS_DOUBLE_VOU;
			if(rpt.isParallelsum != "1") {
			...
			if(rpt.rptType == "GL_RPT_JOURNAL" || rpt.rptType == "GL_RPT_DAILYJOURNAL"|| rpt.rptType == "GL_RPT_CHRBOOK") {
				rpt.initQueryAcco();
			} else {
...
rpt.reqAccList = function() {
	...
	ufma.ajax(rpt.portList.rptAccItemTypeList, "get", accItemArgu, function(result) {
		...
		if(rpt.rptType != "GL_RPT_VOUSUMMARY" && rpt.rptType != 'GL_RPT_DAILYJOURNAL' && rpt.rptType != 'GL_RPT_CHRBOOK') {
	...
	rpt.resBackQuery = function() {
		...
		$(".rpt-query-box-center .rpt-tree-view").each(function(i) {
			...
			$(this).on("click", '.search-btn', function(e) {
				...
				rpt.showHideTree(this, code, text);
			});
		});
...
rpt.backToOrigin = function() {
	...
	} else if(rpt.rptType == "GL_RPT_LEDGER" || rpt.rptType == "GL_RPT_JOURNAL" || rpt.rptType == "GL_RPT_CHRBOOK") {
		//还原会计体系
		rpt.reqAccaList();
...
rpt.isShowFunnelBox = function() {
	$(rpt.namespace).on("click", ".rpt-funnelBtn", function(evt) {
		...
		$box.css({'left':left+'px','top':top+'px'}).show();
		var arrowLeft = $box.outerWidth(true)-($(this).offset().left-left+8);
		$box.find('b').css({'right':arrowLeft+8+'px'});
	});
...
rpt.showHideTree = function(dom, code, text) {
	...
	ufma.open({
		...
		data: {
			'flag': code,
			'rootName': text,
			'checkbox': (rpt.rptType == "GL_RPT_BAL" || rpt.rptType == 'GL_RPT_DAILYJOURNAL') ? true : false,
			'leafRequire': false,
			'data': params,
		},
