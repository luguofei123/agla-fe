$(function () {
	...
	var page = function () {
		...
		var portList = {
			...
			getExtendList: "/cu/bankExtend/getExtendList", //获取对账单扩展字段
			accItemTree: "/cu/common/getAccItemTree", //辅助项内容树
			getBankStatement: "/cu/bankStatement/getBankStatementList", //查询对账单
			saveBankStatement: "/cu/bankStatement/saveBankStatement", //保存对账单
			delBankStatement: "/cu/bankStatement/delBankStatementByGuids" //删除对账单
		};
		...
		return {
			...
			dateToday: function (startId, endId) {
				var mydate = new Date(page.today);
			...
			adjGridTop: function () {
				...
				var gridHeight = $(window).height() - gridTop - 125;
			...
			initTable: function (id, data, colArr) {
				...
				$('#' + id).ufDatagrid({
					...
					initComplete: function (options, data) {
						...
						/*						//控制输入只能有一方有值
												$(document).on('keyup', "#bankDataGridmoneyamtDr", function () {
													if ($(this).val() != '') {
														$("#bankDataGridmoneyamtCr").val('');
														var point = $(this).parent().attr('rowid');
														$("#bankDataGridBody #" + point).eq(1).find("td[name='amtCr']").html('');
													}
						
												})
												$(document).on('keyup', "#bankDataGridmoneyamtCr", function () {
													if ($(this).val() != '') {
														$("#bankDataGridmoneyamtDr").val('');
														var point = $(this).parent().attr('rowid');
														$("#bankDataGridBody #" + point).eq(1).find("td[name='amtDr']").html('');
													}
			...
			//根据对账方案显示扩展表格
			getExtendList: function (bankData) {
				var extArgu = {
					"schemaGuid": page.schemaGuid,
					enabled: "1",
					bookType: "02",
				...
				ufma.ajaxDef(portList.getExtendList, "get", extArgu, function (result) {
			...
			queryData: function (searchKey) {
				...
				if ($.isNull($("#accScheList").getObj().getValue())) {
					...
					ufma.get(portList.getBankStatement, queryArgu, function (result) {
						var bankData = result.data.cuBankStatementList;
						// var extArr = data.extendTableHeadList;
						page.getExtendList(bankData);
			...
			onEventListener: function () {
				...
				$(".btn-save").on("click", function () {
					...
					for (var i = 0; i < tableData.length; i++) {
						...
						tableData[i]["agencyCode"] = page.cbAgency.getValue();
				...
				$('.btn-print').click(function () {
					...
					if (tableData.length > 0) {
						uf.tablePrint({
							...
							pageHeight: 924,
							title: '银行对账单',
							topLeft: page.cbAgency.getValue(),
							topCenter: '',
			...
			initPage: function () {
				...
				page.cbAgency = $("#cbAgency").ufmaTreecombox2({
					...
					onchange: function (data) {
						...
						//结算方式
						// page.reqAccItemTree("SETMODE", "setmode");
						//票据类型
						page.reqAccItemTree("BILLTYPE", "billType");
						//银行对账单固定字段
						page.bankFixedArr = [{
							type: 'checkbox',
							field: '',
