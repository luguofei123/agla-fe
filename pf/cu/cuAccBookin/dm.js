var dm = {
	getCtrl: function(tag) {
		...
		switch(tag) {
			...
			case 'bankTake':
				//url = '/cu/cuAccountBook/select';
				url = '/cu/cuAccountBook/getBookTree?' +'&accountbookTypes=2';//银行日记账取ACCOUNTBOOK_TYPE=2
				break;
			case 'zeroTake':
				//url = '/cu/cuAccountBook/select';
				url = '/cu/cuAccountBook/getBookTree?' +'&accountbookTypes=3';//零余额日记账取ACCOUNTBOOK_TYPE=3
				break;
			//保存提现、存现
			case 'saveBookData':
				url = '/cu/billGenerateJournal/drawOrDepositCash';
				break;
			...
			//获取从国库生成tab标签
			case 'getBillTabs':
				url = '/cu/billGenerateJournal/getTabs';
				break;
			//获取动态表格内容
			case 'getBillColumns':
				url = '/cu/billGenerateJournal/getGeneratedBill';
				break;
			//从国库生成按钮
			case 'getBillBuild':
				url = '/cu/billGenerateJournal/billGenerateJournal';
				break;
			//取消从国库生成
			case 'getBillDelete':
				url = '/cu/billGenerateJournal/delCuJournal';
				break;
	...
	getAccoFZ:function(argu, _callback) {
		this.doPost('getAccoFZ', argu, _callback);
	},
	/*提现、存现、从国库生成--begin*/
	//提现、存现获取账簿
