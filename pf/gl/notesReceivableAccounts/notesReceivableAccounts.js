		...
		return {
			...
			*/
			...
			reDrawTable: function (isShowField, isShowBill, accoColumns) {
				// console.log('更新主表', isShowField, isShowBill)
				// console.log(accoColumns)
				var columns = [{
					title: "序号",
					data: "rowno",
					width: 30,
					...
					render: function(data, type, rowdata, meta) {
						...
						if(rowdata.rowType != 3) {
							...
							return '<a ' + (dis ? "disabled" : "") + ' class="btn btn-icon-only btn-sm btn-permission icon-writeoff btn-cav" data-toggle="tooltip" action= "addlower" title="核销" rowindex="' + meta.row + '">' +
								'<a ' + (dis ? "disabled" : "") + ' class="btn btn-icon-only btn-sm btn-permission icon-bad-debt-back  btn-ageAnalysis" data-toggle="tooltip" action= "addlower" title="账龄分析" rowindex="' + meta.row + '">' +
								...
								'<a ' + (dis ? "disabled" : "") + ' class="btn btn-icon-only btn-sm btn-permission icon-analysis btn-bad-back" data-toggle="tooltip" action= "addlower" title="坏账收回" rowindex="' + meta.row + '">';
						}
				...
				oTable = $('#' + page.tableId).dataTable({
					...
					drawCallback: function(settings) {
						...
						//弹出详细凭证
						$("#receivalbe-account").find("td span").on("click", function () {
							var vouGuid = $(this).attr("data-vouguid"); //凭证id
							var vouMenuId = 'f24c3333-9799-439a-94c9-f0cdf120305d';
							if (vouGuid) {
								var baseUrl = '/pf/gl/vou/index.html?menuid=' + vouMenuId + '&dataFrom=accDetailIncome&action=query&vouGuid=' + vouGuid + '&agencyCode=' + $('#cbAgency').getObj().getValue() + '&acctCode=' + $("#cbAcct").getObj().getValue();
			...
			openDueCav: function(rowData, btnCav) {
				var arguStr = {
				...
				//门户打开方式,1是跳转核销，0是跳转账龄分析
				var baseUrl = '';
				var turnTitle = '';
				if(btnCav == "1") {
					//$(this).attr('data-href', '/pf/gl/dueCav/dueCav.html?menuid=d3c413c3-8de1-4ba0-9534-5d0f24aec42c&dataFrom=notesAccounts&action=query');
					//	$(this).attr('data-title', '应收款核销');
					baseUrl = '/pf/gl/dueCav/dueCav.html?menuid=d3c413c3-8de1-4ba0-9534-5d0f24aec42c&dataFrom=notesAccounts&action=query';
					turnTitle = '应收款核销';
				} else {
					//$(this).attr('data-href', '/pf/gl/ageAnalysis/ageAnalysis.html?menuid=f4130c0f-bc66-46de-84f8-0a61cb924f3a&dataFrom=notesAccounts&action=query');
					//	$(this).attr('data-title', '应收款账龄分析');
					baseUrl = '/pf/gl/ageAnalysis/ageAnalysis.html?menuid=f4130c0f-bc66-46de-84f8-0a61cb924f3a&dataFrom=notesAccounts&action=query';
					turnTitle = '应收款账龄分析';
				}
				//window.parent.openNewMenu($(this));
				uf.openNewPage(page.isCrossDomain, $(this), 'openMenu', baseUrl, false, turnTitle);
			},
			onEventListener: function() {
				...
				$(document).on('click', function(e) {
					...
					} else if($(e.target).is('.btn-bad-back')) {
						var title = '应收款坏账收回';
				...
				$(document).on('click', function(e) {
					var rowIndex = $(e.target).attr('rowindex');
					var rowData = oTable.api(false).rows(rowIndex).data()[0];
					if($(e.target).is('.btn-cav')) {
						page.openDueCav(rowData, "1");
					} else if($(e.target).is('.btn-ageAnalysis')) {
						page.openDueCav(rowData, "0");
					}
				});
				//显示/隐藏列隐藏框
