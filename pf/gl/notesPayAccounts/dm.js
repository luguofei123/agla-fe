var dm = {
    getCtrl: function (tag) {
        ...
        switch (tag) {
            ...
            case 'search':
                ...
                break;
            //应付款收回
            case 'saveAccountBack':
                url = '/gl/current/rp/saveCurAssTakeBack';
                break;
            //删除收回
            case 'deleteCurAssBadDebt':
    ...
    //应付款收回
    doSaveAccountBack: function (argu, _callback) {
        this.doPost('saveAccountBack', argu, _callback);
    },
    //付款单位
...
dm.reqSavePrj = function (quryObj,isNew) {
    ...
    savePrjArgu.rptType = "PAY_ACCOUNT"; //账表类型
    savePrjArgu.setYear = ptData.svSetYear; //业务年度
    savePrjArgu.userId = ptData.svUserId; //用户Id    修改权限  将svUserCode改为 svUserId  20181012
