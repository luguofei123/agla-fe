	...
	var page = function() {
		...
		return {
				...
				ufma.get('/gl/elecommon/getEleCommonTree', reqData, function(result) {
					if(result.data) {
						$("#billType").ufTreecombox({
							...
							onComplete: function(sender) {
								if(window.ownerData.sendObj.billType) {
									$("#billType").getObj().val(window.ownerData.sendObj.billType);
								}
								ufma.hideloading();
							}
						});
			...
			save: function(flag) {
				...
				argu.payAmount = $('#subsidyMoney').val().replace(/,/g, '') //金额
			...
			initData: function() {
				...
				$('#subsidyMoney').val(window.ownerData.sendObj.payAmount);
