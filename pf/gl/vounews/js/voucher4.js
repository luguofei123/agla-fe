	...
	if(keyCode == "13") {
		...
		if(!isInputChange() || editStatus4){
			...
			if($(this).parent(".voucher-yc-bo").nextAll(".voucher-yc-bo").length - $(this).parent(".voucher-yc-bo").nextAll(".deleteclass").length < 1) {
				...
				var remarksval= ''
				if(isAssRemark){
					remarksval= $(this).parent(".voucher-yc-bo").find('.remark').val()
				}
				...
				if(isAssRemark){
					...
					voucherycbo += '<input type="text" class="ycbodyinp remark" value="'+remarksval+'" />'
					voucherycbo += '</div>'
				}
				for(var l in voucherycss) {
					if(isuniversities == true && l == 'projectCode'){
						voucherycbo += '<div  class="ycbody">'
						voucherycbo += '<input type="text" relation = "' + l + '" class="ycbodyinp projectCode" readonly = "true"  />'
						...
						voucherycbo += '<div  class="ycbody">'
						voucherycbo += '<input type="text" relation = "' + l + '" class="ycbodyinp quotaCode" readonly = "true"  />'
				...
				if(quanjuvoudatas.data[ss].field1 ==  '1'){
					voucherycbo += '<div  class="ycbody">'
					voucherycbo += '<input type="text" class="ycbodyinp field1" readonly = "true" />'
				...
				if($(this).parents(".voucher-yc").find(".voucher-yc-bo").length <= 8) {
...
$(document).on("click", ".ycadd,.ycaddopen", function(e) {
	if(!isInputChange()) {
		...
		var remarksval= ''
		if(isAssRemark){
			remarksval= $(this).parents('.voucher-yc-addbtns').next('.voucher-yc-bg').find('.voucher-yc-bo').eq(inde - 1).find('.remark').val()
		}
		var voucherycbo = '';
		voucherycbo += '<div class="voucher-yc-bo" op="0" inde=' + inde + ' >'
		if(isAssRemark){
...
$(document).on("click", ".fzhsadd", function(e) {
	...
	page.openNewPages(page.isCrossDomain,$(this), 'openMenu', baseUrl, false, titles);
})
//高校版本往来号/项目额度处理
//鼠标移入移出展示删除按钮
