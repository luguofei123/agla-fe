							...
							ufma.get(acctUrl, {}, function(result) {
								...
								page.cbAcct = $("#cbAcct").ufCombox({
									...
									onChange: function(sender, data) {
										...
										//改变账套  选择会计科目
										//修改权限  将svUserCode改为 svUserId  20181012				
											ufma.get('/gl/sys/coaAcc/getRptAccoTree?acctCode=' + sendObj.acctCode + '&agencyCode=' + data.agencyCode + '&setYear=' + ptData.svSetYear + '&userId=' + ptData.svUserId + '&accoType=' + sendObj.accoType+'&eleCode=ACCO', {}, function(result) {
													...
													onComplete: function() {
														...
														var timeId = setTimeout(function() {
															$("#cbAcco").getObj().val(openDue ? arguData.accoCode : '11');
															 $("#query_table").trigger("click"); //财务云项目CWYXM-8650--修改没有自动请求 表格数据的问题--zsj
															clearTimeout(timeId);
														}, 200);
													}
												});
				...
				oTable = $("#gridGOV").dataTable({
					...
					initComplete: function(settings, json) {
						...
						$("#dtToolbar .buttons-excel").off().on('click', function(evt) {
							...
							evt.preventDefault();
							//ufma.expXLSForDatatable($('#gridGOV'), '应付款核销');
							uf.expTable({
								title: '应付款核销',
								exportTable: '#gridGOV'
							});
					...
					"drawCallback": function() {
						//弹出详细凭证
						$("#aceCav").find("td span").on("click", function () {
