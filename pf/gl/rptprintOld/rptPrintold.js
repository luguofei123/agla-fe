			...
			initRptTemplate:function(result){
				...
				for(var i=0;i<data.length;i++){
					...
					if(data[i].defaultTmpl=="1"||defaultType==true){
						$op = $('<option value="'+jData+'" selected="selected">'+data[i].formattmplName+'</option>');
						defaultType = false;
					}else{
						$op = $('<option value="'+jData+'">'+data[i].formattmplName+'</option>');
					}
...
/*				if($('#rptType option:selected').val() == "GL_RPT_LEDGER" && $('#rptStyle option:eq(0)').val() == "SHULIANG"){
				...
				//加载模板
				ufma.post("/gl/GlRpt/getPrtFormatList",searchData,page.initRptTemplate);
                            ...
                            "render": function (data, type, rowdata, meta) {
                            	...
                            	if(rowdata.eleName!=null&&rowdata.eleSource!=null&&rowdata.codeRule!=null&&rowdata.eleCode!=null){
                            		...
                            		for(var i=0;i<eleSourceArr.length;i++){
                            			li += '<li class="accounting-item"><label class="mt-checkbox mt-checkbox-outline">'
                            				+'<input type="checkbox" name="'+rowdata.accoCode+'" value="'+eleSourceArr[i]+'" data-rule="'+codeRuleArr[i]+'" data-code="'+eleCodeArr[i]+'">'
                            				+'<i title ='+eleNameArr[i]+'>'+eleNameArr[i]+'</i><span></span></label>'
                            				+'<a class="accounting-btn pull-right"><span class="glyphicon icon-filter"></span></a></li>';
                            		}
                		...
                		},
                		{
                            ...
                            "render": function (data, type, rowdata, meta) {
                                return '<a class="btn btn-icon-only btn-sm btn-permission btn-print-preview" data-toggle="tooltip" action= "" title="打印预览">'
                                	...
                                	+'<a class="btn btn-icon-only btn-sm btn-permission btn-direct-print" data-toggle="tooltip" action= "" title="直接打印">'
			...
			initPage:function(){
				...
				page.cbAcct = $("#cbAcct").ufmaCombox2({
            		...
            		onchange:function(data){
						searchData.acctCode = data.CHR_CODE;
						searchData.acctName = data.CHR_NAME;
				...
				page.cbAgency=$("#cbAgency").ufmaTreecombox2({
				    ...
				    onchange:function(data){
				    	...
				    	//改变单位,账套选择内容改变
				    	var url = '/gl/eleCoacc/getCoCoaccs/'+searchData.agencyCode;
						var callback = function(result){
							page.cbAcct = $("#cbAcct").ufmaCombox2({
								data: result.data
							});
...
//                      height:600,
				...
				//点击辅助核算项表格内的按钮
				$('#rptPrint #rptPrint-data').on('click','.accounting-btn',function(){
					//如果勾选才能打开页面
					if($(this).parents('.accounting-item').find('input').prop('checked')==true){
						var conditionData = {};
...
//	                        height:480,
					...
					if($('#rptPrint-data tbody tr .checkboxes').length>0){
						...
						if($('#rptType option:selected').val()=="GL_RPT_JOURNAL"){
							...
							var getPdata = function(result){
								pDataArr.push(result.data);
							}
							if($('#rptPrint-data tbody tr:visible.selected').get(0)){
								$('#rptPrint-data tbody tr:visible.selected').each(function(){
									ufma.ajaxDef("/gl/rpt/getReportPrintData/"+$('#rptType option:selected').val(),"post",page.backTabArgu($(this)),getPdata);
								});
							}else{
								$('#rptPrint-data tbody tr:visible').each(function(){
									ufma.ajaxDef("/gl/rpt/getReportPrintData/"+$('#rptType option:selected').val(),"post",page.backTabArgu($(this)),getPdata);
							...
							printCache.models = [];
							printCache.agencyCode = searchData.agencyCode;
							printCache.acctCode = searchData.acctCode;
							printCache.componentId = $("#rptType option:selected").val();
							printCache.rptFormat = $("#rptStyle option:selected").val();
							//遍历
							$('#rptTemplate option').each(function(){
								//获取其他的打印模板数据
								var t = eval("("+$(this).val()+")");
								var model = {};
								model.pForm = t.formattmplValue;
								var n = $(this).index();
								model.pFormtmpl = sorttmplValueArr[n];
								model.pPaper = {
								...
								model.tmplImg = t.tmplImg;
								model.defaultTmpl = t.defaultTmpl;
								model.formattmplCode = t.formattmplCode;
								model.formattmplName = t.formattmplName;
								printCache.models.push(model);
							});
							...
							judgeCache.direct = "0";
							judgeCache.index = $('#rptTemplate option:selected').index();
							...
							//账表打印数据查询 ，接口/gl/rpt/getReportPrintData/{rptType}
							var getPdata = function(result){
								var pData = result.data;
								
								//打印缓存
								var printCache = {};
								printCache.pData = pData;
								printCache.models = [];
								printCache.agencyCode = searchData.agencyCode;
								printCache.acctCode = searchData.acctCode;
								printCache.componentId = $("#rptType option:selected").val();
							...
							ufma.post("/gl/rpt/getReportPrintData/"+$('#rptType option:selected').val(),page.backTabArgu(""),getPdata);
						}
					}else{
				...
				$('#rptPrint-data').on('click','a.btn-print-preview',function(){
					...
					if($('#rptType option:selected').val()=="GL_RPT_JOURNAL"){
						...
						ufma.ajaxDef("/gl/rpt/getReportPrintData/"+$('#rptType option:selected').val(),"post",page.countPages($(this).parents('tr')),getPdata);
						//打印缓存
						var printCache = {};
						printCache.pDataArr = pDataArr;
						printCache.models = [];
				...
				//点击行内直接打印
				$('#rptPrint-data').on('click','a.btn-direct-print',function(){
					if($('#rptType option:selected').val()=="GL_RPT_JOURNAL"){
						//明细账先执行保存模板，接口：/gl/GlRpt/saveRptModel
						page.savePrtTmpl();
					}
