		...
		return {
			...
			getTable:function(data){
				var search ={
					...
					startAmt:$("#vbstartTotalAmt").val(),
					endAmt:$("#vbendTotalAmt").val(),
				...
				var theadh = ''
				theadh+='<tr><th style="width: 80px;">凭证日期</th>'
				theadh+='<th style="width: 100px;">凭证字号</th>'
				theadh+='<th style="width: 100px;">往来号</th>'
				theadh+='<th>摘要</th>'
				...
				ufma.post(' /gl/vou/getCancelNexus',search,function(result){
					...
					for(var i=0;i<data.length;i++){
						...
						tr+='<td class="moenytd">'+$.formatMoney(data[i].STAD_AMT)+'</td>'
						var benvouamt = windata.voufield1amt[data[i].FIELD1]
						if(benvouamt == undefined){
							benvouamt = 0
						}
						var nocancelmoney = $.formatMoney(parseFloat(data[i].STAD_AMT)-parseFloat(data[i].CANCEL_MONEY)-parseFloat(benvouamt))
						var cencelmoeny = $.formatMoney(parseFloat(data[i].CANCEL_MONEY)+parseFloat(benvouamt))
