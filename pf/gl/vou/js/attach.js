	...
	if ($(".voucher-head").attr("namess") == undefined) {
		...
		$.ajax({
			...
			success: function (data) {
				...
				fileinputs += '<div class="attach-step2" style="display:block;">'
				...
				$("#vouAttachBox #attachUploadFile").fileinput({
					...
					$(".file-drop-zone-title").html('<span class="glyphicon icon-upload-lg"></span><p>点击或将文件拖拽到这里上传</p>');
				}).on('fileremoved', function (event, files, extra) {
					$(".file-drop-zone-title").addClass("hidden");
					$(".file-drop-zone-title").html('<span class="glyphicon icon-upload-lg"></span><p>点击或将文件拖拽到这里上传</p>');
				}) ;
				$(".file-drop-zone-title").html('<span class="glyphicon icon-upload-lg"></span><p>点击或将文件拖拽到这里上传</p>');
	...
	if (selectdata.data.vouStatus == "O") {
		ufma.open({
			...
			ondestory: function (result) {
				updateattach = result.updateattach
				AttachUpdata()
			}
		});
...
$(document).on("click", "#vouAttachBox .file-drop-zone-title", function () {
	$(this).hide();
	$("#vouAttachBox #attachUploadFile").click();
	$("#vouAttachBox .attach-upload-toolbar").show();
})
...
$(document).on("click", ".attach-show-li .icon-download", function () {
	...
	var filename =  $(this).parents(".attach-show-li").find('.attach-img-name b').text()
...
$(document).on("click", "#attachsaomiaoBtn", function () {
	if (selectdata.data.vouStatus == "O") {
		...
		$("#zezhao").html("");
		var fileinputs = '';
		fileinputs += '<div id="vouAttachBox" style="background:#fff">'
		fileinputs += '<div class="u-msg-title">'
		fileinputs += '<p style="font-size:16px">凭证附件</p>'
		fileinputs += '<div class="attach-close icon-close"></div>'
		fileinputs += '</div>'
		fileinputs += '<div class="u-msg-content">'
		fileinputs += '<div class="attach-step3" style="display:block;">'
		fileinputs += '<div class="attach-toolbar">'
		fileinputs += '<div class="attach-toolbar-back">'
		// shexiang += '<span class="glyphicon icon-angle-left"></span>返回'
		fileinputs += '</div>'
		fileinputs += '</div>'
		fileinputs += '<video id="myVideo" autoplay style="width:640px;height:360px"></video>'
		fileinputs += '<canvas id="luxcanvas" height="480" width="640" style="display:none;"></canvas>'
		fileinputs += '<p id="status" style="display: none;"></p>'
		fileinputs += '<div id="webcam">'
		fileinputs += '</div>'
		fileinputs += '<div id="btn-paishe">拍照</div>'
		fileinputs += '<div class="shangchuanneirong">'
		fileinputs += '</div>'
		fileinputs += '<div id="btn-paishetijiao">上传</div>'
		fileinputs += '</div>'
		fileinputs += '</div>'
		$("#zezhao").html(fileinputs);
		falshqidong = false;
