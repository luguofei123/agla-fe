...
function showbtns(voubtn) {
	if (getUrlParam("action") == "preview" && getUrlParam("preview") == "1") {
		...
		if(isvouHangup){
			z += '<div id="vouHangup" class="btn-voucher">国库单据挂接</div>'
		}
		if(ismodifybtn){
			z += '<div id="btn-voucher-xg" class="btn-voucher">修改</div>'
		}
		
		z += '<div id="voudataRefresh" class="btn-voucher">刷新</div>'
		z += '<div id="btn-voucher-qxbc" class="btn-voucher">取消</div>'
		z += '</div>'
		$(".voucherbtnModal").html(z).show();
...
function voucheralls() {
	...
	if (vousinglepz == false && vousinglepzzy == false) {
		...
		voucherallss += '<a href="javascript:" class="voucherjiass icon-Addto_icon"></a>'
		voucherallss += '<div class="voucher-head">'
		voucherallss += '<div class="abstracthead">摘要&nbsp&nbsp<span class=" despcSearchBtn" title="搜索摘要"></span></div>'
		...
		voucherallss += '<a href="javascript:" class="voucherjiass icon-Addto_icon"></a>'
		voucherallss += '<div class="voucher-head">'
		voucherallss += '<div class="voucherType"></div>'
		voucherallss += '<div class="abstracthead">摘要&nbsp&nbsp<span class=" despcSearchBtn" title="搜索摘要"></span></div>'
		voucherallss += '<div class="accountinghead">会计科目</div>'
		voucherallss += '<div class="fhead jf">'
		voucherallss += '<p>借方金额</p>'
		...
		voucherallss += '</div>'
		$(".voucherhe").after(voucherallss);
		$(".voucherall").height(571)
		$(".nowu").text("暂无信息");
		$(".voucher-history-zhankai").find("b").addClass("icon-angle-bottom").removeClass("icon-angle-top");
		$(".voucher-history-by").css("height", "0px");
		$(".voucher-history-by").html("");
		fixedabsulate()
	} else if (vousinglepzzy == true && vousinglepz == false && isdobabstractinp == false) {
		var voucherallss = '';
		...
		voucherallss += '<div class="abstractheadtwo">摘要&nbsp&nbsp<span class=" despcSearchBtn" title="搜索摘要"></span></div>'
		voucherallss += '<div class="accountinghead">会计科目</div>'
		voucherallss += '<div class="fhead jf">'
		voucherallss += '<p>借方金额</p>'
		voucherallss += '</div>'
		...
		for (var k = 0; k < 4; k++) {
			...
			voucherallss += '<div class="money-xs"></div>'
			voucherallss += '</div>'
			voucherallss += '<div class="voudeletediv"><div class="voudelete"><span class="glyphicon icon-trash"></span></div></div>'
			voucherallss += '</div>'
		...
		if(!isprojectByVou){
			if(windowWidth>1700){
				...
				voucherallss += '<div class="voucher voucher-singelzy voucher-singelzybg voucher-singelzyx">'
			}
		}else{
			if(windowWidth>1700){
				voucherallss += '<div class="voucher voucher-singelzy voucher-singelzybg voucher-singelzyproject">'
			}else{
				voucherallss += '<div class="voucher voucher-singelzy voucher-singelzybg voucher-singelzyx voucher-singelzyprojectx">'
			}
		}
		voucherallss += '<a href="javascript:" class="voucherjiass icon-Addto_icon"></a>'
		voucherallss += '<div class="voucher-head">'
		voucherallss += '<div class="voucherheadindex">序号</div>'
		voucherallss += '<div class="voucherheadcw">'
		voucherallss += '<div class="voucherheadtitle">财务会计</div>'
		voucherallss += '<div class="abstracthead">摘要&nbsp&nbsp<span class=" despcSearchBtn" title="搜索摘要"></span></div>'
		if(isprojectByVou){
			voucherallss += '<div class="projecthead">项目</div>'
		}
		voucherallss += '<div class="accountinghead">会计科目</div>'
		voucherallss += '<div class="fhead jf">'
		voucherallss += '<p>借方金额</p>'
		...
		voucherallss += '<div class="voucherheadtitle">预算会计</div>'
		if(!isprojectByVou){
			voucherallss += '<div class="abstracthead abstractheadys">摘要&nbsp&nbsp<span class=" despcSearchBtn" title="搜索摘要"></span></div>'
		}else{
			// voucherallss += '<div class="projecthead">项目</div>'
		}
		voucherallss += '<div class="accountinghead">会计科目</div>'
		...
		for (var k = 0; k < klen; k++) {
			...
			voucherallss += '</div>'
			if(isprojectByVou){
				voucherallss += '<div class="projectuniver">'
				voucherallss += '<textarea class="projectuniverinp"></textarea>'
				voucherallss += '<span class="vouopendelbycenter icon-close"></span>'
				voucherallss += '<div class="vouopenbtnbycenter">...</div>'
				voucherallss += '<div class="vouopenbtndetailbycenter">详情</div>'
				voucherallss += '</div>'
			}
			voucherallss += '<div class="accounting">'
			voucherallss += '<textarea class="accountinginp"></textarea>'
		...
		if(windowWidth>1700){
			$(".voucherall").height(561)
		}else{
			$(".voucherall").height(611)
		}
...
function tr() {
	...
	if (vousinglepz == false && vousinglepzzy == false) {
		...
		tr += '<a href="javascript:" class="voucherjiass icon-Addto_icon"></a>'
		tr += '</div>'
		...
		tr += '<div class="money-xs"></div>'
		tr += '</div>'
		tr += '<div class="voudeletediv"><div class="voudelete"><span class="glyphicon icon-trash"></span></div></div>'
		tr += '</div>'
		...
		tr += '</div>'
		if(isprojectByVou){
			tr += '<div class="projectuniver">'
			tr += '<textarea class="projectuniverinp"></textarea>'
			tr += '<span class="vouopendelbycenter icon-close"></span>'
			tr += '<div class="vouopenbtnbycenter">...</div>'
			tr += '<div class="vouopenbtndetailbycenter">详情</div>'
			tr += '</div>'
		}
		tr += '<div class="accounting">'
		tr += '<textarea class="accountinginp"></textarea>'
...
//加载会计科目框
function accountingInpload(seledata,type,cy){
	var centerss = '';
	if(isprojectByVou && vousinglepz == false && vousinglepzzy == true && isdobabstractinp == true){
		if(cy==1){
			centerss += '<div class="abstract">'
			if(seledata.descpt != undefined){
				centerss += '<textarea class="abstractinp">'+seledata.descpt+'</textarea>'
			}else{
				centerss += '<textarea class="abstractinp"></textarea>'
			}
			centerss += '</div>'
			centerss += '<div class="projectuniver">'
			centerss += '<textarea class="projectuniverinp"></textarea>'
			centerss += '<span class="vouopendelbycenter icon-close"></span>'
			centerss += '<div class="vouopenbtnbycenter">...</div>'
			centerss += '<div class="vouopenbtndetailbycenter">详情</div>'
			centerss += '</div>'
		}
	}else{
		centerss += '<div class="abstract">'
	...
	if(seledata.accoCode!=undefined && page.Codenameacct[seledata.accoCode]!=undefined){
		var j = page.Codenameacct[seledata.accoCode]
		var selacclen = $("#accounting-container").find("li").eq(j)
		var strdata = ''
		if(selacclen.hasClass("fuhs0") != true && seledata.vouDetailAsss != null) {
				strdata = JSON.stringify(seledata.vouDetailAsss) 
		}
		var accoval = ''
		if(isaccfullname) {
			accoval = seledata.accoCode + " " + selacclen.attr("fname")
		} else {
			accoval = seledata.accoCode + " " + selacclen.find(".ACCO_NAME").text()
		}
		if(strdata == ''){
			centerss += '<div class="accounting">'
		}else{
			centerss += "<div class='accounting' fudata='"+strdata+"'>"
		}
		if(selacclen.hasClass('clik1')) {
			centerss += '<textarea class="accountinginp" acca="'+selacclen.attr('acca')+'" accoindex="'+selacclen.attr('name')+'" code="'+selacclen.find(".ACCO_CODE").text()+'" name="'+selacclen.find(".ACCO_NAME").text()+'"'+
			' drCr="'+seledata.drCr+'" accbal="'+seledata.drCr+'" fname="'+selacclen.attr('fname')+'" cashflow="'+selacclen.attr('cashflow')+'" allowSurplus="'+selacclen.attr('allowSurplus')+'"'+
			'>'+accoval+'</textarea>'
		}else{
			centerss += '<textarea class="accountinginp">'+accoval+'</textarea>'
		}
		centerss += '<span class="accountingye">余额</span>'
		centerss += '<span class="accountingmx">明细账</span>'
		if(selacclen.hasClass("fuhs0") != true && selacclen.hasClass('clik1')) {
			centerss += '<div class="fuyan" style="display:block;"><span>辅</span></div>'
		}else{
	...
	var stadamt = ''
	var xsstadamt = ''
	if($.isNull(seledata.stadAmt)){
		stadamt  =  '0'
	}else{
		stadamt = parseFloat(seledata.stadAmt).toFixed(2)
	}
	if(seledata.stadAmt >= 10000000000 || vousinglepz == true || vousinglepzzy == true) {
		xsstadamt = formatNum(stadamt)
	} else if(seledata.stadAmt < 1 && seledata.stadAmt > 0) {
		xsstadamt = '0' + parseFloat(parseFloat(seledata.stadAmt).toFixed(2) * 100).toFixed(0)
	} else {
		xsstadamt = parseFloat(parseFloat(seledata.stadAmt).toFixed(2) * 100).toFixed(0)
	}
	if(seledata.stadAmt == "" && parseFloat(seledata.stadAmt) !=0){
		stadamt = ''
		xsstadamt = ''
	}
	if(seledata.drCr == -1 && seledata.stadAmt != null) {
		...
		centerss += '<input type="text" class="money-sr money-ys" value ="'+stadamt+'"/>'
		centerss += '<div class="money-xs">'+xsstadamt+'</div>'
		if(type == 'Basics'){
			centerss += '<div class="money-xsbg"></div>'
		}
		centerss += '</div>'
	} else if(seledata.drCr == 1 && seledata.stadAmt != null){
		centerss += '<div center class="money-jd moneyj">'
		centerss += '<input type="text" class="money-sr money-ys"  value ="'+stadamt+'"/>'
		centerss += '<div class="money-xs">'+xsstadamt+'</div>'
		if(type == 'Basics'){
...
//加载单行结构
function trcenterChatr(type,seledata,seledata2){
	if(seledata.linkModule == undefined){
		seledata.linkModule = ''
	}
	if(seledata.linkGuid == undefined){
		seledata.linkGuid = ''
	}
	if(seledata.refBillGuid == undefined){
		seledata.refBillGuid = ''
	}
	if(seledata.vouGuid == undefined){
		seledata.vouGuid = ''
	}
	if(seledata.detailGuid == undefined){
		seledata.detailGuid = ''
	}
	if(seledata2!=undefined){
		if(seledata2.linkModule == undefined){
			seledata2.linkModule = ''
		}
		if(seledata2.linkGuid == undefined){
			seledata2.linkGuid = ''
		}
		if(seledata2.refBillGuid == undefined){
			seledata2.refBillGuid = ''
		}
		if(seledata2.vouGuid == undefined){
			seledata2.vouGuid = ''
		}
		if(seledata2.detailGuid == undefined){
			seledata2.detailGuid = ''
		}
	}
	var  centerss = ''
	if(type == 'Basics'){
		if ($(".chaiwu").hasClass("xuanzhongcy")) {
			centerss += '<div class="voucher-body voucher-center voucher-center-cw" ifEdit=true linkModule="'+seledata.linkModule+'" linkGuid="'+seledata.linkGuid+'" refBillGuid="'+seledata.refBillGuid+'" namess="'+seledata.vouGuid+'" namesss="'+seledata.detailGuid+'">'
		} else {
			centerss += '<div class="voucher-body voucher-center voucher-center-ys" ifEdit=true linkModule="'+seledata.linkModule+'" linkGuid="'+seledata.linkGuid+'" refBillGuid="'+seledata.refBillGuid+'" namess="'+seledata.vouGuid+'" namesss="'+seledata.detailGuid+'">'
		}
		centerss += '<div class="voucherjias">'
		centerss += '<a href="javascript:" class="voucherjiass icon-Addto_icon"></a>'
		centerss += '</div>'
		centerss += accountingInpload(seledata,type)
		centerss += '<div class="voucherjians">'
		centerss += '<a href="javascript:" class="voucherjian icon-trash"></a>'
		centerss += '</div>'
		centerss += '</div>'
	}else if (type =='singel'){
		if (seledata.accaCode != undefined) {
			if (seledata.accaCode == '1') {
				centerss += '<div class="voucher-body voucher-center voucher-center-cw" ifEdit=true linkModule="'+seledata.linkModule+'" linkGuid="'+seledata.linkGuid+'" refBillGuid="'+seledata.refBillGuid+'" namess="'+seledata.vouGuid+'" namesss="'+seledata.detailGuid+'">'
			} else {
				centerss += '<div class="voucher-body voucher-center voucher-center-ys" ifEdit=true linkModule="'+seledata.linkModule+'" linkGuid="'+seledata.linkGuid+'" refBillGuid="'+seledata.refBillGuid+'" namess="'+seledata.vouGuid+'" namesss="'+seledata.detailGuid+'">'
			}
			...
			centerss += '<div class="voucher-body voucher-center" ifEdit=true linkModule="'+seledata.linkModule+'" linkGuid="'+seledata.linkGuid+'" refBillGuid="'+seledata.refBillGuid+'" namess="'+seledata.vouGuid+'" namesss="'+seledata.detailGuid+'">'
			centerss += '<div class="voucherjias">'
			centerss += '<a href="javascript:" class="voucherjiass icon-Addto_icon"></a>'
			centerss += '</div>'
			centerss += '<div class="vouchertypebody">'
		...
		centerss += accountingInpload(seledata)
		centerss += '<div class="voucherjians">'
		centerss += '<a href="javascript:" class="voucherjian icon-trash"></a>'
		centerss += '</div>'
		centerss += '</div>'
	}else if(type == 'singelzy'){

		centerss += '<div class="voucher-body voucher-center voucher-centercw" ifEdit=true linkModule="'+seledata.linkModule+'" linkGuid="'+seledata.linkGuid+'" refBillGuid="'+seledata.refBillGuid+'" namess="'+seledata.vouGuid+'" namesss="'+seledata.detailGuid+'">'
		centerss += '<div class="voucherjias">'
		centerss += '<a href="javascript:" class="voucherjiass icon-Addto_icon"></a>'
		centerss += '</div>'
		centerss += accountingInpload(seledata)
		centerss += '<div class="voudeletediv"><div class="voudelete"><span class="glyphicon icon-trash"></span></div></div>'
		centerss += '</div>'
		centerss += '<div class="voucher-body voucher-center voucher-centerys" ifEdit=true linkModule="'+seledata2.linkModule+'" linkGuid="'+seledata2.linkGuid+'" refBillGuid="'+seledata2.refBillGuid+'" namess="'+seledata2.vouGuid+'" namesss="'+seledata2.detailGuid+'">'
		centerss += accountingInpload(seledata2)
		centerss += '<div class="voudeletediv"><div class="voudelete"><span class="glyphicon icon-trash"></span></div></div>'
		centerss += '<div class="voucherjians">'
		centerss += '<a href="javascript:" class="voucherjian icon-trash"></a>'
		centerss += '</div>'
		centerss += '</div>'
	}else if (type == 'singelzybg'){
		centerss += '<div class="voucher-body voucher-center voucher-centercw" ifEdit=true linkModule="'+seledata.linkModule+'" linkGuid="'+seledata.linkGuid+'" refBillGuid="'+seledata.refBillGuid+'" namess="'+seledata.vouGuid+'" namesss="'+seledata.detailGuid+'">'
		centerss += '<div class="voucherjias">'
		centerss += '<a href="javascript:" class="voucherjiass icon-Addto_icon"></a>'
		centerss += '</div>'
		centerss += accountingInpload(seledata,'',1)
		centerss += '<div class="voudeletediv"><div class="voudelete"><span class="glyphicon icon-trash"></span></div></div>'
		centerss += '</div>'
		centerss += '<div class="voucher-body voucher-center voucher-centerys" ifEdit=true linkModule="'+seledata2.linkModule+'" linkGuid="'+seledata2.linkGuid+'" refBillGuid="'+seledata2.refBillGuid+'" namess="'+seledata2.vouGuid+'" namesss="'+seledata2.detailGuid+'">'
		centerss += accountingInpload(seledata2,'',2)
		centerss += '<div class="voudeletediv"><div class="voudelete"><span class="glyphicon icon-trash"></span></div></div>'
		centerss += '<div class="voucherjians">'
		centerss += '<a href="javascript:" class="voucherjian icon-trash"></a>'
		centerss += '</div>'
...
function chapzTr() {
	...
	if (vousinglepz == false && vousinglepzzy == false) {
		if (selectdata.data.vouDetails.length >= 4) {
			...
			$(".voucherall").height(481 + len)
			for (var i = 0; i < selectdata.data.vouDetails.length; i++) {
				centerss += trcenterChatr('Basics',selectdata.data.vouDetails[i])
			}
		} else {
			$(".voucherall").height(481)
			for (var i = 0; i < selectdata.data.vouDetails.length; i++) {
			...
			for (var i = 0; i < 4-selectdata.data.vouDetails.length; i++) {
				centerss += trcenterChatr('Basics',{})
			}
		}
		...
		if (selectdata.data.vouDetails.length >= 6) {
			...
			$(".voucherall").height(571 + len)
			for (var k = 0; k < selectdata.data.vouDetails.length; k++) {
				centerss += trcenterChatr('singel',selectdata.data.vouDetails[k])
			}
		} else {
			$(".voucherall").height(571)
			for (var k = 0; k < selectdata.data.vouDetails.length; k++) {
			...
			for (var k = 0; k < 6-selectdata.data.vouDetails.length; k++) {
				centerss += trcenterChatr('singel',{})
			}
		}
	} else if (vousinglepz == false && vousinglepzzy == true && isdobabstractinp == false) {
		if (selectdata.data.vouDetails.length >= 8) {
			var len = (selectdata.data.vouDetails.length / 2 - 4) * 50
			$(".voucherall").height(481 + len)
			for (var k = 0; k < selectdata.data.vouDetails.length ; k+=2) {
				centerss += trcenterChatr('singelzy',selectdata.data.vouDetails[k],selectdata.data.vouDetails[k+1])
			}
		} else {
			$(".voucherall").height(481)
			for (var k = 0; k < selectdata.data.vouDetails.length ; k+=2) {
			...
			for (var k = 0; k < 8-selectdata.data.vouDetails.length ; k+=2) {
				centerss += trcenterChatr('singelzy',{},{})
			}
		}
		...
		if (selectdata.data.vouDetails.length >= 8) {
			...
			for (var k = 0; k < selectdata.data.vouDetails.length ; k+=2) {
				centerss += trcenterChatr('singelzybg',selectdata.data.vouDetails[k],selectdata.data.vouDetails[k+1])
			}
			centerss += '</div>'
		} else {
			...
			for (var k = 0; k < 8-selectdata.data.vouDetails.length ; k+=2) {
				centerss += trcenterChatr('singelzybg',{},{})
			}
			centerss += '</div>'
		}
	}
...
function fzhspzpj(fzhscf) {
	...
	if (vousinglepzzy) {
		if(isalonebill && quanjuvoudatas.data[ss].showBill == 1){
			voucheryc += '<div class="voucher-yc-title"><span class="titlename maxwtitle">科目：' + $('.accountcheck').val() + '</span>'
		}else{
		...
		voucheryc += '</label>'
		if ($('.accountcheck').parents(".voucher-center").find('.moneyj').find('.money-xs').html() != '') {
	...
	voucherycss = resOrderAccItem(nowAccoCode, accItemArr,$('.accountcheck').parents(".voucher-center"));
	var noshowaccitem = showAccItem(nowAccoCode, accItemArr)
	//          }
	var isyic = false
	// if(isprojectByVou &&  $('.accountcheck').parents(".voucher-center").find('.accounting').attr('accos')!=undefined){
	// 	var accosdata = JSON.parse( $('.accountcheck').parents(".voucher-center").find('.accounting').attr('accos'))
	// 	// if(voucherycss.quotaCode.quotaCode!=undefined){
	// 	var quotaCodedata = []
	...
	if (quanjuvoudatas.data[ss].field2 == 1) {
		...
		assnoll += '</ul>'
		$('#AssDataAll').append(assnoll)
		lengvouyc += '<div class="ychead" name="field2">合同号</div>'
	}
	...
	for (var i = 0; i < fzbolen; i++) {
		...
		if (isAssRemark) {
			...
			lengvouyc += '<textarea class="ycbodyinp remark"></textarea>'
			lengvouyc += '</div>'
		}
		for (var l in voucherycss) {
			if(isuniversities == true && l == 'projectCode'){
				lengvouyc += '<div  class="ycbody">'
				lengvouyc += '<textarea relation = "' + l + '" class="ycbodyinp projectCode" readonly = "true"></textarea>'
				...
				lengvouyc += '<div  class="ycbody">'
				lengvouyc += '<textarea relation = "' + l + '" class="ycbodyinp quotaCode" readonly = "true"></textarea>'
			...
			if ((voucherycss[l]["ELE_CODE"] == "CURRENT" && isbussDate) || (voucherycss[l]["ELE_CODE"] == "EMPLOYEE" && isbussDate)) {
				lengvouyc += '<div  class="ycbody">'
				lengvouyc += '<input  value="' + $("#dates").getObj().getValue() + '" class="ycbodyinp bussDate"  />'
				lengvouyc += '</div>'
			}
		}
		if (quanjuvoudatas.data[ss].field1 ==  '1') {
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea class="ycbodyinp field1" readonly = "true"></textarea>'
		...
		if (quanjuvoudatas.data[ss].expireDate == 1) {
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<input type="text" value="'+$("#dates").getObj().getValue()+'" class="ycbodyinp daoqiri">'
			lengvouyc += '</div>'
		}
		if (quanjuvoudatas.data[ss].showBill == 1 && !isalonebill) {
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea class="ycbodyinp billNoinp"></textarea>'
			lengvouyc += '</div>'
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea relation="billTypeinp" class="ycbodyinp billTypeinp"></textarea>'
			lengvouyc += '</div>'
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<input type="text" value="'+$("#dates").getObj().getValue()+'" class="ycbodyinp billDateinp">'
			lengvouyc += '</div>'
		}
		if (quanjuvoudatas.data[ss].currency == 1) {
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea relation = "currency" type="text" class="ycbodyinp curcodeinp"></textarea>'
			lengvouyc += '</div>'
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea class="ycbodyinp exrateinp"></textarea>'
			lengvouyc += '</div>'
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea class="ycbodyinp curramtinp"></textarea>'
			lengvouyc += '</div>'
		}
		if (quanjuvoudatas.data[ss].qty == 1) {
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea class="ycbodyinp priceinp" ></textarea>'
			lengvouyc += '</div>'
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea class="ycbodyinp qtyinp"  qty="' + quanjuvoudatas.data[ss].qtyDigits + '" ></textarea>'
			lengvouyc += '</div>'
		}
		if (quanjuvoudatas.data[ss].field2 == 1) {
			lengvouyc += '<div  class="ycbody">'
			lengvouyc += '<textarea relation = "field2" class="ycbodyinp field2inp" ></textarea>'
			lengvouyc += '<span class="fielddetail">详情</span>'
			lengvouyc += '</div>'
		}
		lengvouyc += '<input class="ycbody yctz" />'
		lengvouyc += '<span class="Assaccbala">余额</span>'
		lengvouyc += '</div>'
	}
	lengvouyc += '</div>'
...
function vouYcDrCrRadio(vcenter){
	if( vcenter.find('.fuyan').css('display')=="block"){
		vcenter.find('.voucher-yc-title').css('visibility','visible')
	}else{
		vcenter.find('.voucher-yc-title').css('visibility','hidden')
	}
	if(vcenter.parents('.voucher').hasClass('voucher-singelzybg') || vcenter.parents('.voucher').hasClass('voucher-singelzyx')) {
		$('.voucher-close').click()
		var accountname = ''
		if(isaccfullname) {
			accountname = vcenter.find('.accountinginp').attr('code')+' '+vcenter.find('.accountinginp').attr('fname')
		} else {
			accountname = vcenter.find('.accountinginp').attr('code')+' '+vcenter.find('.accountinginp').attr('name')
		}
		vcenter.find('.voucher-yc-title .titlename').html('科目：' + accountname)
...
function setfzhsxl(fzdata, pasyc) {
	...
	var accItemArr = voucherycss;
	voucherycss = resOrderAccItem(nowAccoCode, accItemArr,pasyc.parents('.voucher-center'));
...
function getAccoByAss(dom,noarr) {
	...
	for (var k = 0; k < headLen; k++) {
		...
		if (noarr[dd] != undefined) {
			...
			var itemCodeName = domsr.find(".ycbody").eq(k).find(".ycbodyinp").val();
			var itemCode = itemCodeName.substring(0, itemCodeName.indexOf(" "));
			var itemId = domsr.find(".ycbody").eq(k).find(".ycbodyinp").attr('itemid')
...
//浮动下拉框全局定位
function treePosition(that,id,top){
	var sctop = that.offset().top - $(window).scrollTop() - that.parents('.voucher-yc-bg').scrollTop() + 50
	var scbottom = that.offset().top - $(window).scrollTop() - that.parents('.voucher-yc-bg').scrollTop()-202
	if(that.parents('.voucher-yc-bg').length==0){
		sctop = that.offset().top - $(window).scrollTop() + 50
		scbottom = that.offset().top - $(window).scrollTop()-202
		if(that.hasClass('billTypeinp')){
			sctop = that.offset().top - $(window).scrollTop() + 20
		}
	}
	var scleft = that.offset().left - $(window).scrollLeft()
	var widthass = that.width()
	var heights = $('html').height()
	if(top || sctop +200>heights){
		id.show().css({
			'position': 'fixed',
			'top': scbottom,
			'height':'200px',
