...
$(document).on("keydown", function(event) {
	...
	if(event.keyCode == 70 && event.ctrlKey) {
		event.preventDefault();
		var kleft = $('.voucher-center').eq(1).find('.moneyd').offset().left + $('.voucher-center').eq(1).find('.moneyd').width() - $(window).scrollLeft()-$("#vouaccSearchs").width()
		$("#vouaccSearchs").show().css({
			'left':kleft-8
		})
		event.keyCode = 0;
		if(event.preventDefault) { // firefox
