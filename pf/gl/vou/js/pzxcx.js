		...
		$.ajax({
			...
			success: function(data) {
				if(data.flag == "success" && data.data != null) {
					...
					rpt.foryear = selectdata.data.setYear
					...
					if(getUrlParam("deguid")!=undefined && getUrlParam("deguid")!='undefined'){
						if(!$.isNull(getUrlParam("desguid"))){
							...
							if(ths.parents(".voubodyscroll").length>0){
								...
								assh.parents('.voucher-center').addClass('voucher-center-active')
								...
								var toplenass = assh.offset().top -keys
								ths.find(".voucher-yc-scroll").scrollTop(toplenass);
							}
						}else{
							var deguid = getUrlParam("deguid")
							var ths = $(".voucher-center[namesss='"+deguid+"']")
...
//判定是否为外来生成凭证，用以修改保存调用方法
var  isforeign =false;
if(getUrlParam("dataFrom") == "foreign" && action=="save"){
	var redisid =getUrlParam("redisid")
	ufma.get('/lp/public/outside/getVouHeadFromRedis/'+redisid,'',function(result){
		if(result.data.flag=='success'){
			var voudata = result.data.data
			var csagencyCode = "";
			var csacctCode = "";
			if(voudata[0] != null && voudata[0] != '') {
				csagencyCode = voudata[0].agencyCode
				csacctCode = voudata[0].acctCode
			} else {
				csagencyCode = voudata[1].agencyCode
				csacctCode = voudata[1].acctCode
			}
			rpt.cbAgency.val(csagencyCode);
			rpt.cbAcct.val(csacctCode);
			rpt.nowAgencyCode = rpt.cbAgency.getValue();
