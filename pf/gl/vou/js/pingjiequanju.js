...
rpt.foryear = ufgovkey.svSetYear
var  forOpenReadonly = false
if(getUrlParam("readonly") == "1") {
	forOpenReadonly = true
}
//判定是否当做弹窗打开页面
...
rpt.cbAgency = $("#cbAgency").ufmaTreecombox2({
	...
	onchange: function(data) {
		...
		ufma.ajaxDef("/gl/vou/getSysRuleSet/" + rpt.nowAgencyCode + "?chrCodes=GL001,GL005,GL002,GL011,GL012,GL010,GL026,GL028,GL038,GL035,GL044,GL046,GL047,GL050,GL051,GL052,GL053,GL054,GL059,GL061,GL062,GL063,GL065,GL067",'get', "", function(data) {
			...
			isprojectByVou = data.data.GL067
			// isprojectByVou = true
			if(isprojectByVou){
			...
			isvouHangup = data.data.GL026
			isalonebill = data.data.GL065
			if(ifForIndex != true) {
				isvousourceclick = true
			}
			if(forOpenReadonly){
				isvousourceclick = false
			}
...
rpt.cbAcct = $("#cbAcct").ufmaCombox2({
	...
	onchange: function(data) {
		...
		ufma.ajaxDef('/pub/user/menu/config/select?agencyCode='+rpt.nowAgencyCode+'&acctCode='+rpt.nowAcctCode+'&menuId=f24c3333-9799-439a-94c9-f0cdf120305d', "get", '', function(data) {
   			...
   			if(data.data.iszysingleinsert == 0){
				$("#iszysingleinsertN").prop('checked', true).attr('checked', true)
			 	$("#iszysingleinsertY").prop('checked', false).attr('checked', false)
				iszysingleinsert = false
   			}else{
				$("#iszysingleinsertN").prop('checked', false).attr('checked', false)
				$("#iszysingleinsertY").prop('checked', true).attr('checked', true)
				iszysingleinsert = true
   			}
   			if(data.data.isetcAmt == 0){
				$("#etcAmtN").prop('checked', true).attr('checked', true)
			 	$("#etcAmtY").prop('checked', false).attr('checked', false)
				 isetcAmt = false
   			}else{
				$("#etcAmtN").prop('checked', false).attr('checked', false)
				$("#etcAmtY").prop('checked', true).attr('checked', true)
				isetcAmt = true
   			}
   			if(data.data.isSummaryVou == 0){
				$("#isSummaryVouN").prop('checked', true).attr('checked', true)
			 	$("#isSummaryVouY").prop('checked', false).attr('checked', false)
				 isSummaryVou = true
   			}else{
				$("#isSummaryVouN").prop('checked', false).attr('checked', false)
				$("#isSummaryVouY").prop('checked', true).attr('checked', true)
				isSummaryVou = false
   			}
		})
		//判定是单凭证还是双凭证
		if(data.isParallel == '1' && data.isDoubleVou == '1') {
			rpt.isParallel = 1
			vousinglepz = false
			$('.vouisdouble,#iszysingleinsertYN').hide()
			$('.voucherhe').css({
				'width': '1094px',
				'margin-left': '-557px'
			})
			...
			rpt.isParallel = 1
			vousinglepz = true
			$('.vouisdouble,#iszysingleinsertYN').show()
			$('.voucherhe').css({
				'width': '1094px',
				'margin-left': '-557px'
			})
		...
		if($("#vouislrud").hasClass('zys') && data.isParallel == '1' && data.isDoubleVou == '0') {
			vousinglepzzy = true
			vousinglepz = false
			if(isdobabstractinp && windowWidth>1700) {
				$('.voucherhe').css({
					'width': '1700px',
					'margin-left': '-850px'
				})
...
//返回正确排序的辅助项对象
function resOrderAccItem(accoCode, accItemArr,ths) {
	var accaCode=quanjuvoudatas.data[page.Codenameacct[accoCode]].accaCode
	...
	if(nowAccItemSeq != undefined) {
		...
		for(var i = 0; i < accitemList.length; i++) {
			if(accitemList[i]["IS_SHOW"] == 1 && accitemList[i]["ACCITEM_CODE"] != "CURRENCY") {
				if((accitemList[i]["ACCITEM_CODE"] !="PROJECT" && isprojectByVou ) || !isprojectByVou || vousinglepz || accaCode!=1){
					...
					if(ths.find('.accounting').attr('accos')!=undefined){
						var accosdata = JSON.parse(ths.find('.accounting').attr('accos'))
						projectCode = accosdata.projectCode
						if(accitemList[i]["ACCITEM_CODE"] == "QUOTA"){
							var quotaCodedata = []
							var quotaCodedatas = fzhsxl['quotaCode']
							for(var z=0;z<accosdata.quotas.length;z++){
								if(quotaCodeindex[accosdata.quotas[z].quotaCode]!=undefined){
									quotaCodedata.push(quotaCodedatas[quotaCodeindex[accosdata.quotas[z].quotaCode]])
								}
							}
							accitemList[i]["RANGEs"] =accitemList[i]["RANGE"]
							accitemList[i]["RANGE"] =quotaCodedata
						}
						
						if(accitemList[i]["ACCITEM_CODE"] == "DEPPRO"){
							var depproCodedata = []
							var depproCodedatas = fzhsxl['depproCode']
							for(var z=0;z<accosdata.depproCodes.length;z++){
								if(depproCodeindex[accosdata.depproCodes[z].chrCode]!=undefined){
									depproCodedata.push(depproCodedatas[depproCodeindex[accosdata.depproCodes[z].chrCode]])
								}
							}
							accitemList[i]["RANGEs"] =accitemList[i]["RANGE"]
							accitemList[i]["RANGE"] =depproCodedata
						}
						if(!$.isNull(accosdata[code])){
							orderAccItem[code].value = accosdata[code]
						}
					} 
					if(ths.find('.accounting').attr('accos')==undefined && accitemList[i]["ACCITEM_CODE"] == "QUOTA" && accitemList[i]["RANGEs"]!=undefined){
						accitemList[i]["RANGE"] = accitemList[i]["RANGEs"]
					}
					if(ths.find('.accounting').attr('accos')==undefined && accitemList[i]["ACCITEM_CODE"] == "DEPPRO" && accitemList[i]["RANGEs"]!=undefined){
						accitemList[i]["RANGE"] = accitemList[i]["RANGEs"]
					}
					if(accitemList[i]["RANGE"] != undefined && orderAccItem[code] != undefined && accitemList[i]["RANGE"].length>0){
						...
						if(isprojectByVou){
							vouassRange[accitemList[i]["ACCO_CODE"]+code+projectCode] =ranges
							vouassRange[accitemList[i]["ACCO_CODE"]+code+projectCode+'data'+''] =accitemList[i]["RANGE"]
						}
...
function upadd(isyear) {
	...
	$("#pzzhuantaiC").hide().text("未挂接").removeAttr("vou-status");
	...
	if(isyear != undefined && rpt.foryear != ufgovkey.svSetYear){
		rpt.foryear = ufgovkey.svSetYear
...
//财务会计科目和预算会计科目加载在一个下拉框内
function acctaccaall() {
	var tr = ''
	page.Codenameacct = {}
	for(var i = 0; i < quanjuvoudatas.data.length; i++) {
		if(quanjuvoudatas.data[i].remark == null){
			quanjuvoudatas.data[i].remark = ''
		}
		page.Codenameacct[quanjuvoudatas.data[i].accoCode] = i
		if(quanjuvoudatas.data[i].enabled===0){
			if(quanjuvoudatas.data[i].levNum > 0) {
				tr += '<li assCode='+quanjuvoudatas.data[i].assCode+' allowSurplus='+quanjuvoudatas.data[i].allowSurplus+' levNum="' + quanjuvoudatas.data[i].levNum + '" style="display:none;" fname="' + quanjuvoudatas.data[i].accoFullName + '" title="' + quanjuvoudatas.data[i].remark + '" acca="' + quanjuvoudatas.data[i].accaCode + '" name="' + i + '" cashflow="' + quanjuvoudatas.data[i].isCashflow + '" role="' + quanjuvoudatas.data[i].accBal + '" class="allnoshow  unselected leave' + quanjuvoudatas.data[i].levNum + ' clik' + quanjuvoudatas.data[i].isLeaf + ' fuhs' + quanjuvoudatas.data[i].accitemNum + '"><p><span class="ACCO_CODE">' + quanjuvoudatas.data[i].accoCode + '</span> <span class="ACCO_NAME">' + quanjuvoudatas.data[i].accoName + '</span></p><span class="fu">辅</span></li>'
			} else {
				tr += '<li assCode='+quanjuvoudatas.data[i].assCode+' allowSurplus='+quanjuvoudatas.data[i].allowSurplus+' levNum="' + quanjuvoudatas.data[i].levNum + '" fname="' + quanjuvoudatas.data[i].accoFullName + '" title="' + quanjuvoudatas.data[i].remark + '" acca="' + quanjuvoudatas.data[i].accaCode + '" name="' + i + '" cashflow="' + quanjuvoudatas.data[i].isCashflow + '"role="' + quanjuvoudatas.data[i].accBal + '" class="allnoshow PopListBoxItem unselected leave' + quanjuvoudatas.data[i].levNum + ' clik' + quanjuvoudatas.data[i].isLeaf + ' fuhs' + quanjuvoudatas.data[i].accitemNum + '"><p class="sq"><span class="ACCO_CODE">' + quanjuvoudatas.data[i].accoCode + '</span> <span class="ACCO_NAME">' + quanjuvoudatas.data[i].accoName + '</span></p><span class="fu">辅</span></li>'
			}
		}else{
			if(quanjuvoudatas.data[i].levNum > 0) {
				tr += '<li assCode='+quanjuvoudatas.data[i].assCode+' allowSurplus='+quanjuvoudatas.data[i].allowSurplus+' levNum="' + quanjuvoudatas.data[i].levNum + '" style="display:none;" fname="' + quanjuvoudatas.data[i].accoFullName + '" title="' + quanjuvoudatas.data[i].remark + '" acca="' + quanjuvoudatas.data[i].accaCode + '" name="' + i + '" cashflow="' + quanjuvoudatas.data[i].isCashflow + '" role="' + quanjuvoudatas.data[i].accBal + '" class="unselected leave' + quanjuvoudatas.data[i].levNum + ' clik' + quanjuvoudatas.data[i].isLeaf + ' fuhs' + quanjuvoudatas.data[i].accitemNum + '"><p><span class="ACCO_CODE">' + quanjuvoudatas.data[i].accoCode + '</span> <span class="ACCO_NAME">' + quanjuvoudatas.data[i].accoName + '</span></p><span class="fu">辅</span></li>'
			} else {
				tr += '<li assCode='+quanjuvoudatas.data[i].assCode+' allowSurplus='+quanjuvoudatas.data[i].allowSurplus+' levNum="' + quanjuvoudatas.data[i].levNum + '" fname="' + quanjuvoudatas.data[i].accoFullName + '" title="' + quanjuvoudatas.data[i].remark + '" acca="' + quanjuvoudatas.data[i].accaCode + '" name="' + i + '" cashflow="' + quanjuvoudatas.data[i].isCashflow + '"role="' + quanjuvoudatas.data[i].accBal + '" class="PopListBoxItem unselected leave' + quanjuvoudatas.data[i].levNum + ' clik' + quanjuvoudatas.data[i].isLeaf + ' fuhs' + quanjuvoudatas.data[i].accitemNum + '"><p class="sq"><span class="ACCO_CODE">' + quanjuvoudatas.data[i].accoCode + '</span> <span class="ACCO_NAME">' + quanjuvoudatas.data[i].accoName + '</span></p><span class="fu">辅</span></li>'
			}
		}
	}
	//高校版本添加部门经济分类和额度的索引
	...
	depproCodeindex = {}
	projectCodeindex = {}
	if(fzhsxl['expecoCode']!=undefined){
		for(var i=0;i<fzhsxl['expecoCode'].length;i++){
			expecoCodeindex[fzhsxl['expecoCode'][i].code]=i
		}
	}
	if(fzhsxl['quotaCode']!=undefined){
		for(var i=0;i<fzhsxl['quotaCode'].length;i++){
			quotaCodeindex[fzhsxl['quotaCode'][i].code]=i
		}
	}
	if(fzhsxl['depproCode']!=undefined){
		for(var i=0;i<fzhsxl['depproCode'].length;i++){
			depproCodeindex[fzhsxl['depproCode'][i].code]=i
		}
	}
	if(fzhsxl['projectCode']!=undefined){
		for(var i=0;i<fzhsxl['projectCode'].length;i++){
			projectCodeindex[fzhsxl['projectCode'][i].code]=i
		}
	}
...
//加载高校版本传回的会计科目范围
function acctaccalforproject(forprojectdata,forprojectacca){
	var forprojectdatatr = ''
	for(var i = 0; i < forprojectdata.length; i++) {
		if(forprojectdata[i].remark == null){
			forprojectdata[i].remark = ''
		}
		if(forprojectdata[i].accaCode == forprojectacca) {
			if(forprojectdata[i].enabled===0){
				if(forprojectdata[i].levNum > 0) {
					forprojectdatatr += '<li assCode='+forprojectdata[i].assCode+' allowSurplus='+forprojectdata[i].allowSurplus+' levNum="' + forprojectdata[i].levNum + '" fname="' + forprojectdata[i].accoFullName + '" title="' + forprojectdata[i].remark + '" acca="' + forprojectdata[i].accaCode + '" name="' + page.Codenameacct[forprojectdata[i].accoCode] + '" cashflow="' + forprojectdata[i].isCashflow + '" role="' + forprojectdata[i].accBal + '" class="allnoshow  unselected leave' + forprojectdata[i].levNum + ' clik' + forprojectdata[i].isLeaf + ' fuhs' + forprojectdata[i].accitemNum + '"><p><span class="ACCO_CODE">' + forprojectdata[i].accoCode + '</span> <span class="ACCO_NAME">' + forprojectdata[i].accoName + '</span></p><span class="fu">辅</span></li>'
				} else {
					forprojectdatatr += '<li assCode='+forprojectdata[i].assCode+' allowSurplus='+forprojectdata[i].allowSurplus+' levNum="' + forprojectdata[i].levNum + '" fname="' + forprojectdata[i].accoFullName + '" title="' + forprojectdata[i].remark + '" acca="' + forprojectdata[i].accaCode + '" name="' + page.Codenameacct[forprojectdata[i].accoCode] + '" cashflow="' + forprojectdata[i].isCashflow + '"role="' + forprojectdata[i].accBal + '" class="allnoshow PopListBoxItem unselected leave' + forprojectdata[i].levNum + ' clik' + forprojectdata[i].isLeaf + ' fuhs' + forprojectdata[i].accitemNum + '"><p class="sq"><span class="ACCO_CODE">' + forprojectdata[i].accoCode + '</span> <span class="ACCO_NAME">' + forprojectdata[i].accoName + '</span></p><span class="fu">辅</span></li>'
				}
			}else{
				if(forprojectdata[i].levNum > 0) {
					forprojectdatatr += '<li assCode='+forprojectdata[i].assCode+' allowSurplus='+forprojectdata[i].allowSurplus+' levNum="' + forprojectdata[i].levNum + '" fname="' + forprojectdata[i].accoFullName + '" title="' + forprojectdata[i].remark + '" acca="' + forprojectdata[i].accaCode + '" name="' + page.Codenameacct[forprojectdata[i].accoCode] + '" cashflow="' + forprojectdata[i].isCashflow + '" role="' + forprojectdata[i].accBal + '" class="unselected leave' + forprojectdata[i].levNum + ' clik' + forprojectdata[i].isLeaf + ' fuhs' + forprojectdata[i].accitemNum + '"><p><span class="ACCO_CODE">' + forprojectdata[i].accoCode + '</span> <span class="ACCO_NAME">' + forprojectdata[i].accoName + '</span></p><span class="fu">辅</span></li>'
				} else {
					forprojectdatatr += '<li assCode='+forprojectdata[i].assCode+' allowSurplus='+forprojectdata[i].allowSurplus+' levNum="' + forprojectdata[i].levNum + '" fname="' + forprojectdata[i].accoFullName + '" title="' + forprojectdata[i].remark + '" acca="' + forprojectdata[i].accaCode + '" name="' + page.Codenameacct[forprojectdata[i].accoCode] + '" cashflow="' + forprojectdata[i].isCashflow + '"role="' + forprojectdata[i].accBal + '" class="PopListBoxItem unselected leave' + forprojectdata[i].levNum + ' clik' + forprojectdata[i].isLeaf + ' fuhs' + forprojectdata[i].accitemNum + '"><p class="sq"><span class="ACCO_CODE">' + forprojectdata[i].accoCode + '</span> <span class="ACCO_NAME">' + forprojectdata[i].accoName + '</span></p><span class="fu">辅</span></li>'
				}
			}
		}
	}
	if(forprojectdatatr ==''){
		if(forprojectacca == 1){
			forprojectdatatr+=acctacca1()
		}else{
			forprojectdatatr+=acctacca2()
		}
	}else{
		forprojectdatatr += '<li class="bukedian"><p>没有对应的会计科目</p></li>'
		forprojectdatatr += '<li class="xzkjkm"><a >+新增会计科目</a></li>'
	}
	return forprojectdatatr
}
//单独加载财务会计科目
function acctacca1() {
	var tr = ''
...
function chaclickfu(ths, strdata, accoSurplus, dfDc) {
	...
	if(vousinglepzzy) {
		if(isalonebill && quanjuvoudatas.data[ss].showBill == 1){
			voucheryc += '<div class="voucher-yc-title"><span class="titlename maxwtitle">科目：</span>'
		}else{
		...
		voucheryc += '</div>&nbsp;&nbsp;'
		if(isalonebill && quanjuvoudatas.data[ss].showBill == 1){
			voucheryc+='<span>票据号：</span>'
			voucheryc += '<input type="text" class="ycbodyinp billNoinp"  value=""/>'
			voucheryc+='<span>票据类型：</span>'
			voucheryc += '<input type="text" relation="billTypeinp" class="ycbodyinp billTypeinp"  />'
			voucheryc+='<span>票据日期：</span>'
			voucheryc += '<input type="text" value="'+$("#dates").getObj().getValue()+'" class="ycbodyinp billDateinp"  />'
		}
		voucheryc += '</div>'
	}else if(isalonebill && quanjuvoudatas.data[ss].showBill == 1){
		voucheryc+= '<div class="voucher-yc-title">'
		voucheryc+='<span>票据号：</span>'
	...
	voucheryc += '<div class="voucher-yc-scroll">'
	...
	voucherycss = resOrderAccItem(nowAccoCode, accItemArr,ths.parents('.voucher-center'));
	var noshowaccitem = showAccItem(nowAccoCode, accItemArr)
	//					}
	if(isAssRemark){
		voucheryc += '<div class="ychead"  name="remark">摘要</div>'
	}
	// if(isprojectByVou && ths.attr('accos')!=undefined){
	// 	var accosdata = JSON.parse(ths.attr('accos'))
	// 	var quotaCodedata = []
	// 	var quotaCodedatas = fzhsxl['quotaCode']
	// 	for(var i=0;i<accosdata.quotas.length;i++){
	// 		if(quotaCodeindex[accosdata.quotas[i].quotaCode]!=undefined){
	// 			quotaCodedata.push(quotaCodedatas[quotaCodeindex[accosdata.quotas[i].quotaCode]])
	// 		}
	// 	}
	// 	voucherycss.quotaCode.valuerange =quotaCodedata
	// 	for(var  k in voucherycss){
	// 		if(accosdata[k]!=undefined){
	// 			voucherycss[k].value = accosdata[k]
	// 		}
	// 	}
	// }
	for(var k in voucherycss) {
				...
				if(curCodefzhsxl[n].eleRateList.length>0){
					if(curexratefzhsxl!=undefined){
						...
						for (var j = 0; j < curCodefzhsxl[n].eleRateList.length; j++) {
							...
							var nT = new Date(curCodefzhsxl[n].eleRateList[j].curDate).getTime()
							var vT = new Date($("#dates").getObj().getValue()).getTime()
							if (nM == vM && nY == vY) {
								nows = j
							}
							if(nT>allnow && nT<vT){
	...
	if(quanjuvoudatas.data[ss].field2 == 1) {
		if (contractfzhsxl.length == 0) {
			ufma.ajaxDef("/ma/sys/userData/selectContracts?rgCode="+ufgovkey.svRgCode+"&setYear="+rpt.foryear+"&agencyCode=" + rpt.nowAgencyCode, "get", '', function(data){
				if(data.data!=undefined){
					contractfzhsxl = data.data
				}else{
					ufma.showTip("合同号未加载成功", function () { }, "error")
				}
			});
		}
		var assnoll = ''
		assnoll += '<ul class="ycbodys field2">'
		for (var n = 0; n < contractfzhsxl.length; n++) {
			if (contractfzhsxl[n].enabled == 1) {
				if (contractfzhsxl[n].levelNum == 1) {
					assnoll += '<li itemid="'+ contractfzhsxl[n].id+'" department = "' + contractfzhsxl[n].departmentCode + '" levels = "' + contractfzhsxl[n].levelNum + '" fname="' + contractfzhsxl[n].chrFullname + '" class="PopListBoxItem unselected  dianji' + contractfzhsxl[n].isLeaf + ' fzlv' + contractfzhsxl[n].levelNum + ' "><p class="sq"><span class="code">' + contractfzhsxl[n].code + '</span> <span class="name">' + contractfzhsxl[n].name + '</span></p></li>'
				} else {
					assnoll += '<li itemid="'+ contractfzhsxl[n].id+'" department = "' + contractfzhsxl[n].departmentCode + '" style="display:none;" levels = "' + contractfzhsxl[n].levelNum + '" fname="' + contractfzhsxl[n].chrFullname + '" class="unselected  dianji' + contractfzhsxl[n].isLeaf + ' fzlv' + contractfzhsxl[n].levelNum + ' "><p><span class="code">' + contractfzhsxl[n].code + '</span> <span class="name">' + contractfzhsxl[n].name + '</span></p></li>'
				}
			} else {
				assnoll += '<li itemid="'+ contractfzhsxl[n].id+'" levels = "' + contractfzhsxl[n].levelNum + '" fname="' + contractfzhsxl[n].chrFullname + '" class=" unselected allnoshow  dianji' + contractfzhsxl[n].isLeaf + ' fzlv' + contractfzhsxl[n].levelNum + ' "><p><span class="code">' + contractfzhsxl[n].code + '</span> <span class="name">' + contractfzhsxl[n].name + '</span></p></li>'
			}
		}
		assnoll += '</ul>'
		$('#AssDataAll').append(assnoll)
		voucheryc += '<div class="ychead" name="field2">合同号</div>'
	}
	...
	if(strdata != null && strdata.length > 0) {
		for(var k = 0; k < strdata.length; k++) {
			...
			if(isAssRemark){
				...
				voucheryc += '<textarea class="ycbodyinp remark"></textarea>'
				voucheryc += '</div>'
			}
			for(var l in voucherycss) {
				if(isuniversities == true && l == 'projectCode'){
					voucheryc += '<div  class="ycbody">'
					voucheryc += '<textarea relation = "' + l + '" class="ycbodyinp projectCode" readonly = "true"  ></textarea>'
					...
					voucheryc += '<div  class="ycbody">'
					voucheryc += '<textarea relation = "' + l + '" class="ycbodyinp quotaCode" readonly = "true"  ></textarea>'
			...
			if(quanjuvoudatas.data[ss].field1 ==  '1'){
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<textarea class="ycbodyinp field1" readonly = "true"  ></textarea>'
			...
			if(quanjuvoudatas.data[ss].expireDate == 1) {
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<input type="text" value="'+$("#dates").getObj().getValue()+'" class="ycbodyinp daoqiri"  />'
				voucheryc += '</div>'
			}
			if(quanjuvoudatas.data[ss].showBill == 1 && !isalonebill) {
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<textarea class="ycbodyinp billNoinp"  ></textarea>'
				voucheryc += '</div>'
				voucheryc += '<div  class="ycbody">'
			...
			if(quanjuvoudatas.data[ss].currency == 1) {
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<textarea relation = "currency" type="text" class="ycbodyinp curcodeinp"  ></textarea>'
				voucheryc += '</div>'
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<textarea class="ycbodyinp exrateinp"  ></textarea>'
				voucheryc += '</div>'
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<textarea class="ycbodyinp curramtinp"  ></textarea>'
				voucheryc += '</div>'
			}
			if(quanjuvoudatas.data[ss].qty == 1) {
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<textarea class="ycbodyinp priceinp"  ></textarea>'
				voucheryc += '</div>'
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<textarea class="ycbodyinp qtyinp"  qty="' + quanjuvoudatas.data[ss].qtyDigits + '" ></textarea>'
				voucheryc += '</div>'
			}
			if(quanjuvoudatas.data[ss].field2 == 1) {
				voucheryc += '<div  class="ycbody">'
				voucheryc += '<textarea relation = "field2" class="ycbodyinp field2inp"></textarea>'
				voucheryc += '<span class="fielddetail">详情</span>'
				voucheryc += '</div>'
			}
			voucheryc += '<input type="text" class="ycbody yctz" />'
			voucheryc += '<span class="Assaccbala">余额</span>'
			voucheryc += '</div>'
		}
	} else {
		for(var k = 0; k < 1; k++) {
	...
	if(strdata != null) {
		...
		if(sdvdvdass.length>0){
			apvfvc.find('.voucher-yc-title').find('.billNoinp').val(sdvdvdass[0].billNo)
			apvfvc.find('.voucher-yc-title').find('.billDateinp').val(sdvdvdass[0].billDate)
			var $li = $('#AssDataAll').find(".billTypeinp").find("li");
			for(var s = 0; s < $li.length; s++) {
				if($li.eq(s).find(".code").text() == sdvdvdass[0].billType) {
					...
					apvfvc.find('.voucher-yc-title').find('.billTypeinp').val(fzxlnrcode + " " + fzxlnrname)
				}
			}
		}
		for(var k = 0; k < sdvdvdass.length; k++) {
			if(sdvdvdass[k].op == "2") {
				...
				for(var r = 0; r < vifvcyb.eq(k).find(".ycbodyinp").length; r++) {
					...
					}else if(headName == "field2") {
						if(!$.isNull(sdvdvdass[k].field2)){
							var ss = sdvdvdass[k].field2.indexOf('|')
							var code = sdvdvdass[k].field2.slice( 0,ss);
							var itemid= sdvdvdass[k].field2.slice(ss+1)
							for(var s = 0; s < $li.length; s++) {
	...
	if(sdvdvdass.length > 8) {
		ths.parents(".voucher-center").find(".voucher-yc").find('.voucher-yc-scroll').addClass("bgheight");
		voucherYcAssCssbgheight(ths.parents(".voucher-center").find(".voucher-yc"))
	}
}
//给单独一行分录赋值
	...
	if(isprojectByVou){
		for(var i=0;i<$(".voucher-center").length;i++){
			if($(".voucher-center").eq(i).find('.accounting').attr('fudata')!=undefined){
				var codeassdata = JSON.parse($(".voucher-center").eq(i).find('.accounting').attr('fudata'))
				if(!$.isNull(codeassdata[0].projectCode) && fzhsxl['projectCode'][projectCodeindex[codeassdata[0].projectCode]]!=undefined){
					var code = fzhsxl['projectCode'][projectCodeindex[codeassdata[0].projectCode]].code
					var name = fzhsxl['projectCode'][projectCodeindex[codeassdata[0].projectCode]].name
					$(".voucher-center").eq(i).find('.projectuniverinp').attr('code',code).attr('name',name).val(code+' '+name)
				}
			}
		}
	}
		...
		if(selectdata.data.vouDetails.length > 0) {
			...
			for(var i = 0; i < selectdata.data.vouDetails.length; i++) {
				if(selectdata.data.vouDetails[i].accaCode == 1) {
					...
					if(selectdata.data.vouDetails[i].vouSeq>cwseq){
						cwseq = selectdata.data.vouDetails[i].vouSeq
					}
					cwseqobj[selectdata.data.vouDetails[i].vouSeq] = selectdata.data.vouDetails[i]
				} else if(selectdata.data.vouDetails[i].accaCode == 2) {
					ysarrvou.push(selectdata.data.vouDetails[i])
					if(selectdata.data.vouDetails[i].vouSeq>ysseq){
						ysseq = selectdata.data.vouDetails[i].vouSeq
					}
					ysseqobj[selectdata.data.vouDetails[i].vouSeq] = selectdata.data.vouDetails[i]
				}
			}
			var allrunvous = []
			if(cwseq >= ysseq) {
				for(var i = 1; i < cwseq+1; i++) {
					if(cwseqobj[i] != undefined) {
						allrunvous.push(cwseqobj[i])
					} else {
						if(ysseqobj[i]!=undefined){
							allrunvous.push({
								'op': '3',
								descpt:ysseqobj[i].descpt
							})
						}else{
							allrunvous.push({
								'op': '3'
...
//
function chapzsummary(){
	var detail = selectdata.data.vouDetails
	var newdetail = []
	for(var i=0;i<detail.length;i++){
		if(detail[i].vouDetailAsss.length>1){
			detail[i].vouGuid=""
			detail[i].detailGuid=""
			var nowAss = JSON.parse(JSON.stringify(detail[i].vouDetailAsss))
			for(var z=0;z<nowAss.length;z++){
				nowAss[i].detailAssGuid=""
				nowAss[i].detailGuid=""
				nowAss[i].vouGuid=""
				detail[i].stadAmt = nowAss[z].stadAmt
				detail[i].vouDetailAsss = [nowAss[z]]
				newdetail.push(JSON.parse(JSON.stringify(detail[i])))
			}
		}else{
			newdetail.push(JSON.parse(JSON.stringify(detail[i])))
		}
	}
	selectdata.data.vouDetails = newdetail
...
function chapz() {
		...
		if(getUrlParam("action") == "preview" && getUrlParam("preview") == "1") {} else {
			if(rpt.nowAgencyCode.substring(0,3) == 999){
				...
				ufma.ajaxDef('/gl/vou/seleteBillNoAndBxNoListByVouGuid/' + selectdata.data.vouGuid+'/'+ufgovkey.svUserCode, 'get', '', function(result) {
					if(result.data.gwzjBillNo!='' && result.data.gwzjBillNo!=undefined){
						$(".bxd").show()
						...
						$(".bxd").show()
						var bxdp = ''
						for(var i=0;i<result.data.billNo.length;i++){
							bxdp += '<a pdf="'+result.data.PDF+'" title="'+result.data.billNo[i]+'" titles="'+result.data.valueField[i]+'">'+result.data.billNo[i]+'</a>'
						}
						$(".bxd").html('报销单：' +bxdp)
					}
				})
	...
	$(".voucher-head").after(chapzTr()); 
	if(isprojectByVou){
		for(var i=0;i<$(".voucher-center").length;i++){
			if($(".voucher-center").eq(i).find('.accounting').attr('fudata')!=undefined){
				var codeassdata = JSON.parse($(".voucher-center").eq(i).find('.accounting').attr('fudata'))
				if(codeassdata.length>0 && !$.isNull(codeassdata[0].projectCode) && fzhsxl['projectCode'][projectCodeindex[codeassdata[0].projectCode]]!=undefined){
					var code = fzhsxl['projectCode'][projectCodeindex[codeassdata[0].projectCode]].code
					var name = fzhsxl['projectCode'][projectCodeindex[codeassdata[0].projectCode]].name
					$(".voucher-center").eq(i).find('.projectuniverinp').attr('code',code).attr('name',name).val(code+' '+name)
				}
	...
	if(vousinglepz) {
		...
		$(".voucherall").height(571 + 50 * cjlen)
		...
		voucherycshowheight()
		$(".voucherall").height(611 + 50 * cjlen)
		...
		if(vousinglepzzy && vousingelzysdob){}else{
			$(".voucherall").height(481 + 50 * cjlen)
		}
	}
		...
		for(var z = 0; z < _this.find(".voucher-yc").length; z++) {
			if(_this.find(".voucher-yc").eq(z).hasClass("deleteclass") != true) {
				...
				for(var j = 0; j < _this.find(".voucher-yc").eq(z).find(".voucher-yc-bo").length; j++) {
					...
					if(isprojectByVou){
						bodyss['projectCode']=_this.find('.projectuniverinp').attr('code')
					}
					var noshows = JSON.parse(_this.find(".voucher-yc").eq(z).find(".voucher-yc-bt").find('.noshowdata').attr('noshow'))
					for(var n in noshows) {
						bodyss[n] = noshows[n]
					}
					if(_this.find(".voucher-yc").eq(z).find('.voucher-yc-title').find('.ycbodyinp').length>0){
						bodyss['billNo'] = _this.find(".voucher-yc").eq(z).find('.voucher-yc-title').find('.billNoinp').val()
						bodyss['billDate'] = _this.find(".voucher-yc").eq(z).find('.voucher-yc-title').find('.billDateinp').val()
						bodyss['billType'] = _this.find(".voucher-yc").eq(z).find('.voucher-yc-title').find('.billTypeinp').val().substring(0, itemCodeName.indexOf(" "));
					}
					bodyss.op = "0";
...
function huoqu(statusname) {
	...
	if($(".voucher-head").attr("namess") == undefined) {
					...
					if(vousinglepzzy && $(".voucher-center").eq(i).hasClass('voucher-centerys') && $(".voucher-center").eq(i).find(".accountinginp").val() == "" && $(".voucher-center").eq(i).find(".moneyj").find(".money-xs").html() == "" && $(".voucher-center").eq(i).find(".moneyd").find(".money-xs").html() == "") {} else {
						...
						if($(".voucher-center").eq(i).find(".accounting").attr('fudata') != undefined) {
							...
							for(var z = 0; z < vouDetailAsss.length; z++) {
								...
								if(isAssRemark && vouDetailAsss[z].remark == ''){
									vouDetailAsss[z].remark = abstract
								}
							}
										...
										if($(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).find(".yctz").val() != "" && parseFloat($(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).find(".yctz").val())!=0) {
											...
											for(var k = 0; k < headLen; k++) {
												...
												if(dd == 'diffTermCode') {
													...
													var itemCodeName = $ycBt.find(".voucher-yc-bo").eq(j).find(".ycbody").eq(k).find(".ycbodyinp").val();
													if(itemCodeName == ''){
														bodyss[dd] = $(".voucher-center").eq(i).find(".abstractinp").val()
													}else{
														bodyss[dd] = itemCodeName
													}
												}  else if(dd == 'billNo' || dd == 'billDate' || dd == 'bussDate' || dd == 'field1') {
													var itemCodeName = $ycBt.find(".voucher-yc-bo").eq(j).find(".ycbody").eq(k).find(".ycbodyinp").val();
													bodyss[dd] = itemCodeName;
													if( dd == 'field1' && itemCodeName!=''){
														bodyss.cancelAssGuid =  $ycBt.find(".voucher-yc-bo").eq(j).find(".ycbody").eq(k).find(".ycbodyinp").attr('cGuid')
													...
													var itemCodeName = $ycBt.find(".voucher-yc-bo").eq(j).find(".ycbody").eq(k).find(".ycbodyinp").val();
													var itemCode = itemCodeName.substring(0, itemCodeName.indexOf(" "));
													var itemId = $ycBt.find(".voucher-yc-bo").eq(j).find(".ycbody").eq(k).find(".ycbodyinp").attr('itemid')
													bodyss[dd] = itemCode+'|'+itemId;
												} else {
													var itemCodeName = $ycBt.find(".voucher-yc-bo").eq(j).find(".ycbody").eq(k).find(".ycbodyinp").val();
													var itemCode = itemCodeName.substring(0, itemCodeName.indexOf(" "));
													bodyss[dd] = itemCode;
											...
											if(isprojectByVou && $(".voucher-center").eq(i).find('.projectuniverinp').length>0){
												bodyss['projectCode']=$(".voucher-center").eq(i).find('.projectuniverinp').attr('code')
											}
											var noshows = JSON.parse($(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bt").find('.noshowdata').attr('noshow'))
											for(var n in noshows) {
												bodyss[n] = noshows[n]
											}
											if($ycBt.find('.voucher-yc-title').find('.ycbodyinp').length>0){
												bodyss['billNo'] = $ycBt.find('.voucher-yc-title').find('.billNoinp').val()
												bodyss['billDate'] = $ycBt.find('.voucher-yc-title').find('.billDateinp').val()
												var itemCodeName = $ycBt.find('.voucher-yc-title').find('.billTypeinp').val()
												var itemCode = itemCodeName.substring(0, itemCodeName.indexOf(" "));
												bodyss['billType'] = itemCode
											}
											bodyss.op = "0";
						...
						if(isprojectByVou && vousinglepz == false && vousinglepzzy == true && $.isNull(statusname)){
							if($(".voucher-center").eq(i).hasClass('voucher-centercw')){
								datass.vouSeq = (i+2)/2
							}else if($(".voucher-center").eq(i).hasClass('voucher-centerys')){
								datass.vouSeq = (i+1)/2
							}
						}
						datasss.push(datass)
		...
		datasd.inputor = $(".voucherfo .vfzd").find("span").attr('code')
		datasd.inputorName = $(".voucherfo .vfzd").find("span").text()
		datasd.auditorName = $(".voucherfo .vfsh").find("span").text()
		datasd.posterName = $(".voucherfo .vfjz").find("span").text()
			...
			if($(".voucher-center").eq(i).attr("op") == 2 && $(".voucher-center").eq(i).attr("namess") != undefined) {
				...
				if($(".voucher-center").eq(i).find(".accountinginp").val() != "" || $(".voucher-center").eq(i).find(".abstractinp").val() != "" || $(".voucher-center").eq(i).find(".moneyj").find(".money-xs").html() != "" || $(".voucher-center").eq(i).find(".moneyd").find(".money-xs").html() != "") {
					if(vousinglepzzy && $(".voucher-center").eq(i).hasClass('voucher-centerys') && $(".voucher-center").eq(i).find(".accountinginp").val() == "" && $(".voucher-center").eq(i).find(".moneyj").find(".money-xs").html() == "" && $(".voucher-center").eq(i).find(".moneyd").find(".money-xs").html() == "") {
								...
								for(var j = 0; j < $(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").length; j++) {
									if($(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).attr("op") == 2 && $(".voucher-center").eq(i).find(".voucher-yc-bo").eq(j).attr("namess") != undefined) {
										...
										if($(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).find(".yctz").val() != "" && parseFloat($(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).find(".yctz").val())!=0) {
											...
											bodyss.detailGuid = $(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).attr("namesss");
											bodyss.detailAssGuid = $(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).attr("namessss");
											if(bodyss.vouGuid == 'undefined'){
												delete bodyss.vouGuid
											}
											if(bodyss.detailGuid == 'undefined'){
												delete bodyss.detailGuid
											}
											if(bodyss.detailAssGuid == 'undefined'){
												delete bodyss.detailAssGuid
											}
											if($(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).attr("refBillGuid")!=undefined){
												bodyss.refBillGuid = $(".voucher-center").eq(i).find(".voucher-yc").eq(z).find(".voucher-yc-bo").eq(j).attr("refBillGuid")
											}
						...
						datass.vouGuid = $(".voucher-center").eq(i).attr("namess");
						datass.detailGuid = $(".voucher-center").eq(i).attr("namesss");
						if(datass.vouGuid == 'undefined'){
							delete datass.vouGuid
						}
						if(datass.detailGuid == 'undefined'){
							delete datass.detailGuid
						}
						if($(".voucher-center").eq(i).attr("refBillGuid")!=undefined && $(".voucher-center").eq(i).attr("refBillGuid")!='undefined'){
							datass.refBillGuid = $(".voucher-center").eq(i).attr("refBillGuid");
						}
	...
	if(selectdata.data.templateGuid == undefined || selectdata.data.templateGuid == '') {
		if(selectdata.data.errFlag == 0 || selectdata.data.vouStatus == "C") {
			...
			if(selectdata.data.vouStatus == "O") {
				$("#pzzhuantai").show();
				$("#pzzhuantai").css("background", "#e5f8f2");
				$("#pzzhuantai").css("color", "#00C285");
				...
				$("#pzzhuantai").show();
				$("#pzzhuantai").css("background", "rgb(249, 226, 224)");
				$("#pzzhuantai").css("color", "rgb(238, 64, 51)");
			...
			$("#pzzhuantai").show();
			$("#pzzhuantai").css("background", "rgb(255, 245, 219)");
			$("#pzzhuantai").css("color", "rgb(255, 192, 23)");
			$("#pzzhuantai").text("未改错");
			vouweigaicuo()
		}
	} else {
...
function fixedabsulate() {
	...
	} else if(vousinglepzzy == true && vousinglepz == false && windowWidth > 1700 == false) {
		fixedheight = $(".voucherall").height() - 85
	} else if(vousinglepzzy == true && vousinglepz == false && windowWidth > 1700 == true) {
		fixedheight = $(".voucherall").height() - 135
	} else {
		fixedheight = $(".voucherall").height()
	}
	windowWidth = $(window).width();
	if(vousinglepzzy == true && vousinglepz == false && windowWidth > 1700 && isdobabstractinp){
		var sctop = $('.voucher-footer').offset().top+65
		var scleft = $('.voucher-footer').offset().left - $(window).scrollLeft()+$('.voucher-footer').width()-$(".voucherfozy").width()-16
		$(".voucherfozy").show().css({
			'position':'absolute',
			'left':scleft+'px',
			'top':sctop+'px',
		})
		$(".voucherfo").hide()
	}else if(vousinglepzzy == true && vousinglepz == false && windowWidth < 1700 && isdobabstractinp){
		var sctop = $('.voucher-footer').offset().top+65
		var scleft = $('.voucher-footer').offset().left - $(window).scrollLeft()+$('.voucher-footer').width()-$(".voucherfozy").width()-16
		$(".voucherfozy").show().css({
			'position':'absolute',
	...
	if($(document).scrollTop() + document.documentElement.clientHeight +80 > $(".voucherall").height()) {
		$(".voucherbtnModal").css({
			"position": "relative",
			"top": "2px",
			"bottom": "auto",
	...
	$(".voucherfo").css({"margin-left":-$(".voucher").width() / 2,"width":$(".voucher").width()});
}
//双凭证用以切换财务凭证还是预算凭证
function cwyspd() {
	if(selectdata.data != undefined) {
...
$(document).on('click', '#btn-Relationsave', function() {
	...
	var thisindex = $("#vouaccRelationtable").attr('remind') - 2
...
$(document).on('click','#voufzhsRelationtable tbody tr',function(){
	var radiotr = $(this).find('input[type="radio"]').eq(0).prop("checked", true).attr('checked',true)
});
