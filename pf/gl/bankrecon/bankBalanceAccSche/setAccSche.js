$(function () {
	...
	var page = function () {
		...
		var portList = {
			...
			operatorList: "/gl/enumerate/GL_BANK_OPERATOR", //括号运算符枚举
			acctList: "/gl/eleCoacc/getRptAccts", //账套列表接口
			accoTree: "/gl/sys/coaAcc/getRptAccoTree", //科目树接口
			accItemList: "/gl/bank/recon/getAccitemByAcco", //辅助项列表
			accItemTree: "/gl/common/glAccItemData/getAccItemTree", //辅助项内容树
			elementList: "/gl/bank/recon/getAccitem", //要素列表
			saveAccSche: "/gl/bank/recon/saveReconSche", //保存对账方案
			getOneAccSche: "/gl/bank/recon/getBankRecon", //请求单个对账方案的内容
			deleteCondition: "/gl/bank/recon/deleteCondition" //删除数据范围(暂时没用)
		};
		...
		return {
				...
				ufma.ajaxDef(portList.operatorList, "get", "", function (result) {
					...
					for (var i = 0; i < data.length; i++) {
						if (data[i].ENU_CODE == "01") {
							...
							var obj3 = {};
							obj3.operCode3 = data[i].ENU_CODE;
							if (data[i].ENU_CODE == "03") {
								obj3.operName3 = "并且";
							} else if (data[i].ENU_CODE == "04") {
								obj3.operName3 = "或者";
							}
							page.OperatorList3.push(obj3);
						}
					}
				});
			},
			...
			reqAcctList: function () {
				...
				ufma.ajaxDef(portList.acctList, "get", argu, function (result) {
					var data = result.data;
					var newData = [];
					for (var i = 0; i < data.length; i++) {
						var obj = {};
			...
			//获取辅助项列表
			reqAccItemList: function (acctCode, accoCode) {
				var argu = {
					"agencyCode": page.agencyCode,
					"acctCode": acctCode,
					"setYear": page.setYear,
			...
			//获取辅助项内容树
			reqAccItemTree: function (acctCode, accoCode, accItemCode) {
				var argu = {
					"agencyCode": page.agencyCode,
					"acctCode": acctCode,
					"setYear": page.setYear,
				...
				$('#' + id).ufDatagrid({
					...
					columns: [
						...
						},
						{
							...
							textField: 'acctName',
							data: page.acctList,
							onChange: page.selectAcct,
							render: function (rowid, rowdata, data) {
								return $.isNull(rowdata.acctName) ? '' : '<span title="' + rowdata.acctName + '">' + rowdata.acctName + '</span>';
							}
						},
						{
							type: 'treecombox',
							...
							render: function (rowid, rowdata, data) {
								return $.isNull(rowdata.accoCodeName) ? '' : '<span title="' + rowdata.accoCodeName + '">' + rowdata.accoCodeName + '</span>';
							}
						},
						{
							type: 'combox',
							...
							render: function (rowid, rowdata, data) {
								return $.isNull(rowdata.eleName) ? '' : '<span title="' + rowdata.eleName + '">' + rowdata.eleName + '</span>';
							}
						},
						{
							type: 'treecombox',
							...
							beforeExpand: function (e) { //下拉框初始化
								...
								if (acctCode != "" && acctCode != null && accoCode != "" && accoCode != null && accItemCode != "" && accItemCode != null) {
									//获取辅助项内容树
									page.reqAccItemTree(acctCode, accoCode, accItemCode);
							...
							render: function (rowid, rowdata, data) {
								return $.isNull(rowdata.accContCodeName) ? '' : '<span title="' + rowdata.accContCodeName + '">' + rowdata.accContCodeName + '</span>';
							}
						},
						{
							type: 'combox',
			...
			selectAcco: function (e) {
				...
				var acctCode = $trEdit.find('[name="acctCode"].uf-combox').getObj().getValue();
				var accoCode = $trEdit.find('[name="accoCode"].uf-treecombox').getObj().getValue();
				if (acctCode != "" && acctCode != null && accoCode != "" && accoCode != null) {
					var $eleObj = $trEdit.find('[name="eleCode"].uf-combox').getObj();
					page.reqAccItemList(acctCode, accoCode);
			...
			selectAccItem: function (e) {
				...
				//获取辅助项内容树
				page.reqAccItemTree(acctCode, accoCode, eleCode);
			...
			saveAccSche: function (flag) {
				...
				if (aRangeData.length > 0 && !$.isNull(aRangeData[0])) {
					...
					for (var i = 0; i < aRangeData.length; i++) {
						var acctName = aRangeData[i].acctName;
						var accoName = aRangeData[i].accoCodeName;
						if (acctName == "" || acctName == null || accoName == "" || accoName == null) {
							ufma.showTip("第" + (i + 1) + "行数据范围的账套或者科目不能为空！", function () { }, "warning");
							return false;
						}
				...
				scheObj.glBankJournalConditions = condArr;
				scheObj.glBankExtends = allExtArr;
				scheObj.glBankJournalOpts = journalOpts;
				/*{
					fieldObj,
					glBankJournalConditions: condArr,
					glBankExtends: allExtArr,
					glBankReconSche: fieldObj,
					glBankJournalOpts: journalOpts,
				}*/
			...
			init: function () {
				...
				if (page.action == "edit") {
					...
					ufma.ajaxDef(portList.getOneAccSche, "get", argu, function (result) {
						...
						//方案名称等字段
						//						var formData = scheData.glBankReconSche;
						...
						var checkArr = scheData.glBankJournalOpts;
						...
						//扩展列
						var allColArr = scheData.glBankExtends;
						...
						//数据范围
						page.condEditArr = scheData.glBankJournalConditions;
				...
				for (var i = 0; i < tempData.length; i++) {
					...
					var acctFlag = true;
					if (tempData[i].acctCode != "" && tempData[i].acctCode != null) {
						acctFlag = true;
						obj.acctCode = tempData[i].acctCode;
						var acctObj = $.inArrayJson(page.acctList, 'acctCode', tempData[i].acctCode);
						if (!$.isNull(acctObj)) {
							obj.acctName = acctObj.acctName;
						}
					} else {
						obj.acctCode = null;
						obj.acctName = null;
					...
					if (acctFlag && accoFlag && tempData[i].accoAccItem != "" && tempData[i].accoAccItem != null) {
						...
						page.reqAccItemList(tempData[i].acctCode, tempData[i].accoCode);
					...
					if (acctFlag && accoFlag && eleFlag && tempData[i].chrCode != "" && tempData[i].chrCode != null) {
						...
						page.reqAccItemTree(tempData[i].acctCode, tempData[i].accoCode, tempData[i].accoAccItem);
