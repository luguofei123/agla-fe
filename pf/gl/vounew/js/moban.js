		...
		ondestory: function(result) {
			...
			if(result.action == 'ok') {
				...
				if(!$.isNull(vouTemplateGuid)) {
					...
					$.ajax({
						type: "get",
						url: "/gl/vouTemp/getTempPairFromVou/" + vouTemplateGuid + "/" + rpt.nowAgencyCode + "/" + rpt.nowAcctCode +  "/" + ufgovkey.svRgCode + "/" + ufgovkey.svSetYear +"?ajax=1&rueicode="+hex_md5svUserCode,
						async: false,
						success: function(data) {
							if(data.flag == "success") {
...
$(document).on("click", "#mbsrcom", function(e) {
	...
	ufma.open({
		...
		ondestory: function(result, voutempSendList) {
			searchmobandata = ''
			if(result.action == 'ok') {
				vouTemplateGuid = result.voutempSendList;
				if(!$.isNull(vouTemplateGuid)) {
...
$(document).on("click", ".mbtabbody", function() {
	if($(this).find(".mbtabbody-lixin").attr("name") != undefined) {
		...
		//		if(mobanneirong.data.cwVouTempVo != null) {
		//			quanjuvouchaiwu = mobanneirong.data.cwVouTempVo
		//		} else {
		//			$.ajax({
		//				type: "get",
		//				url: "/gl/vou/getVouByAcca/" + vouGroupId + "/1",
		//				async: false,
		//				success: function(data) {
		//					if(data.flag == "success") {
		//						quanjuvouchaiwu = data.data
		//					}
		//				}
		//			});
		//		}
		//		if(mobanneirong.data.ysVouTempVo != null) {
		//			quanjuvouyusuan = mobanneirong.data.ysVouTempVo
		//		} else {
		//			$.ajax({
		//				type: "get",
		//				url: "/gl/vou/getVouByAcca/" + vouGroupId + "/2",
		//				async: false,
		//				success: function(data) {
		//					if(data.flag == "success") {
		//						quanjuvouyusuan = data.data
		//					}
		//				}
		//			});
		//		}
		...
		//			if($(".xuanzhongcy").hasClass("chaiwu")) {
		//				selectdata.data = mobanneirong.data.cwVouTempVo
		//			} else {
		//				selectdata.data = mobanneirong.data.ysVouTempVo
		//			}
