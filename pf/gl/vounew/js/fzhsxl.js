...
//	var index = $('.assCheck').eq(0).parent(".ycbody").prevAll(".ycbody").length-1;
	...
	for(var i = 0; i < allelecode.length; i++) {
		if(allelecode.eq(i).attr('name') != undefined) {
			...
			if(nowele.eleCode != "BILLNO" && nowele.eleCode != "BILLTYP" && nowele.eleCode != "BILLDAT") {
				if(nowelecodes != "billNo" && nowelecodes != "billType" && nowelecodes != "billDate") {
	...
	if(nameCode == "billNo" || nameCode == "billType" || nameCode == "billDate") {
		toAjax = false
	}
	if(toAjax) {
		var argu = {
...
function selectItemssmodal(data, titlehead, thisobj) {
	...
	page.editor = ufma.showModal('voufzhsRelation', 750, 350);
}
...
$(document).on("blur", ".ycbodyinp", function() {
	var _this = $(this);
	if($(this).hasClass("daoqiri") != true && $(this).hasClass("bussDate") != true && $(this).hasClass("billNoinp") != true && $(this).hasClass("billDateinp") != true && $(this).hasClass("daoqiriend") != true) {
		if($(this).hasClass("priceinp") != true && $(this).hasClass("qtyinp") != true && $(this).hasClass("exrateinp") != true && $(this).hasClass("curramtinp") != true) {
			setTimeout(function() {
...
$(document).on("click", ".ycbodys .dianji0", function(e) {
	...
	setTimeout(function() {
		...
		if(_this.parents('.ycbodys').attr('isAlllength')=='now'){
...
$(document).on("click", ".ycbodys .dianji1", function(e) {
	...
	if($(this).parents(".ycbodys").hasClass('employeeCode')) {
	...
	if($(this).parents(".ycbodys").hasClass('departmentCode')) {
...
$(document).on("keydown", ".daoqiri,.billNoinp,.billDateinp,.priceinp,.qtyinp,.exrateinp,.curramtinp,.bussDate,.billTypeinp,.curcodeinp", function(event) {
	event = event || window.event;
	if(event.keyCode == 13) {
		$(".ycbodys").hide();
		if($(this).parents(".ycbody").next(".ycbody").find(".ycbodyinp").length == 1) {
