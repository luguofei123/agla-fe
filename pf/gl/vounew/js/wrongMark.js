...
$(document).on("click", "#btn-voucher-biaocuo", function(e) {	
	if(selectdata.data.inputor == ufgovkey.svUserId) {
		if(!isauditbiaocuo){
...
$(document).on("click", ".biaocuo .bcbc", function() {
...
$(document).on("click", ".biaocuo .quxiao", function() {
	$(".voucherall").show();
	$("#pingzengjiucuo").css("background", "none");
	$("#pingzengjiucuo").hide();
	$("#pingzengjiucuo").html("");
...
$(document).on("click", ".leftbiaocuo", function(e) {
	...
	$.ajax({
		...
		success: function(data) {
			if(data.flag == "success") {
				if(parseFloat($("#xiaocuobj").attr("name")) + 1 < data.data.length) {
