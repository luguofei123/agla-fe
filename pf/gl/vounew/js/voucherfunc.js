...
var voucherindex = 0;
$(document).on("focus", ".voucher-center", function() {
	voucherindex = $(this).index();
})
...
function getLastDay() {
	var dt = new Date(Date.parse(dd.replace(/-/g, "/")));
	dt.setDate(1);
...
$(document).on("click", ".accountingmx", function(e) {
	...
	page.openNewPages(page.isCrossDomain,$(this), 'openMenu', baseUrl, false, '明细账');
})
...
//	page.editor = ufma.showModal('vouAccoBal', 750, 350);
    ...
    ufma.ajaxDef("/gl/vou/getAccoBal/" + rpt.nowAgencyCode +"/"+rpt.nowAcctCode, "post",{'Acco':acco},function(data) {
...
$(document).on("input", ".nowu", function() {
	if($(".voucher-head").attr("namess") != undefined) {
		voubiaoji();
	}
})
...
$(document).on("focus", ".money-jd", function() {
	var editStatus0 = ($("#pzzhuantai").attr("vou-status") == undefined);
	if(!editStatus0) {
		var editStatus1 = ($("#pzzhuantai").attr("vou-status") == "O");
		var editStatus2 = (isInputor == true && (selectdata.data.inputor == ufgovkey.svUserId || selectdata.data.inputor == undefined))
...
$(document).on("focus", "#dates,#leftbgselect", function() {
	if($(".voucher-head").attr("namess") != undefined) {
		if(selectdata.data.vouStatus == "O") {
			$("#pzzhuantai").hide();
			voubiaoji();
...
$(document).on("click", ".icondate,.ycadd,.ycdelect", function() {
	if($(".voucher-head").attr("namess") != undefined) {
		if(selectdata.data.vouStatus == "O") {
			$("#pzzhuantai").hide();
			voubiaoji();
...
$(document).on("click", "#fjd", function() {
	if($(".voucher-head").attr("namess") != undefined) {
		if(selectdata.data.vouStatus == "O") {
			$("#pzzhuantai").hide();
			voubiaoji();
...
$(document).scroll(function() {
	...
	for(var i = 0; i < $(".accountinginp").length; i++) {
		var sctop = $(".accountinginp").eq(i).offset().top - $(window).scrollTop() - $(".accountinginp").eq(i).parents('.voucher-yc-bg').scrollTop() + 50
		var scleft = $(".accountinginp").eq(i).offset().left - $(window).scrollLeft()
		$(".accountinginp").eq(i).next(".accountinginps").css({
			'position': 'fixed',
			'top': sctop,
			'left': scleft
		});
	...
	for(var i = 0; i < $(".abstract").length; i++) {
		var sctop = $(".abstract").eq(i).offset().top - $(window).scrollTop() - $(".abstract").eq(i).parents('.voucher-yc-bg').scrollTop() + 50
		var scleft = $(".abstract").eq(i).offset().left - $(window).scrollLeft()
		$(".abstract").eq(i).find(".abstractinps").css({
			'position': 'fixed',
			'top': sctop,
			'left': scleft
		});;
...
$(document).on("click", ".bxd", function(e) {
	if(rpt.nowAgencyCode.substring(0,3) == 999){
		...
		var httpClientdata = {
			'url':"http://2k345b3304.imwork.net:46378/sys/interface/supportAccountPart/getBoePathForYonyou",
			'params':loaddata,
			'type':'POST'
		}
		var _this = $(this)
...
$(document).on("click", "#voudataRefresh", function(e) {
	...
	//------------end 
	//获取科目辅助项的顺序
	ufma.ajax("/gl/vou/selDesc/" + rpt.nowAgencyCode + "/" + rpt.nowAcctCode, "get",'', function(data) {
		zaiyao = data;
		var ssr = ''
		for(var i = 0; i < data.data.length; i++) {
...
$(document).on("click", ".accountingye", function(e) {
	...
	ufma.ajaxDef("/gl/vou/getAccoBal/" + rpt.nowAgencyCode +"/"+rpt.nowAcctCode+'/'+acco, "get",'',function(data) {
		...
		if(data[acco]!=undefined){
			var tr = '<tr><td>'+acco+' '+acconame+'</td><td>'+ data[acco]+'</td></tr>'
			$('#vouaccBaltab').find('tbody').html(tr)
		}else{
			ufma.showTip('当前科目余额为0',function(){},'warning')
		}
	});
	page.editor = ufma.showModal('vouaccBalRelation', 750, 350);
})
