		...
		return {
			...
			getPdfData: function (result) {
				...
				for(var k=0;k<result.data.data.length;k++){
					...
					if ($('#rptType option:selected').attr('valuecode') == 'GL_RPT_JOURNAL' || $('#rptType option:selected').attr('valuecode') == 'GL_RPT_BAL') {
						var j = 1;
						var ntitle = {}
						for (var z in ztitle) {
							ntitle['ext' + j + 'Name'] = ztitle[z]
							for (var i = 0; i < tblData.length; i++) {
								// tblData[i].outTableData = nowout
					...
					if ($('#rptType option:selected').attr('valuecode') != 'GL_RPT_BAL' && $('#rptType option:selected').attr('valuecode') != 'GL_RPT_JOURNAL') {
						datas = {
							"GL_RPT_PRINT": tblData,
							'GL_RPT_HEAD_EXT': [ntitle]
						}
			...
			getPdfDatayun: function (result) {
				...
				for(var k=0;k<result.data.data.length;k++){
					...
					if ($('#rptType option:selected').attr('valuecode') == 'GL_RPT_JOURNAL') {
						var j = 1;
						var ntitle = {}
						for (var z in ztitle) {
							ntitle['ext' + j + 'Name'] = ztitle[z]
					...
					if ($('#rptType option:selected').attr('valuecode') == 'GL_RPT_LEDGER') {
						var runNum = page.rowNum
						var outTableData = {}
						outTableData.agency= searchData.agencyCode+' '+searchData.agencyName
						outTableData.times = $("#startDate").getObj().getValue()+'至'+$("#endDate").getObj().getValue()
			...
			getPdf: function (result) {
				...
				// // xhr.open('POST', 'http://dev1.cwy.com/pqr/api/printpdfbydata', true)
				...
				var responseId = result.data.printData.responseId
			...
			getpdfcatalog:function(result){
				...
				ufma.ajaxDef('/gl/rpt/getCatalogGuid','post',{"id":catalogid},function(datas){
					var GL_RPT_CATALOG_HEAD =[{
						"titleType": '科目'+$("#rptType").find("option:selected").html()+'目录',
			...
			getExcel: function (result) {
				...
				if($('#rptType option:selected').attr('valuecode') == 'GL_RPT_JOURNAL'){
					rptTypeName = "明细账";
				}else if($('#rptType option:selected').attr('valuecode') == 'GL_RPT_LEDGER'){
					rptTypeName = "总账";
				}
				var argu = {
					...
					rptType:$('#rptType option:selected').attr('valuecode')
				}
			...
			getExcelData: function (result) {
				if (result.data.printData.length == 0) {
					// ufma.showTip('发生额与余额都为零，不需导出')
					return false;
				}
				for(var k=0;k<result.data.printData.length;k++){
					var tblData = result.data.printData[k].GL_RPT_PRINT
					var datanowqryItems = JSON.parse(JSON.stringify(prjContentsnow.qryItems))
					var ztitle = {}
					var baltitle = {}
					for (var i = 0; i < datanowqryItems.length; i++) {
			...
			initRptTemplate: function (result) {
				...
				for (var i = 0; i < data.length; i++) {
					if(mxdata[data[i].formattmplCode]==undefined && zzdata[data[i].formattmplCode]==undefined){
						var dataname = ''
			...
			initRptStyle: function (result) {
				...
				var postSetData = {
					...
					formatTmplCode: $('#rptStyle option:selected').attr("value")
				};
					...
					ufma.get('/gl/rpt/prj/getPrjcontent', searchsData, function (result) {
						if (!$.isNull(result.data)) {
							...
							var argu = {
								...
								prjCode:$('#rptTemplate').find("option:selected").attr('valueid'),
								prjName:$('#rptTemplate').find("option:selected").text(),
							...
							if ($('#rptTemplate').find("option:selected").attr('agencyCode') == '*') {
								var savePrjArgu = {}
								savePrjArgu.acctCode = "*"; //账套代码
								savePrjArgu.agencyCode = "*"; //单位代码
								savePrjArgu.prjCode = $('#rptTemplate').find("option:selected").attr('valueid'); //方案代码
								savePrjArgu.prjName = $('#rptTemplate').find("option:selected").text(); //方案名称
								savePrjArgu.prjScope = '3' //方案作用域
								savePrjArgu.maxPageQuantity = ''
			...
			showTable: function (result) {
				...
				var columnDefsArr = [{
					...
					"render": function (data, type, rowdata, meta) {
						return '<label class="mt-checkbox mt-checkbox-single mt-checkbox-outline"> ' +
							'<input type="checkbox" class="checkboxes" values="' + meta.row + '"/>&nbsp; ' +
							'<span></span> ' +
							'</label>';
					}
				}]
				...
				columnDefsArr.push({
					...
					"className": "col-check",
					"render": function (data, type, rowdata, meta) {
						return '<a class="btn btn-icon-only btn-sm btn-permission btn-print btn-print-preview"  values="' + meta.row + '" data-toggle="tooltip" action= "" title="打印">'
			...
			backTabArgu: function (dom) {
				...
				tabArgu.prjCode = $('#rptTemplate').find("option:selected").attr('valueid');//方案代码
				tabArgu.prjName = $('#rptTemplate').find("option:selected").text();//方案名称
				tabArgu.prjScope = "";//方案作用域

				tabArgu.rptType = $('#rptType option:selected').attr('valuecode');//账表类型
			...
			qryitemslists:function(dom,trss,isExcel){
				...
				var trssouttable = ''
				for (var z = 0; z < trss.find(".searchTxt").length; z++) {
					if (trss.find(".searchTxt").eq(z).attr('outtabledata') != undefined) {
						if(datanowqryItems[0].isShowCode!=0 || isExcel){
							trssouttable += trss.find(".searchTxt").eq(z).html() + ' '
						}else{
							trssouttable += trss.find(".searchTxt").eq(z).attr('outtabledata') + ' '
						}
					}
				}
				var srrs = []
			...
			prjContObj: function (dom) {
				...
				if ($('#rptType option:selected').attr('valuecode') == "GL_RPT_BAL" || $('#rptType option:selected').attr('valuecode') == "GL_RPT_LEDGER") {
				...
				} else if ($('#rptType option:selected').attr('valuecode') == "GL_RPT_JOURNAL") {
				...
				prjContentsnows.rptStyle = $('#rptStyle option:selected').attr('valuecode');//账表样式
				prjContentsnows.rptTitleName = $('#rptType option:selected').text();//账表中标题名称
			...
			onEventListener: function () {
				...
				$('#rptPrint #rptTemplate').on('change', function () {
					// var Guids = $('#rptTemplate').find("option:selected").attr('valueid')
					// page.getTableData(Guids)
				})
				//点击打印封面按钮
				...
				$('#rptPrint #btn-print-set').on('click', function () {
					...
					ufma.open({
						...
						ondestory: function (data) {
							//窗口关闭时回传的值
							destroyGuid = $('#rptTemplate').find("option:selected").attr('valueid')
							var postSetData = {
								agencyCode: searchData.agencyCode,
								acctCode: searchData.acctCode,
								componentId: "GL_RPT",
				...
				$('#btn-print-search').on('click', function () {
					// var Guids = $('#rptTemplate').find("option:selected").attr('valueid')
					// page.getTableData(Guids)
				});
				$(document).on('click', '.btn-print-preview', function () {
					...
					for (var z = 0; z < trss.find(".searchTxt").length; z++) {
						if (trss.find(".searchTxt").eq(z).attr('outtabledata') != undefined) {
							nowout += trss.find(".searchTxt").eq(z).attr('outtabledata') + ' '
						}
					}
					...
					ufma.post("/gl/rpt/getReportPrintBatchDataNew/" + $('#rptType option:selected').attr('valuecode'), backTabArgus, function(){
				...
				$(document).on('click', '#btn-print-pdfs,#btn-print-pdf', function () {
					...
					for (var i = 0; i < $("#rptPrint-data .checkboxes").length; i++) {
						if ($("#rptPrint-data .checkboxes").eq(i).is(":checked")) {
							var data = rptPrintDatanew[$("#rptPrint-data .checkboxes").eq(i).attr('values')]
				...
				$(document).on('click', '#btn-print-yun', function () {
					...
					if($("#rptStyle").find("option:selected").attr('value')=="mxShulSys"){
						postSetData.reportCode='GL_RPT_JOURNAL_SHULIANG'
					}else if($("#rptStyle").find("option:selected").attr('value')=="mxWbSys"){
						postSetData.reportCode='GL_RPT_JOURNAL_WAIBI'
					}else if($("#rptStyle").find("option:selected").attr('value')=="mxSlwbSys"){
						postSetData.reportCode='GL_RPT_JOURNAL_SLWB'
					}
					if($("#rptStyle").find("option:selected").attr('value')=="zzShulSys"){
					...
					setTimeout(function(){
						page.setget()
						ufma.ajaxDef("/gl/rpt/getReportPrintBatchDataNew/" + $('#rptType option:selected').attr('valuecode'), "post", backTabArgus, page.getPdfDatayun);
						ufma.hideloading();
						if(htmlprint == ''){
							ufma.showTip('发生额与余额都为零，不需打印')
							ufma.hideloading()
				...
				$(document).on('click', '#btn-print-catalog', function () {
					...
					// 		for (var z = 0; z < trss.find(".searchTxt").length; z++) {
					// 			if (trss.find(".searchTxt").eq(z).attr('outtabledata') != undefined) {
					// 				if(datanowqryItems[0].isShowCode!=0){
					// 					name += trss.find(".searchTxt").eq(z).html() + ' '
					// 				}else{
					// 					name += trss.find(".searchTxt").eq(z).attr('outtabledata') + ' '
					// 				}
					// 			}
					// 		}
				...
				$(document).on('click', '#btn-print-catalogAuto', function () {
					...
					for (var i = 0; i < $("#rptPrint-data .checkboxes").length; i++) {
						if ($("#rptPrint-data .checkboxes").eq(i).is(":checked")) {
							var data = rptPrintDatanew[$("#rptPrint-data .checkboxes").eq(i).attr('values')]
							var trss = $("#rptPrint-data .checkboxes").eq(i).parents('tr')
							var qryitemss= page.qryitemslists([data],trss,true)
							backTabArgus.prjContent.qryItemsList.push(qryitemss) 
							nowout = ''
							for (var z = 0; z < trss.find(".searchTxt").length; z++) {
								if (trss.find(".searchTxt").eq(z).attr('outtabledata') != undefined) {
							...
							// ufma.ajaxDef("/gl/rpt/getReportPrintBatchData/" + $('#rptType option:selected').attr('valuecode'), "post", backTabArgus, page.getPdfDatayun);
						}
					}
					if(backTabArgus.prjContent.qryItemsList.length==0){
						ufma.showTip('请选中至少一行后打印')
					...
					page.setget()
					ufma.ajaxDef("/gl/rpt/getReportPrintBatchDataNew/" + $('#rptType option:selected').attr('valuecode'), "post", backTabArgus, page.getpdfcatalog);
				})
