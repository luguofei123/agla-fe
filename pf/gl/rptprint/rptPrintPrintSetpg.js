        ...
        return {
            ...
            getPrtFormatList: function () {
                ...
                if (modelData.isSys) {
                    ...
                    // for(var i=0;i<ztreeDataAgc.length;i++){
                    //     ztreeDatanow.push(ztreeDataAgc[i])
                    // }
                } else {
                    for (var i = 0; i < ztreeDataSys.length; i++) {
                        ztreeDatanow.push(ztreeDataSys[i])
                ...
                ufma.post('/gl/vouPrint/getPrtTmplPdfNew', seacData, function (data) {
                    for (var i = 0; i < data.data.length; i++) {
                        if(mxdata[data.data[i].formattmplCode]==undefined && zzdata[data.data[i].formattmplCode]==undefined){
                            data.data[i].pId = data.data[i].formattmplCode
                            data.data[i].id = data.data[i].tmplGuid
                            data.data[i].isLeaf = '1'
                            if (data.data[i].agencyCode != '*' && modelData.isSys) { } else {
                ...
                function bzTreeOnClick(treeId, treeNode, clickFlag) {
                    ...
                    function trueclick(){
                        ...
                        if (treeNode.isLeaf == 1) {
                            for (var i = 0; i < ztreeDataSys.length; i++) {
                                if (ztreeDataSys[i].id == treeNode.pId) {
                                    page.checkTrees.fullName = ztreeDataSys[i].fullName
                                    break;
                                }
                            }
                            for (var i = 0; i < ztreeDataAgc.length; i++) {
                                if (ztreeDataAgc[i].id == treeNode.pId) {
                                    page.checkTrees.fullName = ztreeDataAgc[i].fullName
                                    break;
                                }
                            }
                            if(modelData.isSys){
                            ...
                            page.getModelData()
                            $("#fananName").val(treeNode.tmplName).attr("Guid", treeNode.id).removeAttr('formattmplCode')
            ...
            initPage: function () {
                ...
                $(document).on("click", '.btn-add', function (e) {
                    ...
                    if (key.isLeaf == '1') {
                        key.id = key.pId
                        for (var i = 0; i < ztreeDataSys.length; i++) {
                            if (ztreeDataSys[i].id == key.pId) {
                                page.checkTrees.fullName = ztreeDataSys[i].fullName
                                break;
                            }
                        }
                        for (var i = 0; i < ztreeDataAgc.length; i++) {
                            if (ztreeDataAgc[i].id == key.pId) {
                                page.checkTrees.fullName = ztreeDataAgc[i].fullName
                                break;
                            }
                        }
                ...
                $(document).on("click", '.btn-fanan', function (e) {
                    ...
                    var datasid = page.checkTrees.pId
                    datasid = datasid.substring(0, datasid.length - 3);
                    if (key.isLeaf == '1') {
                        key.id = key.pId
                        for (var i = 0; i < ztreeDataSys.length; i++) {
                            if (ztreeDataSys[i].id == key.pId) {
