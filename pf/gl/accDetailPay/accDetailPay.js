$(function() {
	var page = function() {
		...
		//用于存储表头信息
		var headArr;
		var pageLength = ufma.dtPageLength('#payTable'); //分页
		var vouTypeArray = {};
		var columnsArr = [{
				data: "rq",
				width: "40px",
		...
		return {
			setTable: function(liArr, tableData, colArrBase, type) {
				...
				var columnList = [];
				if(type == "SANLAN") {
					page.payTableThead.html('<tr>' + page.incomehtml1 + '</tr><tr>' + page.incomehtml2 + '</tr>');
				}
			...
			newTable: function(tableData, columnsArr, type) {
				var id = "payTable"; //表格id
				var toolBar = $('#' + id).attr('tool-bar');
				page.oTable = page.payTable.DataTable({
					"language": {
					...
					"initComplete": function(settings, json) {
						...
						$("#dtToolbar .buttons-excel").off().on('click', function(evt) {
							...
							//ufma.expXLSForDatatable($('#payTable'), '应付款明细账');
						});
						//导出end	
					...
					"drawCallback": function(settings) {
						...
						page.payTable.find("td.dataTables_empty").text("")
						...
						$(rpt.namespace).find("td span").on("click", function() {
							//rpt.openVouShow(this, "accDetailPay");
							var vouGuid = $(this).attr("data-vouguid"); //凭证id
							var vouMenuId = 'f24c3333-9799-439a-94c9-f0cdf120305d';
							if(vouGuid) {
								var baseUrl = '/pf/gl/vou/index.html?menuid=' + vouMenuId + '&dataFrom=accDetailPay&action=query&vouGuid=' + vouGuid + '&agencyCode=' + rpt.nowAgencyCode + '&acctCode=' + rpt.nowAcctCode;
								uf.openNewPage(page.isCrossDomain, $(this), 'openMenu', baseUrl, false, "凭证录入");
							}
							ufma.setBarPos($(window));
						});

						$('#payTable').closest('.dataTables_wrapper').ufScrollBar({
							hScrollbar: true,
							mousewheel: false
						});
						ufma.setBarPos($(window));
			...
			initPage: function() {
				...
				//需要根据自己页面写的ID修改
				page.payTable = $('#payTable'); //当前table的ID
				page.payTableThead = $('#payTableThead'); //当前table的头部ID
				//默认三栏式表格
				var tableData = [];
				page.payTableThead.html('<tr>' + page.incomehtml1 + '</tr><tr>' + page.incomehtml2 + '</tr>');
			...
			onEventListener: function() {
				...
				$('#sureSaveMethod,#saveAs').on('click', function(e) {
					...
					pageLength = ufma.dtPageLength('#payTable');
					ufma.setBarPos($(window));
				});
