...
rpt.createTags = function(treeId, type) {
	...
	var num = 0;
	var checkNodes;
	if(rpt.rptType == "accDetailIncome") {
...
rpt.reqAcctList = function() {
	...
	ufma.get(rpt.portList.acctList, acctArgu, function(result) {
		...
		if(data.length > 0) {
			...
			var code = data[0].code; //多区划
			var name = data[0].name; //多区划
			var codeName = data[0].codeName; //多区划
	...
	ufma.ajax(reqUrl, "get", "", function(result) {
		...
		for(var i = 0; i < data.length; i++) {
			var sHtml = ufma.htmFormat('<option value="<%=code%>"><%=name%></option>', {
				...
				//name: data[i].codeName//多区划
				name: data[i].name //bug75765--zsj--凭证号 不应显示编码
			});
			selectHtml += sHtml;
		}
