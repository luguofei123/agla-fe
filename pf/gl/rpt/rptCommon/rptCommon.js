...
if (selEnviornmentVar) {
	rpt.nowAgencyCode = selEnviornmentVar.selAgecncyCode; //登录单位代码
	rpt.nowAgencyName = selEnviornmentVar.selAgecncyName; //登录单位名称
	rpt.nowAcctCode = selEnviornmentVar.selAcctCode; //账套代码
	rpt.nowAcctName = selEnviornmentVar.selAcctName; //账套名称
}
...
var ii = "00";
rpt.selectTree = function(treeId, nodes, checkFlag) {
	var myTree;
	var setting = {
		view: {
			selectedMulti: false,
...
rpt.reqVoutype = function() {
	setTimeout(function(){
		var reqUrl
		if($('#accaList .btn-primary').eq(0).attr('data-code')==''){
			if(rpt.isDoubleVousum==1){//双凭证
				reqUrl= rpt.portList.getVoutype + rpt.nowAgencyCode + "/" + rpt.nowSetYear+"/"+rpt.nowAcctCode+"/1,2";
			}else{
				reqUrl= rpt.portList.getVoutype + rpt.nowAgencyCode + "/" + rpt.nowSetYear+"/"+rpt.nowAcctCode+"/*";
			}
		}else{
			reqUrl= rpt.portList.getVoutype + rpt.nowAgencyCode + "/" + rpt.nowSetYear+"/"+rpt.nowAcctCode+"/"+$('#accaList .btn-primary').eq(0).attr('data-code');
		}
		ufma.ajax(reqUrl, "get", "", function(result) {
			...
			if(data.length!=0){
				var selectHtml = "";
				for(var i = 0; i < data.length; i++) {
					var sHtml = ufma.htmFormat('<option data-code='+i+' value="<%=code%>"><%=name%></option>', {
				...
				ufma.showTip('凭证类型不存在！',{},'warning')
				$('#rpt-pzzh-select').html('')
			}
...
rpt.reqAgencyList = function() {
	...
	ufma.ajax(rpt.portList.agencyList, "get", "", function(result) {
		...
		//    	console.info(rpt.nowAgencyName);
	...
	ufma.ajax(rpt.portList.acctList, "get", acctArgu, function(result) {
		...
		if(data.length > 0) {
			...
			if(rpt.nowAcctCode != "" && rpt.nowAcctName != "") {
				//rpt.cbAcct.setValue(rpt.nowAcctCode,rpt.nowAcctName);
				var flag = rpt.cbAcct.val(rpt.nowAcctCode);
				//console.info(rpt.cbAcct.val(rpt.nowAcctCode));
				if(flag == "undefined") {
			...
			if(rpt.rptType != "GL_RPT_VOUSUMMARY" && rpt.rptType != "GL_RPT_CFSTATEMENT") {
				//请求会计科目的推荐列表
				rpt.reqItemTip("ACCO");
			...
			if(rpt.rptType != "GL_RPT_CFSTATEMENT") {
				//请求查询方案列表
				rpt.reqPrjList();
				//请求共享方案列表
				rpt.reqSharePrjList()
...
rpt.reqSelectItems = function() {
	...
	ufma.ajax(url, "POST", [], function(result) {
		...
		//		}else if(rpt.rptType == "GL_RPT_COLUMNAR"){
		//清空新增的辅助项
		$(rpt.namespace + " .rpt-query-box-bottom .rpt-query-box-li").last().prevAll().remove();
		//		}
...
rpt.showAccaList = function(result) {
	...
	if(rpt.isParallelsum == '1' && rpt.isDoubleVousum == '0'){
		for(var i = 0; i < list.length; i++) {
			...
			var btn = ufma.htmFormat('<button class="btn btn-default" data-code="<%=code%>"><%=name%></button>', {
				code: list[i].accaCode,//list[i].accaCode
	...
	if(list.length == 1) {
		$('#accaList .btn-primary').eq(0).attr('data-code','*')
		$('#accaList .btn-primary').eq(0).trigger('click')
		if(rpt.rptType == "GL_RPT_VOUSUMMARY") {
...
rpt.showOptList = function(result) {
	...
	if(rpt.rptType == "GL_RPT_VOUSUMMARY") {
		...
		pzztHtml+='<label class="rpt-check mt-checkbox mt-checkbox-outline"><input checked type="checkbox" id="zf">已作废<span></span></label>'
...
rpt.reqAccItemTree = function(accItemType) {
	...
	ufma.ajaxDef(rpt.portList.accItemTree, "get", treeArgu, callback);
};
//回调函数——创建返回树
...
rpt.showSelectTree = function() {
	$(rpt.namespace).on("click", "div.rpt-tree-view", function(e) {
		...
		var $Data = $(this).siblings("div.rpt-tree-data");
...
//设置查询条件的控制逻辑by wangxin
$(function() {
	$('#rpt-hzfs-select').change(function() {
		$('.rpt-hzfs-check')[$(this).val() == '1' ? 'addClass' : 'removeClass']('hidden');
	});
});

// 表格单击行后选中高亮
rpt.tableTrHighlight = function () {
	if (rpt.namespace === "#glRptChrBook") rpt.namespace = "#glRptChr";
