			...
			initPage:function(){
				...
				ufma.post('/gl/CbBillBook/selectVouCreateRuleSet', wwt, function (result) {
					for(var i=0;i<result.data.length;i++){
						...
						}else if(result.data[i].billStatus=='02'){//背书业务凭证模板
							page.appendHtml('bs',result.data[i])
						}else if(result.data[i].billStatus=='03'){//贴现业务凭证模板
							page.appendHtml('tx',result.data[i])
						}else if(result.data[i].billStatus=='04'){//收款业务凭证模板
							page.appendHtml('sk',result.data[i])
