		...
		return {
			...
			onEventListener: function() {
				...
				$('#btnSave').click(function() {
					...
					argu.op = 0; //新增传0，修改传1
					argu.receivableType = "03"; //登记是01，背书是02，贴现是03
					...
					if($.isNull(businessDate)) {
						ufma.showTip("贴现日期不能为空", function() {

						}, "warning");
						return false;
					}
					if(argu.discountRate == 0) {
						ufma.showTip('贴现率不能为0', function() {}, 'warning');
						return false;
					}
					if(parseFloat(argu.discountRate) > 100) {
						ufma.showTip('贴现率不能超过100%', function() {}, 'warning');
						return false;
					}
					if(argu.discountAmount == 0) {
						ufma.showTip("贴现金额不能为0", function() {

						}, "warning");
						return false;
					}
					if(parseFloat(argu.billfaceAmount) < parseFloat(argu.discountAmount)) {
						ufma.showTip("贴现金额应不大于票面金额", function() {
				...
				$('#discountRate').on("blur", function() {
					...
					var newAmount = window.ownerData.billbook.billfaceAmount * rate / 100;
			...
			initPage: function() {
				...
				$('#discountRate').amtInput();
				$('#discountAmount').amtInput();
				//set form
				//page.setForm();
			},
