		...
		return {
			...
			onEventListener: function() {
				...
				$('#btnSave').click(function() {
					...
					argu.op = 0; //新增传0，修改传1
					argu.receivableType = "04"; //登记是01，背书是02，贴现是03，收款是04
					...
					if($.isNull(businessDate)) {
						ufma.showTip("收款日期不能为空", function() {

						}, "warning");
						return false;
					}
					if(argu.payAmount == 0) {
						ufma.showTip("收款金额不能为0", function() {

						}, "warning");
						return false;
					}
					if(parseFloat(argu.billfaceAmount) < parseFloat(argu.payAmount)) {
						ufma.showTip("收款金额应不大于票面金额", function() {
