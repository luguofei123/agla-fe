$(function () {
	...
	var menuId = 'eb9ef3ac-8319-4fc8-a3c9-19329dcf034b';
	var iValue = 0;
  ...
  var compireMember = []; // 存放记忆列数据
	var divId = 'budgetItemDetailQuery';
	var page = function () {
		var agencyCode = null;
		var oTable, status;
		var tblDt = null; //指标数据
    ...
    var assiEleCode = [];
		return {
     // 解决从监管跳转到指标台账的需求--zsj--2020-08-10 11:11--与yanbx联调
			//初始化单位
			// initAgency: function () {
        ...
        ufma.get("/bg/sysdata/getAgencyList", arguAge, function (result) {
          $('#cbAgency').ufTreecombox({
            ...
            onComplete: function (sender) {
              // 解决从监管跳转到指标台账的需求--zsj--2020-08-10 11:11--与yanbx联调
              if (!$.isNull(page.turnAgencyCode)) {
                $('#cbAgency').getObj().val(page.turnAgencyCode);
              } else if (page.pfData.svAgencyCode) {
			...
			initSearchPnl: function () {
                ...
                // 解决从监管跳转到指标台账的需求--zsj--2020-08-10 11:11--与yanbx联调
                if (!$.isNull(page.turnBgItemCode) && page.turnFlag == true) {
									$('#bgItemCode p').html(page.turnDetailQuery.bgItemCode); //CWYXM-10755 --指标预算执行情况表，编码联查到指标台账，应按所选指标带出预算方案--zsj
									$('.clearBill').css("margin-top", "-29px");
									$('#bgItemCode p').css('color', '#333');
									page.bgItemCode = page.turnDetailQuery.bgItemCode;
									page.turnFlag = false;
								} else {
									page.bgItemCode = [];
                ...
                // 从其他界面跳转到指标台账
								// 解决从监管跳转到指标台账的需求--zsj--2020-08-10 11:11--与yanbx联调
								if ($.isNull(page.turnDetailQuery) || page.turnFlag == false) {
                  ...
                  // 解决从监管跳转到指标台账的需求--zsj--2020-08-10 11:11--与yanbx联调
									element.getObj().val(page.turnPlanId);
								}
							}
						});
					}
        ...
        for (var k = 0; k < planData.planVo_Txts.length; k++) {
          ...
          searchMap[textCode] = $('#'+ textCode).val();
			...
			getSearchMap: function (planData) {
				...
				searchMap['setYear'] = planData.setYear;
				//searchMap['bgItemCode'] = $("input[ name='bgItemCode']").val();
        ...
        if (page.setMenberData && page.setMenberData.length > 0) {
          for (var i = 0; i < page.setMenberData.length; i++) {
            ...
            if ($.inArray(setData, moneyArr) > -1) {
              ...
              column.push({
                data: 'BG_ITEM_ID_MX',
                title: '财政指标ID',
                className: 'BG_ITEM_ID_MX isprint nowrap BGasscodeClass',
                // width: 220,
                "render": function (data) {
                  if (!$.isNull(data)) {
                    return '<code title="' + data + '">' + data + '</code>';
          ...
          if (page.planData) {
            ...
            for (var index = curBgPlanEleMsg.eleCodeArr.length - 1; index >= 0; index--) { //CWYXM-11317 --指标台账联查指标编码，查询方案中的要素顺序建议与预算方案一致--zsj
              ...
              } else if(curBgPlanEleMsg.eleCodeArr[index] != "ISUPBUDGET" && curBgPlanEleMsg.eleCodeArr[index] != "sendDocNum" && curBgPlanEleMsg.eleCodeArr[index] != "bgItemIdMx"){
      ...
      getPlan: function(argu, _callback) {
          ...
          _callback(result.data.items);
			...
			onEventListener: function () {
				...
				$('#bgItemCode').find('.btnSetAccItem').on('click', function () {
					...
					//注释弹窗勾选已选单据--zsj
					//openData.queryData = $('#bgItemCode p').attr('all-data');
					//openData.tableCheck = $('#bgItemCode p').attr('title');
					ufma.open({
						url: 'chooseCode.html',
						title: '指标台账编码选择',
						width: 1000,
				...
				$('#queryTable').on('click', '.common-jump-link', function (e) {
					...
					if (page.turnType == 'vouTurnType') {
						...
						ufma.ajaxDef('/gl/vou/getVou/' + vouGuid, 'get', '', function (result) {
							if (result.data != null) {
								var baseUrl = '/pf/gl/vou/index.html?menuid=f24c3333-9799-439a-94c9-f0cdf120305d&dataFrom=budgetItemDetailQuery&action=query&vouGuid=' + vouGuid + '&agencyCode=' + page.agencyCode + '&acctCode=' + page.tableAcctCode;
        ...
        for (var k = 0; k < planData.planVo_Txts.length; k++) {
          ...
          var formCtrl = $('<input id="'+codeId+'" class="form-control" style=" width:300px;margin-left:3px;"/>').appendTo($curgroupTetx);
			...
			initPage: function () {
				...
				if (!$.isNull(page.turnDetailQuery)) {
					if (!$.isNull(page.turnDetailQuery.bgItemCode)) {
						$('#bgItemCode p').html(page.turnDetailQuery.bgItemCode); //CWYXM-10755 --指标预算执行情况表，编码联查到指标台账，应按所选指标带出预算方案--zsj
						$('.clearBill').css("margin-top", "-29px");
						//$('#bgItemCode').val(page.turnDetailQuery.bgItemCode)
					}
				}
				//page.initQueryTable();
			...
			init: function () {
				...
				page.setShow = false;
        // 解决从监管跳转到指标台账的需求--zsj--2020-08-10 11:11--与yanbx联调
        page.turnDetailQuery = '';
        page.turnAgencyCode = '';
        page.turnPlanId = '';
        page.turnBgItemCode = '';
				page.showTrue = true;
        var myDataFrom = page.GetQueryString("dataFrom");
        var myPlanId = page.GetQueryString("planId");
				// if (!$.isNull(myDataFrom)) {
        //   if (myDataFrom == 'budgetItemBalance') {
        //     page.turnDetailQuery = JSON.parse(window.sessionStorage.getItem("balanceTurnDetailQuery"));
        //   } else if (myDataFrom == 'GA') {
        //     page.turnDetailQuery = myPlanId;
        //   } 
				// 	if (!$.isEmptyObject(page.turnDetailQuery)) {
				// 		page.turnFlag = true;
				// 	} else {
				// 		page.turnFlag = false;
				// 	}
        ...
        if (!$.isNull(myDataFrom)) {
          if (myDataFrom == 'budgetItemBalance') {
            page.turnDetailQuery = JSON.parse(window.sessionStorage.getItem("balanceTurnDetailQuery"));
            page.turnPlanId = page.turnDetailQuery.bgPlanChrId;
            page.turnBgItemCode = page.turnDetailQuery.bgItemCode
          } else if (myDataFrom == 'GA') {
            page.turnAgencyCode =  page.GetQueryString("agencyCode");
            page.turnPlanId = myPlanId;
            page.turnBgItemCode = page.GetQueryString("bgItemCode");
          } 
          // 解决从监管跳转到指标台账的需求--zsj--2020-08-10 11:11--与yanbx联调
          page.turnFlag = true;
        }
