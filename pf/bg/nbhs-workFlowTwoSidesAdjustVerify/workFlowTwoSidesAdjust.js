    ...
    $("a.billRow-a").off("click").on("click", function (e) {
      ...
      ufma.post(url, argu, function (result) {
        ...
        if (!$.isNull(openData)) {
          ...
          openData.planDataIn = curBgPlanData;
          if (statusNav == 'todo') {
            openData.editEnable = true;
          } else if (statusNav == 'done') {
            openData.editEnable = false;
          }
          ufma.open({
    ...
    */
      ...
      var variables = [{
        ...
        },
        {
          ...
          type: "string",
          name: "msgUrl",
         // value: '/pf/bg/nbhs-workFlowTwoSidesAdjustVerify/workFlowTwoSidesAdjust.html?mennuid=1eeebf70-f02d-43df-83d5-ec97c805a843'
      ...
      */
      ...
      if (roleCodeSet.indexOf(roleCode) != -1 && billCurValue != 0) {
        ...
        emiter.emit('approve', {
          ...
          bizExtra: {
            bizTitle: '待审批指标',
            //bizUrl: '/pf/bg/nbhs-workFlowTwoSidesAdjustVerify/workFlowTwoSidesAdjust.html?mennuid=1eeebf70-f02d-43df-83d5-ec97c805a843'
            //由于制单岗和审批岗有不同的界面url，所以将此url传给工作流，然后在门户点击已办/待办跳转对应界面时根据interfaceType=1来请求/bg/public/workflow/getDispenseUrl，从而获取真实url，实现正确跳转--zsj
    ...
    $("#" + tblId + " tbody").on("click", ".mainCancelAuditSpan", function (e) {
      ...
      var ctrlMoney= rowDt.billCur;//金额
      var ctrlDeptNum = rowDt.ctrlDeptNum;//归口部门数量
      var taskId = rowDt.taskId;//$(this).closest('td').prev().find('a').attr('data-taskId') ;
                      ...
                      onComplete: function (aa) {
                        if (aa) {
                          ...
                          emiter.emit('cancel', {
                            ...
                            bizExtra: {
                                bizTitle: '待审批指标',
                                //bizUrl: '/pf/bg/nbhs-workFlowTwoSidesAdjustVerify/workFlowTwoSidesAdjust.html?menuid=1eeebf70-f02d-43df-83d5-ec97c805a843'
                                //由于制单岗和审批岗有不同的界面url，所以将此url传给工作流，然后在门户点击已办/待办跳转对应界面时根据interfaceType=1来请求/bg/public/workflow/getDispenseUrl，从而获取真实url，实现正确跳转--zsj
                                bizUrl: '/bg/public/workflow/getDispenseUrl?interfaceType=1'                               
                              },
                            onCancel: function () {},
