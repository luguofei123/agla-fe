	...
	var page = function () {
		return {
			...
			getBudgetHead: function () {
				...
				column.push({ field: 'createUser', name: '编制人', width: 120 });
				column.push({ field: 'createDate', name: '编制日期', width: 100 });
				column.push({ field: 'checkUser', name: '审核人', width: 100 });
			...
			setBudgetTable: function () {
				...
				var url = bg.getUrl('budget') + '?agencyCode=' + page.agencyCode + '&setYear=' + page.pfData.svSetYear + '&bgReserve=1&billType=8';
			...
			getAdjustHead: function () {
				...
				column1.push(
					{ field: '', name: '指标支付', colspan: 3, width: 200 },
				...
				},
				{
					...
					onKeyup: function (e) {
						/*							var $row = $('tr[id="'+e.rowId+'"]');
													...
													if(hj < 0) {
														ufma.showTip('可执行指标金额不足！', function() {}, 'warning');
				...
				},
				{
					...
					align: 'right',
					render: function (rowid, rowdata, data) {
						return $.formatMoney(data - rowdata.bgItemFpedCur, 2);
					}
				}
				];
				...
				$('#zbtz-data-editor').ufDatagrid({
					...
					},
					{
						...
						action: function () {
							...
							if (checkData.length == 0) {
								ufma.alert('请选择待支付指标！', 'warning');
								return false;
							}
							var data = [];
							for (var i = 0; i < checkData.length; i++) {
								var rowData = checkData[i];
								data.push({ 'billId': page.billId, 'billCode': page.billCode, 'status': 1, 'billItemId': rowData.bgItemId, 'bgItemCode': rowData.bgItemCode });
							}
							page.delItems(data);
						}
					}
					...
					initComplete: function (options, data) {
						$('#zbtz-data-editor tr').on('click', '.btn-del', function (e) {
							...
							page.delItems([{ 'billId': page.billId, 'billCode': page.billCode, 'status': 1, 'billItemId': rowData.bgItemId, 'bgItemCode': rowData.bgItemCode }]);
						});
						$('button[data-toggle="tooltip"]').tooltip();
					}
				});
			...
			initTimeline: function () {
				$('#tzdTimeline').ufTimeline({
					steps: [
						{ step: '选择待支付指标', target: 'pnl-xzzb' },
						{ step: '支付指标', target: 'pnl-tzzb' },
			...
			setEditWindow: function () {
				...
				for (var j = 0; j < page.curBill.billWithItems.length; j++) {
					row = $.extend({}, page.curBill.billWithItems[j]);
					row.bgItemFpedCur = row.useCurCurrent;
					data.push(row);
				};
			...
			save: function () {
				...
				for (var i = 0; i < itemData.length; i++) {
					...
					item.useCurCurrent = item.bgItemFpedCur;
			...
			delItems: function (data) {
				...
				ufma.confirm('您确定要删除所选择指标吗？', function (ac) {
					if (ac) {
						if (window.ownerData.action == 'edit') {
							var argu = { 'agencyCode': page.agencyCode, 'setYear': page.pfData.svSetYear, 'latestOpUser': page.pfData.svUserCode, 'latestOpUserName': page.pfData.svUserName, 'items': data };
							var url = bg.getUrl('delRepay');
							var callback = function (result) {
			...
			onEventListener: function () {
				...
				$('#btn-next').click(function () {
					...
					if (page.timeline.stepIndex() == 1) {
						...
						if (data.length == 0) {
							ufma.showTip('请选择可执行指标！', function () { }, 'error');
							return false;
						}
					}
					page.timeline.next();
