	...
	var page = function () {
		return {
			...
			setTable: function () {
				...
				var url = bg.getUrl('getRepay') + '?agencyCode=' + page.agencyCode + '&bgReserve=1&billType=8';
			...
			showLog: function (rowid) {
				var rowData = $('#zbtz-data').getObj().getRowByTId(rowid);
				var url = bg.getUrl('unacBillLog');
				var argu = { 'billId': rowData.billId };
				var callback = function (result) {
			...
			checkBills: function (data, status) {
				...
				var url = bg.getUrl('auditRepay');
				if (status == 3) {
					url = bg.getUrl('cancelAuditRepay');
				}
				var callback = function (result) {
