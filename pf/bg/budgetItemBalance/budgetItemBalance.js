$(function () {
	...
	var oTable;
	var menuId = '91f4e25a-75af-4cd0-9712-1aab9962ccb9';
	var iValue = 0;
  dm.moreMsgSetting = {};
      ...
      formatPlanData:function(){
        ...
        dm.moreMsgSetting.textCodeArr = textCodeArr;
          ...
          if (page.planData.isFinancialBudget == "1") {
            ...
            $('#zbye-data th').each(function(){
              var $columnName = $(this);
              $(this).find('span').html('')
              // 点击表头后端排序--CWYXM-18061--预算执行情况表需要增加表头排序--zsj
              if ($(this).attr('class').split(' ')[1].trim() == page.orderEleCode && page.orderSort == 'asc') {
                var btn = $('<span class="orderClass"><button class="orderAsc hide"></button><br/><button class="orderDesc"></button></span>').appendTo($columnName) 
              } else if ($(this).attr('class').split(' ')[1].trim() == page.orderEleCode && page.orderSort == 'desc') {
                var btn = $('<span class="orderClass"><button class="orderAsc"></button><br/><button class="orderDesc hide"></button></span>').appendTo($columnName) 
              } else {
                var btn = $('<span class="orderClass"><button class="orderAsc"></button><br/><button class="orderDesc"></button></span>').appendTo($columnName)
              }
			  //ZJGA820-1782 --预算执行情况表部分要素的默认列宽和最小列宽应适当调小些。--zsj
			  // 预算执行情况表的所有列宽度再小一些。
              var w = $(this).width() + 50
              $(this).css('width',w +'px')
            })
            //ZJGA820-1782 --预算执行情况表部分要素的默认列宽和最小列宽应适当调小些。--zsj
            var wTable = $(this).width() + 20 * $('#zbye-data th').length
            $('#zbye-data').css('width',wTable +'px')
            //ZJGA820-1579 指标账表每列的宽度增加可拖拽调整宽度的功能--zsj
            var widthTab = 0;
            $('#zbye-data').tblcolResizable({},function(){
              for(var i=0;i< $("#zbye-data thead").find('th').length;i++){
          ...
          "showDetailBg": showDetailBg,
            // CWYXM-18061---预算执行情况表需要增加表头排序；---zsj
          'orderEleCode': page.orderEleCode,
          'orderSort': page.orderSort,
        ...
        */                
        ...
        // var textArgu = {};
        // for (var m = 0;m < textCodeArr.length; m++) {
        //   items[textCodeArr[m]] = $('#'+textCodeArr[m]).val();
        //   items.push(textArgu);
        // }
				return items;
			},
      ...
      // 预算方案
			initSearchPnl: function () {
                ...
                dm.moreMsgSetting.needSetPlan = page.getPlanItems();
								...
								_bgPub_PNL_ReportFind("budgetItemBalance", dm.moreMsgSetting);
        ...
        // 点击表头后端排序--CWYXM-18061--预算执行情况表需要增加表头排序--zsj
        $('#zbye-data').on('click', 'th .orderAsc', function (e) {
          $(this).addClass('orderActive').siblings().removeClass('orderActive')
          $('.orderDesc').removeClass('orderActive')
          $(this).addClass('hide')
          $(this).parent().find('.orderDesc').removeClass('hide')
          page.orderEleCode = ''
          page.orderSort = ''
          page.orderSort = 'asc'
          page.orderEleCode = $(this).closest('th').attr('class').split(' ')[1]
          $('#btnQry').trigger('click')
        })
          // 点击表头后端排序--CWYXM-18061--预算执行情况表需要增加表头排序--zsj
        $('#zbye-data').on('click', 'th .orderDesc', function (e) {
            $(this).addClass('orderActive').siblings().removeClass('orderActive')
            $('.orderAsc').removeClass('orderActive')
            $(this).addClass('hide')
            $(this).parent().find('.orderAsc').removeClass('hide')
            page.orderEleCode = ''
            page.orderSort = ''
            page.orderSort = 'desc'
            page.orderEleCode =  $(this).closest('th').attr('class').split(' ')[1]
            ...
            获取可以直接用JSON.parse(window.sessionStorage.getItem("dataname"));
					 */
					ufma.removeCache("balanceTurnDetailQuery"); //删除缓存
					ufma.setObjectCache("balanceTurnDetailQuery", page.useTurnArgu);
					var bgMenuId = 'eb9ef3ac-8319-4fc8-a3c9-19329dcf034b'; //凭证id
					var pageTile = "budgetItemBalance";
					if (page.bgItemCode) {
						var url = '/pf/bg/budgetItemDetailQuery/budgetItemDetailQuery.html?menuid=' + bgMenuId + '&dataFrom=' + pageTile + '&action=query';
						...
						uf.openNewPage(page.isCrossDomain, $(this), 'openMenu', baseUrl, false, "指标台账");
					}

				});
				$('#showMethodTip').click(function () {
					if ($("#rptPlanList").find('li').length == 0) {
						$("#rptPlanList ul").append('<li class="tc noplan">无可用方案</li>');
					}
				});
