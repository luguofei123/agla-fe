    ...
    return {
        ...
        ufma.get('/bg/sysdata/getBgPlanArray', argu, function (result) {
          $('#cbBgPlan').ufCombox({ //初始化
            ...
            onComplete: function (sender) {
              //ufma.hideloading();
              //ZJGA820-1764 --指标权限管理模块，点击新增指标界面的预算方案那里没有记忆功能，查询条件预算项目那里不支持手输模糊查询。点击弹出下拉框还挡住了部分输入框。--zsj
              if (!$.isNull(saveAllData.memberPlan)) {
                $('#cbBgPlan').getObj().val(saveAllData.memberPlan);
              } else {
      ...
      initBgPlanItemPnl: function ($pnl, planData) {
        ...
        for (var i = 0; i < items.length; i++) {
          ...
          if (item.eleCode == 'sendDocNum' || item.eleCode == 'bgItemIdMx') {
            ...
            $('<lable class="control-label auto commonControl" title="' + item.eleName + '" style="display:inline-block;width:10em;text-align: right;vertical-align: sub; ">' + item.eleName + '：</lable>').appendTo($curgroup);
        ...
        for (var k = 0; k < planData.planVo_Txts.length; k++) {
          ...
          $('<lable class="control-label auto commonControl" title="' + textItem.eleName + '" style="display:inline-block;width:10em;text-align: right;vertical-align: sub; ">' + textItem.eleName + '：</lable>').appendTo($curgroupTetx);
      ...
      inSave: function () {
        ...
        $('#inTable_wrapper').find(".checkboxes:checked").each(function () {
          ...
          var bgItemData = {};
          if (rowIndex) {
            bgItemData = inOtable.api(false).row(rowIndex).data();
            bgItemData.setId = inOtable.api(false).row(rowIndex).data().bgItemId;
            bgItemData.setCode = inOtable.api(false).row(rowIndex).data().bgItemCode;
            bgItemData.isShow = isShow;
          }
          items.push(bgItemData);
        });
