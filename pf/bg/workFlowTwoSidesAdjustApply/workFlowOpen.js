    ...
    return {
      ...
      initInTable: function () {
        ...
        columns.push({
          ...
          data: "realBgItemCur",
          title: "金额",
          className: "bgPubMoneyCol isprint nowrap",
          //width: "150px",
        ...
        columns.push({
          ...
          title: "余额",
          className: "bgPubMoneyCol print nowrap",
          //width: "150px",
          render: $.fn.dataTable.render.number(',', '.', 2, '') //修改金额千分位处理问题
      ...
      initAdjustInTable: function () {
        ...
        // CWYXM-18408--指标年结涉及采购合同报销模块占用的指标结转完善--zsj
        {
          ...
          data: "bgItemSummary",
          title: '摘要',
          className: "isprint nowrap bgSummary BGThirtyLen",
          //width: "250px",
          "render": function (data, type, rowdata, meta) {
            if (!$.isNull(data)) {
              return '<span title="' + data + '">' + data + '</span>';
        ...
        if (isSendDocNum == "是" || isComeDocNum == "是") {
          targetNum = 6;
        }
        if (isSendDocNum == "是" && isComeDocNum == "是") {
          targetNum = 7;
      ...
      onEventListener: function () {
        ...
        $("#inNext").on("click", function () {
          ...
          if ($('#inTable_wrapper').find('input.checkboxes:checked').length == 0) {
            ...
            $('#adjustInTable,#inPrev,#saveAddAll,#btnSave,#inApplyTitle,#adjustTotal').removeClass('hide');
            page.initAdjustInTable();
        ...
        $("#inPrev").on("click", function () {
          ...
          $('#adjustInTable,#inPrev,#saveAddAll,#btnSave,#adjustInTable_wrapper,#inApplyTitle,#adjustTotal').addClass('hide');
          page.initSearchPnl();
        });
        //输入结转金额进行判断：
        ...
        $(document).on('click', function (e) {
          if ($(e.target).is('.detailLog')) {
            ...
            var dt = inOtable.api(false).row(index).data();
...
*/
      ...
      init: function () {
        ...
        if (window.ownerData.openAction == "inAdd") {
          ...
          $('#adjustInTable,#inApplyTitle,.inCount').removeClass('hide');
