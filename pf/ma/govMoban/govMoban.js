	...
	var page = function() {
		...
		return {
					...
					"columnDefs": [{
						...
						},
						{
							...
							"render": function(data, type, rowdata, meta) {
								...
								return '<a class="common-jump-link" style="display:block;text-indent:' + textIndent + '" title = "' + data + '" href="javascript:;" data-href=\'' + alldata + '\'>' + data + '</a>';
							}
						},
						...
						},
						{
							...
							"render": function(data, type, rowdata, meta) {
								...
								if (isOperate == "1") {
									return '<a class="btn btn-icon-only btn-sm btn-addlower btn-permission" data-toggle="tooltip" ' + addlower + '  action= "addlower" rowid="' + data + '" chrCode="' + chrCode + '" agencyCode="' + agencyCode + '" title="增加下级"  disabled>' +
									'<span class="glyphicon icon-add-subordinate"></span></a>' +
									'<a class="btn btn-icon-only btn-sm btn-start btn-permission" data-toggle="tooltip" ' + active + ' action= "active" rowid="' + data + '" chrCode="' + chrCode + '" agencyCode="' + agencyCode + '" title="启用"  disabled>' +
									'<span class="glyphicon icon-play"></span></a>' +
									'<a class="btn btn-icon-only btn-sm btn-stop btn-permission" data-toggle="tooltip" ' + unactive + ' action= "unactive" rowid="' + data + '" chrCode="' + chrCode + '" agencyCode="' + agencyCode + '" title="停用"  disabled>' +
			...
			setFormEnabled: function() {
				if(page.action == 'edit') {
					ma.isRuled = true;
					$('#expfunc-edt .u-msg-title h4').text("票据类型编辑");
					if($('body').data("code")) {
					...
					if (isOperate == "1") {
						...
						// $(".btn-agyChoose").addClass("disabled");
					}

				} else if(page.action == 'add') {
					$('#expfunc-edt .u-msg-title h4').text("票据类型新增");
					$('#expFunc-chrCode').removeAttr('disabled');
					$('#form-expfunc')[0].reset();
			...
			//获取并渲染票据来源下拉列表
			getBillSourceList: function() {
				var url = "/ma/pub/enumerate/MA_ELE_BILLTYPE_BILL_SOURCE";
				var argu = {
					"rgCode": ma.rgCode,
					"setYear": ma.setYear
				};
