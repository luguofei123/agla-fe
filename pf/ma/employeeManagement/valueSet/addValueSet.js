$(function() {
  ...
  //接口URL集合
  var interfaceURL = {
    saveAsValSet: '/ma/asvalset/saveAsValSet', //新增保存
    updateAsValSet: '/ma/asvalset/updateAsValSet', //修改保存
    getVal: '/ma/asvalset/getVal', //查询
    getPreViewData: '/ma/asvalset/getPreViewData' //预览
  }
  var pageLength = 25
  ...
  var page = (function() {
    return {
      ...
      setformData: function() {
        ...
        /*$("#nameTable2").click(function () {

        });*/
        // $("#nameTable2").find('input[name="isUsed"]').attr('disabled', true);
      },
      // 保存校验
      ...
      onEventListener: function() {
        ...
        $(document).on('mousedown', '.to-up span', function() {
          ...
          var rowindex =
            $(this)
              .parents('tr')
              .index() - 1
          //新增的时候没有重新获取表格数据，导致这里的tableObjData为空 guohx 17515
          page.tableObjData = $('#nameTable2').getObj().getData(); 
          ...
          if (rowindex > 0) {
            ...
            page.showTable(page.tableObjData)
        ...
        $('.btn-preview').on('click', function() {
        	ufma.showloading('正在加载数据请耐心等待...');
        	var sqlStr = $("#valSql").val();
        	var argu = { sqlStr: sqlStr };
            ufma.post(interfaceURL.getPreViewData, argu, function(result) {
              ufma.hideloading()
              if (result.flag === 'success') {
                //_close('sure', result.msg)
            	  page.showTable(result.data);
              } else {
                ufma.showTip(result.msg, function() {}, result.flag)
              }
            })
        ...
        // 停用
        $(document).on('mousedown', '.btn-stop-row', function() {
          var obj = $('#nameTable2').getObj()
          var checkData = obj.getCheckData()
          var $check = $('#nameTable2 .uf-grid-body-view').find('tr.checked')
          if ($check.length > 0) {
            for (var i = 0; i < checkData.length; i++) {
              $check.eq(i).find('.isUsed[name="isUsed"]').text('停用')
              checkData[i].isUsed = '0'
            }
          } else {
            ufma.showTip('请选择要停用的数据', function() {}, 'warning')
            return false
          }
        })
        // 启用
        $(document).on('mousedown', '.btn-start-row', function() {
          var obj = $('#nameTable2').getObj()
          var checkData = obj.getCheckData()
          var $check = $('#nameTable2 .uf-grid-body-view').find('tr.checked')
          if ($check.length > 0) {
            for (var i = 0; i < checkData.length; i++) {
              $check.eq(i).find('.isUsed[name="isUsed"]').text('启用')
              checkData[i].isUsed = '1'
            }
          } else {
            ufma.showTip('请选择要启用的数据', function () {
            }, 'warning')
            return false
          }
        })
